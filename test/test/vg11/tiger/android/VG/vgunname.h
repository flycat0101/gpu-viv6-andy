#ifndef __vgunname_h_
#define __vgunname_h_

#if defined(_VG_APPENDIX)

#undef vgAppendPath
#undef vgAppendPathData
#undef vgChildImage
#undef vgClear
#undef vgClearGlyph
#undef vgClearImage
#undef vgClearPath
#undef vgColorMatrix
#undef vgConvolve
#undef vgCopyImage
#undef vgCopyMask
#undef vgCopyPixels
#undef vgCreateEGLImageTargetKHR
#undef vgCreateFont
#undef vgCreateImage
#undef vgCreateMaskLayer
#undef vgCreatePaint
#undef vgCreatePath
#undef vgDestroyFont
#undef vgDestroyImage
#undef vgDestroyMaskLayer
#undef vgDestroyPaint
#undef vgDestroyPath
#undef vgDrawGlyph
#undef vgDrawGlyphs
#undef vgDrawImage
#undef vgDrawPath
#undef vgFillMaskLayer
#undef vgFinish
#undef vgFlush
#undef vgGaussianBlur
#undef vgGetColor
#undef vgGetError
#undef vgGetImageSubData
#undef vgGetMatrix
#undef vgGetPaint
#undef vgGetParameterVectorSize
#undef vgGetParameterf
#undef vgGetParameterfv
#undef vgGetParameteri
#undef vgGetParameteriv
#undef vgGetParent
#undef vgGetPathCapabilities
#undef vgGetPixels
#undef vgGetString
#undef vgGetVectorSize
#undef vgGetf
#undef vgGetfv
#undef vgGeti
#undef vgGetiv
#undef vgHardwareQuery
#undef vgImageSubData
#undef vgInterpolatePath
#undef vgIterativeAverageBlurKHR
#undef vgLoadIdentity
#undef vgLoadMatrix
#undef vgLookup
#undef vgLookupSingle
#undef vgMask
#undef vgModifyPathCoords
#undef vgMultMatrix
#undef vgPaintPattern
#undef vgParametricFilterKHR
#undef vgPathBounds
#undef vgPathLength
#undef vgPathTransformedBounds
#undef vgPointAlongPath
#undef vgProjectiveMatrixNDS
#undef vgReadPixels
#undef vgRemovePathCapabilities
#undef vgRenderToMask
#undef vgRotate
#undef vgScale
#undef vgSeparableConvolve
#undef vgSetColor
#undef vgSetGlyphToImage
#undef vgSetGlyphToPath
#undef vgSetPaint
#undef vgSetParameterf
#undef vgSetParameterfv
#undef vgSetParameteri
#undef vgSetParameteriv
#undef vgSetPixels
#undef vgSetf
#undef vgSetfv
#undef vgSeti
#undef vgSetiv
#undef vgShear
#undef vgTransformPath
#undef vgTranslate
#undef vgWritePixels

#undef vguArc
#undef vguBevelKHR
#undef vguComputeWarpQuadToQuad
#undef vguComputeWarpQuadToSquare
#undef vguComputeWarpSquareToQuad
#undef vguDropShadowKHR
#undef vguEllipse
#undef vguGlowKHR
#undef vguGradientBevelKHR
#undef vguGradientGlowKHR
#undef vguLine
#undef vguPolygon
#undef vguRect
#undef vguRoundRect
#undef vguTransformClipLineNDS

#endif /* _VG_APPENDIX */
#endif /* __vgrename_h_ */
