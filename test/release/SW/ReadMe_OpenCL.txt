...
1) Uncompress the source code, and set the root directory of test souce code.
   ...
   tar zxvf VIVANTE_GAL_Unified_Src_tst_OCL-addon_<version>.tgz -C $PROJECTS_TEST_DIR.
...
2. Build
   ...
   One sepecial build options for OpenCL test need be enabled as following:

   make -f makefile.linux USE_OPENCL=1 ocl11_tst V_TARGET=clean
   make -f makefile.linux USE_OPENCL=1 ocl11_tst V_TARGET=install
