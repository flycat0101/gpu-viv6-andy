...
/*
 * This is Vxworks platform, still base on fbdev.
 * Just for reference.
 * Customer should realize fbdev driver in kernel.
 *
 */
...
/* Please provide display virtual address. */
#ifndef DEFAULT_FB_MEM
#define DEFAULT_FB_MEM 0
#endif
    ...
    )
{
    ...
    do
    {
        ...
        display->multiBuffer = FRAMEBUFFER_COUNT;
        ...
        if (display->file < 0)
        {
            ...
            while ((display->file == -1) && fbDevicePath[i])
            {
                ...
                display->file = open(fbDevName, O_RDWR, 0);
                i++;
            }
        }
        ...
        /* Please provide the display virtual address. */
        display->memory = DEFAULT_FB_MEM;
...
{
    return vxworks_GetDisplayByIndex(0, Display, Context);
}

gceSTATUS
vxworks_GetDisplayInfo(
...
typedef struct _vxworksDISPLAY_INFO
{
...
gceSTATUS
vxworks_GetDisplayInfoEx(
    IN PlatformDisplayType Display,
    IN PlatformWindowType Window,
    IN gctUINT DisplayInfoSize,
    OUT vxworksDISPLAY_INFO * DisplayInfo
    )
{
    gceSTATUS status = gcvSTATUS_OK;
    struct _FBDisplay* display;
    ...
    if ((display == gcvNULL) || (DisplayInfoSize != sizeof(vxworksDISPLAY_INFO)))
    {
        status = gcvSTATUS_INVALID_ARGUMENT;
        gcmFOOTER();
        return status;
...
{
    return vxworks_SetDisplayVirtual(Display, Window, Offset, X, Y);
}

gceSTATUS
vxworks_CancelDisplayBackbuffer(
    IN PlatformDisplayType Display,
    IN PlatformWindowType Window,
    IN gctPOINTER Context,
    IN gcoSURF Surface,
...
gceSTATUS
vxworks_DeinitLocalDisplayInfo(
    IN PlatformDisplayType Display,
    IN OUT gctPOINTER * localDisplay
    )
{
...
{
    gceSTATUS status = vxworks_GetDisplayInfoEx(Display, Window, DisplayInfoSize, DisplayInfo);
...
gceSTATUS
vxworks_GetDisplayBackbufferEx(
    IN PlatformDisplayType Display,
    IN PlatformWindowType Window,
    IN gctPOINTER  localDisplay,
    OUT gctPOINTER  *  context,
...
{
    return vxworks_GetDisplayBackbuffer(Display, Window, context, surface, Offset, X, Y);
}

gceSTATUS
vxworks_IsValidDisplay(
    IN PlatformDisplayType Display
    )
{
    struct _FBDisplay* display;
    ...
    )
{
    ...
    if (gcmIS_ERROR(vxworks_GetWindowInfo(
                          Display,
                          Window,
                          X,
                          Y,
...
{
    return vxworks_DrawImage(Display,
                           Window,
                           Left,
                           Top,
                           Right,
...
{
    if (gcmIS_ERROR(vxworks_GetPixmapInfo(
                        Display,
                        Pixmap,
    ...
    )
{
    ...
    vxworks_GetDisplay(&display, gcvNULL);
...
{
    vxworks_DestroyDisplay((PlatformDisplayType) Display);
}
...
{
    if (gcmIS_ERROR(vxworks_IsValidDisplay((PlatformDisplayType) Display)))
    {
        return EGL_FALSE;
    }
    ...
    )
{
    ...
    status = vxworks_InitLocalDisplayInfo((PlatformDisplayType) Display->hdc,
                                        &Display->localInfo);
    ...
    )
{
    ...
    status = vxworks_DeinitLocalDisplayInfo((PlatformDisplayType) Display->hdc,
                                          &Display->localInfo);
    ...
    )
{
    ...
    vxworks_GetNativeVisualId((PlatformDisplayType) Display->hdc, &visualId);
    return visualId;
}
    ...
    )
{
    ...
    status = vxworks_GetSwapInterval((PlatformDisplayType) Display->hdc,
                                   Min, Max);
    ...
    )
{
    ...
    status = vxworks_SetSwapInterval((PlatformWindowType)Surface->hwnd, Interval);
    ...
    )
{
    ...
    /* Get Window info. */
    status = vxworks_GetWindowInfoEx((PlatformDisplayType) Display->hdc,
    ...
    gcoOS_ZeroMemory(&dInfo, sizeof (vxworksDISPLAY_INFO));

    status = vxworks_GetDisplayInfoEx2((PlatformDisplayType) Display->hdc,
                                     (PlatformWindowType) Window,
                                     Display->localInfo,
                                     sizeof (vxworksDISPLAY_INFO),
                                     &dInfo);
    ...
    /* Get virtual size. */
    status = vxworks_GetDisplayVirtual((PlatformDisplayType) Display->hdc,
    ...
    )
{
    ...
    vxworks_CreateDrawable(Display->localInfo, (PlatformWindowType) win);
    ...
    )
{
    ...
    vxworks_DestroyDrawable(Display->localInfo, (PlatformWindowType) win);
    ...
    )
{
    ...
    if (Surface->openVG)
    {
            ...
            (info->type != gcvSURF_BITMAP))
        {
            ...
            status = vxworks_SetWindowFormat((PlatformDisplayType) Display->hdc,
                                           (PlatformWindowType) win,
                                           gcvSURF_BITMAP,
                                           format);
    ...
    )
{
    ...
    if (info->fbDirect)
    {
        ...
        status = vxworks_GetDisplayBackbufferEx((PlatformDisplayType) Display->hdc,
                                              (PlatformWindowType) win,
                                               Display->localInfo,
                                               &BackBuffer->context,
                                               &BackBuffer->surface,
    ...
    )
{
    ...
    if (info->fbDirect)
    {
        ...
        status = vxworks_SetDisplayVirtualEx((PlatformDisplayType) Display->hdc,
                                           (PlatformWindowType) win,
                                           BackBuffer->context,
                                           surface,
                                           0,
    ...
    else
    {
        ...
        for (i = 0; i < Region->numRects; i++)
        {
            ...
            /* Draw image. */
            status = vxworks_DrawImageEx((PlatformDisplayType) Display->hdc,
                                       (PlatformWindowType) win,
    ...
    )
{
    ...
    status = vxworks_CancelDisplayBackbuffer((PlatformDisplayType) Display->hdc,
                                           (PlatformWindowType) win,
                                           BackBuffer->context,
                                           surface,
                                           0,
...
{
    return vxworks_SynchronousFlip((PlatformDisplayType)Display->hdc);
}
    ...
    )
{
    ...
    else if (!Surface->newSwapModel)
    {
        ...
        /*
         * In vxworks, back buffers are returned in order. It's safe to return
    ...
    )
{
    ...
    else
    {
        ...
        gcmONERROR(vxworks_CopyPixmapBits((PlatformDisplayType) Info->hdc,
    ...
    )
{
    ...
    {
        /* Call underlying OS layer function to copy pixels. */
        gcmONERROR(vxworks_DrawPixmap((PlatformDisplayType) Info->hdc,
                                    (PlatformPixmapType) Pixmap,
                                    0, 0,
                                    Info->width,
                                    Info->height,
    ...
    )
{
    ...
    status = vxworks_GetPixmapInfoEx((PlatformDisplayType) Display->hdc,
                                   (PlatformPixmapType) Pixmap,
                                   &width,
                                   &height,
                                   &bitsPerPixel,
    ...
    )
{
    ...
    gcmONERROR(vxworks_GetPixmapInfoEx((PlatformDisplayType) Display->hdc,
                                     (PlatformPixmapType) Pixmap,
                                     &pixmapWidth,
                                     &pixmapHeight,
                                     &pixmapBpp,
    ...
    /* Query pixmap bits. */
    status = vxworks_GetPixmapInfo((PlatformDisplayType) Display->hdc,
                                 (PlatformPixmapType) Pixmap,
                                 gcvNULL,
                                 gcvNULL,
                                 gcvNULL,
    ...
    )
{
    ...
    gcmONERROR(
        vxworks_GetPixmapInfoEx((PlatformDisplayType) Display->hdc,
                              (PlatformPixmapType) Pixmap,
                              &width,
                              &height,
                              &bitsPerPixel,
...
VEGLPlatform
vxworks_GetVxworksPlatform(
    void * NativeDisplay
    )
{
    return &vxworksPlatform;
}

static struct eglFbPlatform vxworksBackend =
{
    EGL_PLATFORM_FB_VIV,
    vxworks_GetDisplay,
    vxworks_GetDisplayByIndex,
