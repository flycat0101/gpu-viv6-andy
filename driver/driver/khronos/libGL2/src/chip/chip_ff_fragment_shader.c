    ...
    )
{
    ...
    if (chipCtx->drawClearRectEnabled)
    {
        ...
        status = gcUNIFORM_SetValueF_Ex(Uniform, 1, chipCtx->currProgram->hints, &gc->state.raster.clear.r);
        gcmFOOTER();
        return status;
    }
    else
    ...
    )
{
    ...
    status = gcUNIFORM_SetValueF_Ex(Uniform, gc->constants.numberOfTextureUnits, chipCtx->currProgram->hints, valueArray);
    ...
    )
{
    ...
    for (i = 0; i < gc->constants.numberOfTextureUnits; i++)
    {
        ...
        vector[0] = gc->state.texture.texUnits[i].texObj[enableDim].params.borderColor.r;
        vector[1] = gc->state.texture.texUnits[i].texObj[enableDim].params.borderColor.g;
        vector[2] = gc->state.texture.texUnits[i].texObj[enableDim].params.borderColor.b;
        vector[3] = gc->state.texture.texUnits[i].texObj[enableDim].params.borderColor.a;
        vector += 4;
    }
    ...
    )
{
    ...
    status = glfUsingUniform(
        ...
        "uTexColor",
        gcSHADER_FRAC_X4,
        gc->constants.numberOfTextureUnits,
        set_uTexColor,
    ...
    )
{
    ...
    while ((i < gc->constants.numberOfTextureUnits) && texEnable)
    {
