...
GLvoid DoEvalCoord1(__GLcontext *gc, GLfloat u)
{
    ...
    //    else
    {
        if (gc->state.enables.eval.map1[__GL_C4])
        {
            ...
            DoDomain1(&em, u, &eval[__GL_C4], &gc->state.current.color.r,
                evalData[__GL_C4]);
            (*gc->currentImmediateTable->dispatch.Color4fv)(&gc->state.current.color.r);
        }
    }

    if (gc->state.enables.eval.map1[__GL_T4])
    {
        DoDomain1(&em, u, &eval[__GL_T4], &__GL_ACTIVE_TEXCOORD(gc).x,
            evalData[__GL_T4]);
        (*gc->currentImmediateTable->dispatch.TexCoord4fv)(&__GL_ACTIVE_TEXCOORD(gc).x);
    }
    else if (gc->state.enables.eval.map1[__GL_T3])
    {
        DoDomain1(&em, u, &eval[__GL_T3], &__GL_ACTIVE_TEXCOORD(gc).x,
            evalData[__GL_T3]);
        (*gc->currentImmediateTable->dispatch.TexCoord3fv)(&__GL_ACTIVE_TEXCOORD(gc).x);
    }
    else if (gc->state.enables.eval.map1[__GL_T2])
    {
        DoDomain1(&em, u, &eval[__GL_T2], &__GL_ACTIVE_TEXCOORD(gc).x,
            evalData[__GL_T2]);
        (*gc->currentImmediateTable->dispatch.TexCoord2fv)(&__GL_ACTIVE_TEXCOORD(gc).x);
    }
    else if (gc->state.enables.eval.map1[__GL_T1])
    {
        DoDomain1(&em, u, &eval[__GL_T1], &__GL_ACTIVE_TEXCOORD(gc).x,
            evalData[__GL_T1]);
        (*gc->currentImmediateTable->dispatch.TexCoord1fv)(&__GL_ACTIVE_TEXCOORD(gc).x);
    }
    ...
    if (gc->state.enables.eval.map1[__GL_N3])
    {
        DoDomain1(&em, u, &eval[__GL_N3], &gc->state.current.normal.x,
            evalData[__GL_N3]);
        (*gc->currentImmediateTable->dispatch.Normal3fv)(&gc->state.current.normal.x);
    }
    ...
    if (gc->state.enables.eval.map1[__GL_V4])
    {
        ...
        (*gc->currentImmediateTable->dispatch.Vertex4fv)(&vvec.x);
    }
    else if (gc->state.enables.eval.map1[__GL_V3])
    {
        ...
        (*gc->currentImmediateTable->dispatch.Vertex3fv)(&vvec.x);
    }
    if (restorecolor0)
    {
                  ...
                  StateChange *changes)
{
    ...
    if (gc->state.enables.eval.autonormal)
    {
        if (gc->state.enables.eval.map2[__GL_V4])
        {
            ...
            ComputeNormal2(gc, &vnormal.x, du, dv);
            (*gc->currentImmediateTable->dispatch.Normal3fv)(&vnormal.x);
            if (changes)
            {
    ...
    else
    {
        ...
        {
            DoDomain2(&em, u, v, &eval[__GL_N3], &vnormal.x,
                evalData[__GL_N3]);
            (*gc->currentImmediateTable->dispatch.Normal3fv)(&vnormal.x);
            if (changes)
            {
    ...
    }
    {
        if (gc->state.enables.eval.map2[__GL_C4])
        {
            ...
            DoDomain2(&em, u, v, &eval[__GL_C4], &color.r,
                evalData[__GL_C4]);
            (*gc->currentImmediateTable->dispatch.Color4fv)(&color.r);
            if (changes)
            {
    ...
    {
        DoDomain2(&em, u, v, &eval[__GL_T4], &__GL_ACTIVE_TEXCOORD(gc).x,
            evalData[__GL_T4]);
        (*gc->currentImmediateTable->dispatch.TexCoord4fv)(&texcoord.x);
        if (changes)
        {
    ...
    {
        DoDomain2(&em, u, v, &eval[__GL_T3], &texcoord.x,
            evalData[__GL_T3]);
        (*gc->currentImmediateTable->dispatch.TexCoord3fv)(&texcoord.x);
        if (changes)
        {
            changes->changed |= CHANGE_TEXTURE;
            changes->texture = texcoord;
    ...
    {
        DoDomain2(&em, u, v, &eval[__GL_T2], &texcoord.x,
            evalData[__GL_T2]);
                (*gc->currentImmediateTable->dispatch.TexCoord2fv)(&texcoord.x);
        if (changes)
        {
            changes->changed |= CHANGE_TEXTURE;
            changes->texture = texcoord;
    ...
    {
        DoDomain2(&em, u, v, &eval[__GL_T1], &texcoord.x,
            evalData[__GL_T1]);
        (*gc->currentImmediateTable->dispatch.TexCoord1fv)(&texcoord.x);
        if (changes)
        {
            changes->changed |= CHANGE_TEXTURE;
            changes->texture = texcoord;
...
GLvoid __glEvalMesh1Line(__GLcontext *gc, GLint low, GLint high)
{
    ...
    */
    (*gc->currentImmediateTable->dispatch.Begin)(GL_LINE_STRIP);
    for (i = low; i <= high ; i++)
    {
...
GLvoid __glEvalMesh1Point(__GLcontext *gc, GLint low, GLint high)
{
    ...
    ** accumulation of error.
    */
    (*gc->currentImmediateTable->dispatch.Begin)(GL_POINTS);
    for (i = low; i <= high ; i++)
    {
        DoEvalCoord1(gc, (i == u->n) ? u->finish : (u->start + i * du));
    }
...
GLvoid sendChange(__GLcontext *gc, StateChange *change)
{
    if (change->changed & CHANGE_COLOR)
    {
        ...
        (*gc->currentImmediateTable->dispatch.Color4fv)(&change->color.r);
    }
    if (change->changed & CHANGE_TEXTURE)
    {
        __GL_ACTIVE_TEXCOORD(gc) = change->texture;
        (*gc->currentImmediateTable->dispatch.TexCoord4fv)(&change->texture.x);
    }
    if (change->changed & CHANGE_NORMAL)
    {
        gc->state.current.normal = change->normal;
        (*gc->currentImmediateTable->dispatch.Normal3fv)(&change->normal.x);
    }
    if (change->changed & CHANGE_VERTEX3)
    {
        (*gc->currentImmediateTable->dispatch.Vertex3fv)(&change->vertex.x);
    }
    else if (change->changed & CHANGE_VERTEX4)
    {
        (*gc->currentImmediateTable->dispatch.Vertex4fv)(&change->vertex.x);
    }
                       ...
                       GLint highU, GLint highV)
{
    ...
    for (i = lowU; i < highU; i++)
    {
        ...
        (*gc->currentImmediateTable->dispatch.Begin)(GL_QUAD_STRIP);
        row=0;
...
GLvoid APIENTRY __glim_Map1d(GLenum type, GLdouble u1, GLdouble u2,
                           GLint stride, GLint order, const GLdouble *points)
{
    ...
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_Map1d", DT_GLenum, type, DT_GLdouble, u1, DT_GLdouble, u2, DT_GLint, stride, DT_GLint, order, DT_GLdouble_ptr, points, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_Map1f(GLenum type, GLfloat u1, GLfloat u2,
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_Map1f", DT_GLenum, type, DT_GLfloat, u1, DT_GLfloat, u2, DT_GLint, stride, DT_GLint, order, DT_GLfloat_ptr, points, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_Map2d(GLenum type,
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_Map2d", DT_GLenum, type, DT_GLdouble, u1, DT_GLdouble, u2, DT_GLint, uStride, DT_GLint, uOrder, DT_GLdouble, v1, DT_GLdouble, v2,
        DT_GLint, vStride, DT_GLint, vOrder, DT_GLdouble_ptr, points, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_Map2f(GLenum type,
                           GLfloat u1, GLfloat u2,
                           GLint uStride, GLint uOrder,
                           GLfloat v1, GLfloat v2,
                           GLint vStride, GLint vOrder,
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_Map2f", DT_GLenum, type, DT_GLfloat, u1, DT_GLfloat, u2, DT_GLint, uStride, DT_GLint, uOrder,
        DT_GLfloat, v1, DT_GLfloat, v2, DT_GLint, vStride, DT_GLint, vOrder, DT_GLfloat_ptr, points, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_EvalCoord1d(GLdouble u)
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord1d", DT_GLdouble, u, DT_GLnull);
#endif

    __glDoEvalCoord1(gc, u);
}

GLvoid APIENTRY __glim_EvalCoord1dv(const GLdouble u[1])
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord1dv", DT_GLdouble_ptr, u, DT_GLnull);
#endif

    __glDoEvalCoord1(gc, u[0]);
}

GLvoid APIENTRY __glim_EvalCoord1f(GLfloat u)
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord1f", DT_GLfloat, u, DT_GLnull);
#endif

    __glDoEvalCoord1(gc, u);
}

GLvoid APIENTRY __glim_EvalCoord1fv(const GLfloat u[1])
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord1fv", DT_GLfloat_ptr, u, DT_GLnull);
#endif

    __glDoEvalCoord1(gc, u[0]);
}

GLvoid APIENTRY __glim_EvalCoord2d(GLdouble u, GLdouble v)
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord2d", DT_GLdouble, u, DT_GLdouble, v, DT_GLnull);
#endif

    __glDoEvalCoord2(gc, u, v);
}

GLvoid APIENTRY __glim_EvalCoord2dv(const GLdouble u[2])
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord2dv", DT_GLdouble_ptr, u, DT_GLnull);
#endif

    __glDoEvalCoord2(gc, u[0], u[1]);
}

GLvoid APIENTRY __glim_EvalCoord2f(GLfloat u, GLfloat v)
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord2f", DT_GLfloat, u, DT_GLfloat, v, DT_GLnull);
#endif

    __glDoEvalCoord2(gc, u, v);
}

GLvoid APIENTRY __glim_EvalCoord2fv(const GLfloat u[2])
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalCoord2fv", DT_GLfloat_ptr, u, DT_GLnull);
#endif

    __glDoEvalCoord2(gc, u[0], u[1]);
}

GLvoid APIENTRY __glim_MapGrid1d(GLint nu, GLdouble u0, GLdouble u1)
{
    __GL_SETUP();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_MapGrid1d", DT_GLint, nu, DT_GLdouble, u0, DT_GLdouble, u1, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_MapGrid1f(GLint nu, GLfloat u0, GLfloat u1)
{
    __GL_SETUP_NOT_IN_BEGIN();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_MapGrid1f", DT_GLint, nu, DT_GLfloat, u0, DT_GLfloat, u1, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_MapGrid2d(GLint nu, GLdouble u0, GLdouble u1,
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_MapGrid2d", DT_GLint, nu, DT_GLdouble, u0, DT_GLdouble, u1, DT_GLint, nv, DT_GLdouble, v0, DT_GLdouble, v1, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_MapGrid2f(GLint nu, GLfloat u0, GLfloat u1,
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_MapGrid2f", DT_GLint, nu, DT_GLfloat, u0, DT_GLfloat, u1, DT_GLint, nv, DT_GLfloat, v0, DT_GLfloat, v1, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_EvalMesh1(GLenum mode, GLint low, GLint high)
{
    __GL_SETUP_NOT_IN_BEGIN();

#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalMesh1", DT_GLenum, mode, DT_GLint, low, DT_GLint, high, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_EvalPoint1(GLint i)
{
    __GLevaluatorGrid *u;
    GLfloat du;
    __GL_SETUP();
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalPoint1", DT_GLint, i, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_EvalMesh2(GLenum mode,
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalMesh2", DT_GLenum, mode, DT_GLint, lowU, DT_GLint, highU, DT_GLint, lowV, DT_GLint, highV, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_EvalPoint2(GLint i, GLint j)
{
    __GLevaluatorGrid *u;
    __GLevaluatorGrid *v;
    GLfloat du;
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_EvalPoint2", DT_GLint, i, DT_GLint, j, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_GetMapfv(GLenum target, GLenum query, GLfloat buf[])
{
    __GLevaluator1 *eval1;
    __GLevaluator2 *eval2;
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_GetMapfv", DT_GLenum, target, DT_GLenum, query, DT_GLfloat_ptr, buf, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_GetMapdv(GLenum target, GLenum query, GLdouble buf[])
{
    __GLevaluator1 *eval1;
    __GLevaluator2 *eval2;
    __GLfloat *eval1Data, *eval2Data;
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_GetMapdv", DT_GLenum, target, DT_GLenum, query, DT_GLdouble_ptr, buf, DT_GLnull);
#endif
...
GLvoid APIENTRY __glim_GetMapiv(GLenum target, GLenum query, GLint buf[])
{
    __GLevaluator1 *eval1;
    __GLevaluator2 *eval2;
    __GLfloat *eval1Data, *eval2Data;
...
#if (defined(_DEBUG) || defined(DEBUG))
    if(dbg_logAPIFilter)
        dbgLogFullApi("__glim_GetMapiv", DT_GLenum, target, DT_GLenum, query, DT_GLint_ptr, buf, DT_GLnull);
#endif
