...
GLvoid APIENTRY __glim_PushAttrib(GLuint mask)
{
        ...
        */
        ...
        if (mask & GL_COLOR_BUFFER_BIT) {
            ...
            if(DRAW_FRAMEBUFFER_BINDING_NAME != 0){
                sp->state.raster.drawBuffer[0] = gc->frameBuffer.drawFramebufObj->drawBuffer[0];
            }
#endif
        }
...
GLvoid APIENTRY __glim_PopAttrib(GLvoid)
{
    __GLattributeStack **spp;
    __GLattributeStack *sp;
    ...
    if (spp > &gc->attribute.stack[0]) {
        ...
        if (mask & GL_COLOR_BUFFER_BIT) {
            ...
            if(DRAW_FRAMEBUFFER_BINDING_NAME != 0){
                GLenum drawbuffer = gc->state.raster.drawBuffer[0];
                gc->frameBuffer.drawFramebufObj->drawBuffer[0] = sp->state.raster.drawBuffer[0];
                gc->state.raster = sp->state.raster;
                gc->state.raster.drawBuffer[0] = drawbuffer;
            }
            else
#endif
            {
        ...
        if (mask & GL_POINT_BIT) {
            ...
            /* Delayed notify attribute changes to DP.
            */
            __GL_SET_ATTR_DIRTY_BIT(gc, __GL_DIRTY_ATTRS_2, __GL_POINT_ATTR_BITS);
        }
        ...
        if (mask & GL_SCISSOR_BIT) {
            ...
            /* Delayed notify attribute changes to DP.
            */
            __GL_SET_ATTR_DIRTY_BIT(gc, __GL_DIRTY_ATTRS_1, __GL_SCISSORTEST_ENDISABLE_BIT | __GL_SCISSOR_BIT);
        }
        ...
        if (mask & GL_TRANSFORM_BIT) {
            ...
            __glim_MatrixMode(gc->state.transform.matrixMode);
        ...
        if (mask & GL_MULTISAMPLE_BIT) {
            ...
            __GL_SET_ATTR_DIRTY_BIT(gc, __GL_DIRTY_ATTRS_3, __GL_MULTISAMPLE_ATTR_BITS);
        }
...
GLuint __glCopyContext(__GLcontext *dst, __GLcontext *src, GLuint mask)
{
    ...
    if (mask & GL_ACCUM_BUFFER_BIT) {
        ...
        /* Delayed notify attribute changes to DP.
        */
        __GL_SET_ATTR_DIRTY_BIT(dst, __GL_DIRTY_ATTRS_1, __GL_CLEARACCUM_BIT);
    }
    ...
    if (mask & GL_HINT_BIT) {
        ...
        /* Delayed notify attribute changes to DP.
        */
        __GL_SET_ATTR_DIRTY_BIT(dst, __GL_DIRTY_ATTRS_3, __GL_HINT_BIT);
    }
    ...
    if (mask & GL_POLYGON_BIT) {
        ...
        __GL_SET_ATTR_DIRTY_BIT(dst, __GL_DIRTY_ATTRS_2, __GL_POLYGON_ATTR_BITS);
    }
    ...
    if (mask & GL_POLYGON_STIPPLE_BIT) {
        ...
        /* Delayed notify attribute changes to DP.
        */
        __GL_SET_ATTR_DIRTY_BIT(dst, __GL_DIRTY_ATTRS_2, __GL_POLYGONSTIPPLE_BIT);
    }
    ...
    if (mask & GL_SCISSOR_BIT) {
        ...
        /* Delayed notify attribute changes to DP.
        */
        __GL_SET_ATTR_DIRTY_BIT(dst, __GL_DIRTY_ATTRS_1, __GL_SCISSOR_BIT);
    }
    ...
    if (mask & GL_TRANSFORM_BIT) {
        ...
        __glim_MatrixMode(dst->state.transform.matrixMode);
...
GLvoid APIENTRY __glim_PushClientAttrib(GLbitfield mask)
{
    ...
    if (spp < &gc->attribute.clientStack[gc->constants.maxClientAttribStackDepth]) {
        ...
        if (mask & GL_CLIENT_VERTEX_ARRAY_BIT) {
            sp->clientState.vertexArray = gc->clientState.vertexArray;
        }
    }
    else {
        __glSetError(GL_STACK_OVERFLOW);
...
GLvoid APIENTRY __glim_PopClientAttrib()
{
    __GLclientAttribStack **spp;
    __GLclientAttribStack *sp;
    GLuint mask;
    ...
    if (spp > &gc->attribute.clientStack[0]) {
        ...
        if (mask & GL_CLIENT_PIXEL_STORE_BIT) {
            ...
            */
            ...
            {
                __glBindBuffer(gc, __GL_PIXEL_PACK_BUFFER_INDEX,
                    gc->clientState.pixel.packBufBinding);
            }

            /* Restore PIXEL_UNPACK_BUFFER binding point. */
            if (gc->clientState.pixel.unpackBufBinding !=
                gc->bufferObject.boundBuffer[__GL_PIXEL_UNPACK_BUFFER_INDEX])
            {
                __glBindBuffer(gc, __GL_PIXEL_UNPACK_BUFFER_INDEX,
        ...
        if (mask & GL_CLIENT_VERTEX_ARRAY_BIT) {
            ...
            gc->clientState.vertexArray = sp->clientState.vertexArray;
            ...
            */
            if (arrayBufBinding != gc->bufferObject.boundBuffer[__GL_ARRAY_BUFFER_INDEX])
            {
                __glBindBuffer(gc, __GL_ARRAY_BUFFER_INDEX, arrayBufBinding);
            }
            ...
            */
            if (gc->clientState.vertexArray.elementBufBinding !=
                gc->bufferObject.boundBuffer[__GL_ELEMENT_ARRAY_BUFFER_INDEX])
            {
                __glBindBuffer(gc, __GL_ELEMENT_ARRAY_BUFFER_INDEX,
                    gc->clientState.vertexArray.elementBufBinding);
            }
