/****************************************************************************
*
*    Copyright (c) 2005 - 2016 by Vivante Corp.  All rights reserved.
*
*    The material in this file is confidential and contains trade secrets
*    of Vivante Corporation. This is proprietary information owned by
*    Vivante Corporation. No part of this work may be disclosed,
*    reproduced, copied, transmitted, or used in any way for any purpose,
*    without the express written permission of Vivante Corporation.
*
*****************************************************************************/


#include "gc_gl_context.h"

extern GLint __glElementsPerGroup(GLenum format, GLenum type);
extern GLint __glBytesPerElement(GLenum type);
GLubyte __glMsbToLsbTable[256] = {
    ,
};

GLubyte LowBitsMask[9] = {
    0x00, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f, 0xff,
};

GLubyte HighBitsMask[9] = {
    0x00, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff,
};


/************************************************************************/

GLvoid __glConvertStipple(__GLcontext *gc)
{
    GLubyte b0, b1, b2, b3, *stipple;
    GLuint *dst;
    GLint i;

    stipple = &gc->state.polygonStipple.stipple[0];
    dst = (GLuint *)&gc->state.polygonStipple.stipple[0];
#if __GL_STIPPLE_MSB
    /*
    ** Convert input stipple bytes which are in little endian MSB format
    ** into a single long word, whose high bit represents the left most
    ** X coordinate of a 32 bit span.
    */
    for (i = 0; i < 32; i++)
    {
        b0 = *stipple++;
        b1 = *stipple++;
        b2 = *stipple++;
        b3 = *stipple++;
        *dst++ = (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
    }
#else
    /*
    ** Make 32bit form of the stipple for easier rendering.
    */
    for (i = 0; i < 32; i++)
    {
        b0 = __glMsbToLsbTable[*stipple++];
        b1 = __glMsbToLsbTable[*stipple++];
        b2 = __glMsbToLsbTable[*stipple++];
        b3 = __glMsbToLsbTable[*stipple++];
        *dst++ = b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
    }
#endif
}

/************************************************************************/

/*
** Compute memory required for internal packed array of data of given type
** and format.
*/
GLint __glImageSize(GLsizei width, GLsizei height, GLenum format, GLenum type)
{
    GLint bytes_per_row;
    GLint components;

    components= __glElementsPerGroup(format, type);
    if (type == GL_BITMAP)
    {
        bytes_per_row = (width + 7) >> 3;
    }
    else
    {
        bytes_per_row = __glBytesPerElement(type) * width;
    }
    return (bytes_per_row * height * components);
}

GLint __glImageSize3D(GLsizei width, GLsizei height, GLsizei depth,
                      GLenum format, GLenum type)
{
    return (depth * __glImageSize(width, height, format, type));
}

/*
** Extract array from user's data applying all pixel store modes.
** The internal packed array format used has LSB_FIRST = FALSE and
** ALIGNMENT = 1.
*/
GLvoid __glFillImage3D(__GLcontext *gc,
                     GLsizei width, GLsizei height, GLsizei depth,
                     GLenum format, GLenum type,
                     const GLvoid *userdata, GLubyte *newimage)
{
    GLint components;
    GLint element_size;
    GLint rowsize;
    GLint imagesize;
    GLint padding;
    GLint line_length, alignment, skip_pixels, skip_lines, lsb_first, swap_bytes;
    GLint groups_per_line;
    GLint group_size;
    GLint elements_per_line;
    const GLubyte *start;
    const GLubyte *rowStart;
    const GLubyte *src;
    GLubyte *dst;
    GLint h, i, j, k;

    /* The client state of the dlist create time affect the commands within dlist. So when encountering
    ** client state, we should know whether the command are within the execution of a dlist.
    */
    if (gc->dlist.pCurrentDlist)
    {
        line_length = 0;
        alignment = 1;
        skip_pixels = 0;
        skip_lines = 0;
        lsb_first = GL_FALSE;
        swap_bytes = GL_FALSE;
    }
    else
    {
        line_length = gc->clientState.pixel.unpackModes.lineLength;
        alignment = gc->clientState.pixel.unpackModes.alignment;
        skip_pixels = gc->clientState.pixel.unpackModes.skipPixels;
        skip_lines = gc->clientState.pixel.unpackModes.skipLines;
        lsb_first = gc->clientState.pixel.unpackModes.lsbFirst;
        swap_bytes = gc->clientState.pixel.unpackModes.swapEndian;
    }

    components = __glElementsPerGroup(format, type);
    if (line_length > 0)
    {
        groups_per_line = line_length;
    }
    else
    {
        groups_per_line = width;
    }

    /* All formats except GL_BITMAP fall out trivially */
    if (type == GL_BITMAP)
    {
        GLint elements_left;
        GLint bit_offset;
        GLint current_byte;
        GLint next_byte;
        GLint high_bit_mask;
        GLint low_bit_mask;

        rowsize = (groups_per_line * components + 7) / 8;
        padding = (rowsize % alignment);
        if (padding)
        {
            rowsize += alignment - padding;
        }
        start = ((const GLubyte*) userdata) + skip_lines * rowsize +
                (skip_pixels * components / 8);
        bit_offset = (skip_pixels * components) % 8;
        high_bit_mask = LowBitsMask[8-bit_offset];
        low_bit_mask = HighBitsMask[bit_offset];
        elements_per_line = width * components;
        dst = newimage;
        for (i = 0; i < height; i++)
        {
            elements_left = elements_per_line;
            src = start;
            while (elements_left)
            {
                /* First retrieve low bits from current byte */
                if (lsb_first)
                {
                    current_byte = __glMsbToLsbTable[src[0]];
                }
                else
                {
                    current_byte = src[0];
                }
                if (bit_offset)
                {
                    /* Need to read next byte to finish current byte */
                    if (elements_left > (8 - bit_offset))
                    {
                        if (lsb_first)
                        {
                            next_byte = __glMsbToLsbTable[src[1]];
                        }
                        else
                        {
                            next_byte = src[1];
                        }
                        current_byte =
                            ((current_byte & high_bit_mask) << bit_offset) |
                            ((next_byte & low_bit_mask) >> (8 - bit_offset));
                    }
                    else
                    {
                        current_byte =
                            ((current_byte & high_bit_mask) << bit_offset);
                    }
                }
                if (elements_left >= 8)
                {
                    *dst = (GLubyte) current_byte;
                    elements_left -= 8;
                }
                else
                {
                    *dst = (GLubyte)
                           (current_byte & HighBitsMask[elements_left]);
                    elements_left = 0;
                }
                dst++;
                src++;
            }
            start += rowsize;
        }
    }
    else
    {
        element_size = (GLint)__glBytesPerElement(type);
        group_size = element_size * components;
        if (element_size == 1) swap_bytes = 0;

        rowsize = groups_per_line * group_size;
        padding = (rowsize % alignment);
        if (padding)
        {
            rowsize += alignment - padding;
        }

        imagesize = height * rowsize;

        start = ((const GLubyte*) userdata)
                + skip_lines * rowsize
                + skip_pixels * group_size;
        dst = newimage;
        elements_per_line = width * components;

        if (swap_bytes)
        {
            for (h = 0; h < depth; h++)
            {
                rowStart = start;
                for (i = 0; i < height; i++)
                {
                    src = rowStart;
                    for (j = 0; j < elements_per_line; j++)
                    {
                        for (k = 1; k <= element_size; k++)
                        {
                            dst[k-1] = src[element_size - k];
                        }
                        dst += element_size;
                        src += element_size;
                    }
                    rowStart += rowsize;
                }
                start += imagesize;
            }
        }
        else
        {
            for (h = 0; h < depth; h++)
            {
                if (rowsize == elements_per_line * element_size)
                {
                    /* Ha!  This is mondo easy! */
                    __GL_MEMCOPY(dst, start, height * rowsize);
                    dst += height * rowsize;
                }
                else
                {
                    src = start;
                    for (i = 0; i < height; i++)
                    {
                        __GL_MEMCOPY(dst, src, elements_per_line * element_size);
                        dst += elements_per_line * element_size;
                        src += rowsize;
                    }
                }
                start += imagesize;
            }
        }
    }
}

GLvoid __glFillImage(__GLcontext *gc, GLsizei width, GLsizei height,
                   GLenum format, GLenum type,
                   const GLvoid *userdata, GLubyte *newimage)
{
    __glFillImage3D(gc, width, height, 1, format, type, userdata, newimage);
}


/*
** Insert array into user's data applying all pixel store modes.
** The internal packed array format used has LSB_FIRST = FALSE and
** ALIGNMENT = 1.  __glEmptyImage() because it is the opposite of
** __glFillImage().
*/
GLvoid __glEmptyImage(__GLcontext *gc, GLsizei width, GLsizei height,
                    GLenum format, GLenum type,
                    const GLubyte *oldimage, GLvoid *userdata)
{
    GLint components;
    GLint element_size;
    GLint rowsize;
    GLint padding;
    GLint line_length, alignment, skip_pixels, skip_lines, lsb_first, swap_bytes;
    GLint groups_per_line;
    GLint group_size;
    GLint elements_per_line;
    GLubyte *start;
    GLubyte *iter;
    const GLubyte *iter2;
    GLint i, j, k;

    if (gc->dlist.pCurrentDlist)
    {
        line_length = 0;
        alignment = 1;
        skip_pixels = 0;
        skip_lines = 0;
        lsb_first = GL_FALSE;
        swap_bytes = GL_FALSE;
    }
    else
    {
        line_length = gc->clientState.pixel.unpackModes.lineLength;
        alignment = gc->clientState.pixel.unpackModes.alignment;
        skip_pixels = gc->clientState.pixel.unpackModes.skipPixels;
        skip_lines = gc->clientState.pixel.unpackModes.skipLines;
        lsb_first = gc->clientState.pixel.unpackModes.lsbFirst;
        swap_bytes = gc->clientState.pixel.unpackModes.swapEndian;
    }

    components = __glElementsPerGroup(format, type);
    if (line_length > 0)
    {
        groups_per_line = line_length;
    }
    else
    {
        groups_per_line = width;
    }

    /* All formats except GL_BITMAP fall out trivially */
    if (type == GL_BITMAP)
    {
        GLint elements_left;
        GLint bit_offset;
        GLint current_byte;
        GLint high_bit_mask;
        GLint low_bit_mask;
        GLint write_mask;
        GLubyte write_byte;

        rowsize = (groups_per_line * components + 7) / 8;
        padding = (rowsize % alignment);
        if (padding)
        {
            rowsize += alignment - padding;
        }
        start = ((GLubyte*) userdata) + skip_lines * rowsize +
                (skip_pixels * components / 8);
        bit_offset = (skip_pixels * components) % 8;
        high_bit_mask = LowBitsMask[8-bit_offset];
        low_bit_mask = HighBitsMask[bit_offset];
        elements_per_line = width * components;
        iter2 = oldimage;
        for (i = 0; i < height; i++)
        {
            elements_left = elements_per_line;
            iter = start;
            write_mask = high_bit_mask;
            write_byte = 0;
            while (elements_left)
            {
                /* Set up write_mask (to write to current byte) */
                if (elements_left + bit_offset < 8)
                {
                    /* Need to trim write_mask */
                    write_mask &= HighBitsMask[bit_offset+elements_left];
                }

                if (lsb_first)
                {
                    current_byte = __glMsbToLsbTable[iter[0]];
                }
                else
                {
                    current_byte = iter[0];
                }

                if (bit_offset)
                {
                    write_byte |= (GLubyte) (iter2[0] >> bit_offset);
                    current_byte = (current_byte & ~write_mask) |
                                   (write_byte & write_mask);
                    write_byte = (GLubyte) (iter2[0] << (8 - bit_offset));
                }
                else
                {
                    current_byte = (current_byte & ~write_mask) |
                                   (iter2[0] & write_mask);
                }

                if (lsb_first)
                {
                    iter[0] = __glMsbToLsbTable[current_byte];
                }
                else
                {
                    iter[0] = (GLubyte) current_byte;
                }

                if (elements_left >= 8)
                {
                    elements_left -= 8;
                }
                else
                {
                    elements_left = 0;
                }
                iter2++;
                iter++;
                write_mask = 0xff;
            }
            if (write_byte)
            {
                /* Some data left over that still needs writing */
                write_mask &= low_bit_mask;
                if (lsb_first)
                {
                    current_byte = __glMsbToLsbTable[iter[0]];
                }
                else
                {
                    current_byte = iter[0];
                }

                current_byte = (current_byte & ~write_mask) |
                               (write_byte & write_mask);

                if (lsb_first)
                {
                    iter[0] = __glMsbToLsbTable[current_byte];
                }
                else
                {
                    iter[0] = (GLubyte) current_byte;
                }
            }
            start += rowsize;
        }
    }
    else
    {
        element_size = (GLint)__glBytesPerElement(type);
        group_size = element_size * components;
        if (element_size == 1) swap_bytes = 0;

        rowsize = groups_per_line * group_size;
        padding = (rowsize % alignment);
        if (padding)
        {
            rowsize += alignment - padding;
        }
        start = ((GLubyte*) userdata) + skip_lines * rowsize
                + skip_pixels * group_size;
        iter2 = oldimage;
        elements_per_line = width * components;

        if (swap_bytes)
        {
            for (i = 0; i < height; i++)
            {
                iter = start;
                for (j = 0; j < elements_per_line; j++)
                {
                    for (k = 1; k <= element_size; k++)
                    {
                        iter[k-1] = iter2[element_size - k];
                    }
                    iter2 += element_size;
                    iter += element_size;
                }
                start += rowsize;
            }
        }
        else
        {
            if (rowsize == elements_per_line * element_size)
            {
                /* Ha!  This is mondo easy! */
                __GL_MEMCOPY(start, iter2,
                             elements_per_line * element_size * height);
            }
            else
            {
                iter = start;
                for (i = 0; i < height; i++)
                {
                    __GL_MEMCOPY(iter, iter2, elements_per_line * element_size);
                    iter2 += elements_per_line * element_size;
                    iter += rowsize;
                }
            }
        }
    }
}

