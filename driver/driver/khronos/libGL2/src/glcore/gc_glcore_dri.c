...
extern GLuint __glLoseCurrent(__GLcontext *gc, GLvoid **thrArea);
extern GLuint __glMakeCurrent(__GLcontext *gc, GLuint thrHashId, GLvoid **thrArea);
...
extern GLint __glDpInitialization(GLint processID, __GLdeviceStruct deviceEntry[]);
...
static GLvoid
__vivImpInternalSwapBuffers(__GLcontext *gc,  GLboolean bSwapFront)
{
    vivDriMirror *pDriMirror;
    __DRIdrawablePrivate *driDrawPriv;
...
__GLimports imports = {
    ...
    __vivImpWarning,
    __vivImpFatal,
    NULL, //__vivSetProcTable,
    __vivImpNoop, //__vivLockDrawableMutex,
    __vivImpNoop, //__vivUnlockDrawableMutex,
    __vivImpNoop, //__vivCreateMutex,
    __vivImpNoop, //__vivDestroyMutex,
    __vivImpNoop, //__vivLockMutex,
    __vivImpNoop, //__vivUnlockMutex,
    __vivImpGetHWND,
    NULL,
    NULL,
...
GLvoid vivGetLock( __GLcontext *gc, GLuint flags )
{
    ...
    */
    ...
    if (changeMask) {
        ...
        /* notify the swp to update the swp specific drawable information */
        if (glPriv->swp.updateDrawable) {
            (*glPriv->swp.updateDrawable)(glPriv);
        }
...
vivInitDriver( __DRIscreenPrivate *sPriv )
{
    ...
    */
    *sPriv->nopDispatchPtrAddr = &__glNopDispatchFuncTable.dispatch;

    __glDpInitialization(getpid(), &__glDevicePipeEntry[sPriv->myNum]);
                    ...
                    GLboolean isPixmap)
{
    ...
    glPriv->lock = NULL; //__wglCreateDrawableMutex();
...
vivDestroyDrawableBuffers(__GLdrawablePrivate *glPriv)
{
    ...
    /* Free swp specific freeBuffers function */
    if (glPriv->swp.freeBuffers) {
        (*glPriv->swp.freeBuffers)(glPriv);
    }
...
vivDestroyDrawable(__DRIdrawablePrivate *driDrawPriv)
{
    ...
    /* Free the swp specific data structure attached to the drawable */
    if (glPriv->swp.destroyPrivateData) {
        (*glPriv->swp.destroyPrivateData)(glPriv);
    }
...
#if DEBUG
    curGc = _glapi_get_context();
    if (curGc == NULL || gc != curGc) {
        printf("There is no current context in SwapBuffers! \n");
    }
#endif
                    ...
                    __DRIdrawablePrivate *driReadPriv )
{
    ...
    /* Get threadId and thrHashId */
    threadId = _glthread_GetID();
    thrHashId = getThreadHashId(threadId);

    /* Call GLcore makecurrent function */
    if (__glMakeCurrent(gc, thrHashId, &threadHashTable[thrHashId].thrArea))
    {
        /* Set the current GL context */
        _glapi_set_context(gc);

        /* Set the current GL API dispatch table */
...
vivLoseCurrent( __DRIcontextPrivate *driContextPriv )
{
    ...
    /* Call GLcore makecurrent function */
    if (__glLoseCurrent(gc, &threadHashTable[thrHashId].thrArea))
    {
        /* Set the current GL context to NULL */
        _glapi_set_context(NULL);

        /* Set the current GL API dispatch table to Nop API table */
        _glapi_set_dispatch((GLvoid *)&__glNopDispatchFuncTable.dispatch);
    }
    else {
        retVal = GL_FALSE;
    }
