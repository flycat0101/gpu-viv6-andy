...
GLvoid __glComputeRequiredInputMask(__GLcontext *gc)
{
    ...
    if (gc->shaderProgram.vertShaderEnable)
    {
        ...
        {
            vsInputMask = gc->shaderProgram.currentShaderProgram->bindingInfo.vsInputMask;
        }
        else
        {
            GL_ASSERT(0);
    ...
    if (gc->shaderProgram.fragShaderEnable)
    {
        if (gc->shaderProgram.currentShaderProgram)
        {
            psInputMask = gc->shaderProgram.currentShaderProgram->bindingInfo.psInputMask;
        }
        else
        {
            GL_ASSERT(0);
...
GLvoid __glSwitchToNewPrimtiveFormat_Material(__GLcontext *gc, GLuint attFmtIdx)
{
    ...
    GLuint inputOffsetDW[__GL_TOTAL_VERTEX_ATTRIBUTES];
    GLuint inputSize[__GL_TOTAL_VERTEX_ATTRIBUTES];
...
GLvoid APIENTRY __glim_Begin(GLenum mode)
{
    __GL_SETUP();
...
GLvoid APIENTRY __glim_End(GLvoid)
{
    __GL_SETUP();
...
GLvoid APIENTRY __glim_End_Error(GLvoid)
{
    __GL_SETUP();
