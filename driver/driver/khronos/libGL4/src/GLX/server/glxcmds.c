...
*/
static GLvoid ChangeCurrentContext(__GLXclientState *cl, __GLXcontext *glxc,
                GLXContextTag tag)
{
    __GLXcontext **table = cl->currentContexts;
    table[tag-1] = glxc;
...
static GLvoid StopUsingContext(__GLXcontext *glxc)
{
    if (glxc) {
...
static GLvoid StartUsingContext(__GLXclientState *cl, __GLXcontext *glxc)
{
           ...
           GLXContextID contextId, GLXContextTag tag )
{
    ...
    __GLcontextInterface *gc, *prevgc;
    ...
    */
    if (!(*prevgc->exports.loseCurrent)((__GLcontext *)prevgc, NULL, NULL)) {
        return __glXBadContext;
    }
    ...
    if (!(*gc->exports.makeCurrent)((__GLcontext *)gc, NULL, NULL, GL_FALSE)) {
        glxc->drawPriv = NULL;
        glxc->readPriv = NULL;
        return __glXBadContext;
    }
    ...
    if (!__glXResizeDrawableBuffers(drawPriv)) {
        ...
        (*gc->exports.loseCurrent)((__GLcontext *)gc, NULL, NULL);
        glxc->drawPriv = NULL;
        glxc->readPriv = NULL;
        return __glXBadContext;
    }
...
#define WRITE_PAIR(tag,value) \
    ...
    WRITE_PAIR( GLX_BUFFER_SIZE,      modes->rgbaBits );
