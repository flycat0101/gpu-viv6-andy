...
static GLvoid TransposeMatrixf(GLfloat m[16])
{
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < i; j++) {
...
/* Used for GL_ARB_transpose_matrix */
static GLvoid TransposeMatrixb(GLboolean m[16])
{
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < i; j++) {
...
/* Used for GL_ARB_transpose_matrix */
static GLvoid TransposeMatrixd(GLdouble m[16])
{
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < i; j++) {
...
/* Used for GL_ARB_transpose_matrix */
static GLvoid TransposeMatrixi(GLint m[16])
{
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < i; j++) {
...
GLvoid __indirect_glGetPointerv(GLenum pname, GLvoid **params)
{
    ...
    switch(pname) {
      case GL_VERTEX_ARRAY_POINTER:
        *params = (GLvoid *)gc->state.vertArray.vertex.ptr;
        return;
      case GL_NORMAL_ARRAY_POINTER:
        *params = (GLvoid *)gc->state.vertArray.normal.ptr;
        return;
      case GL_COLOR_ARRAY_POINTER:
        *params = (GLvoid *)gc->state.vertArray.color.ptr;
        return;
      case GL_INDEX_ARRAY_POINTER:
        *params = (GLvoid *)gc->state.vertArray.index.ptr;
        return;
      case GL_TEXTURE_COORD_ARRAY_POINTER:
        *params = (GLvoid *)gc->state.vertArray.texCoord[gc->state.vertArray.activeTexture].ptr;
        return;
      case GL_EDGE_FLAG_ARRAY_POINTER:
        *params = (GLvoid *)gc->state.vertArray.edgeFlag.ptr;
        return;
      case GL_FEEDBACK_BUFFER_POINTER:
        *params = (GLvoid *)gc->feedbackBuf;
        return;
      case GL_SELECTION_BUFFER_POINTER:
        *params = (GLvoid *)gc->selectBuf;
        return;
      default:
        __glXSetError(gc, GL_INVALID_ENUM);
        return;
