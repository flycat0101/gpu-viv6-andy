    ...
    )
{
    ...
    while((i < glvMAX_LIGHTS) && enableMask) {
        if (enableMask & 1) {
            valueArray[index++] = gc->state.light.source[i].positionEye.f.x;
            valueArray[index++] = gc->state.light.source[i].positionEye.f.y;
            valueArray[index++] = gc->state.light.source[i].positionEye.f.z;
            valueArray[index++] = gc->state.light.source[i].positionEye.f.w;
        } else {
            index += 4;
        }
        i++;
    ...
    )
{
    ...
    for (i = 0; i < glvMAX_CLIP_PLANES; i++) {
        valueArray[index++] = gc->state.transform.eyeClipPlanes[i].f.x;
        valueArray[index++] = gc->state.transform.eyeClipPlanes[i].f.y;
    ...
    )
{
    ...
    while((i < glvMAX_LIGHTS) && enableMask) {
        if (enableMask & 1) {
            vPpli[index++] = gc->state.light.source[i].positionEye.f.x;
            vPpli[index++] = gc->state.light.source[i].positionEye.f.y;
            vPpli[index++] = gc->state.light.source[i].positionEye.f.z;
            vPpli[index++] = gc->state.light.source[i].positionEye.f.w;
        } else {
            index += 4;
        }
        i++;
    ...
    )
{
    ...
    while((i < glvMAX_LIGHTS) && enableMask) {
        if (enableMask & 1) {
            gcChipUtilNorm3Vector4f(gc->state.light.source[i].direction.v,
                            &vNormedSdli[i * 4]);
        }
    ...
    )
{
    ...
    status = glfUsingUniform(
        ...
        "uTexMatrix",
        gcSHADER_FRAC_X4,
        4 * gcmMIN(shaderCaps->maxTextureSamplers,8),
        set_uTexMatrix,
    ...
    do
    {
        if (ShaderControl->rVPpli == 0)
        {
            ...
            */
            ...
            {
                if (!(gc->state.light.source[LightIndex].positionEye.f.w == 0.0f))
                {
                    /* Compute current position in eye space. */
                    gcmERR_BREAK(pos2Eye(gc, ShaderControl));
            ...
            if ((LightIndex >= 0) &&
                (gc->state.light.source[LightIndex].positionEye.f.w == 0.0f))
            {
