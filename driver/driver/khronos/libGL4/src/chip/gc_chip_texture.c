...
#ifdef OPENGL40
/* temporiy set Rectangle as CUBEMAP */
    gcvTEXTURE_UNKNOWN,       /*__GL_TEXTURE_RECTANGLE_INDEX*/
    ...
    )
{
    ...
    scale = gcChipUtilFloatFromRaw(Value, Type);
    ...
    )
{
    ...
    case GL_UNSIGNED_INT_5_9_9_9_REV:
        gcmFOOTER_ARG("result=0x%04x", gcvENDIAN_SWAP_DWORD);
        return gcvENDIAN_SWAP_DWORD;
...
static gceSTATUS setTextureWrapperFormat(
    __GLcontext* gc,
    ...
    )
{
    ...
    gcmHEADER_ARG("Context=0x%x Texture=0x%x Format=0x%04x", gc, texObj, Format);
    ...
    {
    case GL_ALPHA:
        texInfo->combineFlow.targetEnable = gcSL_ENABLE_W;
    ...
    case GL_DEPTH_STENCIL:
        ...
        texInfo->combineFlow.argSwizzle   = gcSL_SWIZZLE_XYZZ;
        break;
    ...
    case GL_BGRA_EXT:
    case GL_INTENSITY:
        texInfo->combineFlow.targetEnable = gcSL_ENABLE_XYZW;
        texInfo->combineFlow.tempEnable   = gcSL_ENABLE_XYZW;
        texInfo->combineFlow.tempSwizzle  = gcSL_SWIZZLE_XYZW;
        texInfo->combineFlow.argSwizzle   = gcSL_SWIZZLE_XYZW;
        break;
    ...
    const GLvoid *buf)
{
    ...
    gcmHEADER_ARG("gc=0x%x texObj=0x%x face=%d level=%d buf=0x%x", gc, texObj, 0, level, buf);
...
GLboolean __glChipTexSubImage1D(
    __GLcontext* gc,
    __GLtextureObject *texObj,
    GLint level,
    GLint xoffset,
    ...
    const GLvoid* buf)
{
    ...
    gcmHEADER_ARG("gc=0x%x texObj=0x%x face=%d level=%d xoffset=%d yoffset=%d "
                  ...
                  gc, texObj, 0, level, xoffset, 0, width, 1, buf);

    gcmONERROR(gcChipTexSubImage(gc, texObj, 0, level, xoffset, 0, 0, width, 1, 1, buf));
...
GLboolean __glChipCopyTexImage1D(
__GLcontext* gc,
__GLtextureObject *texObj,
GLint level,
GLint x,
GLint y)
{
    ...
    gcmHEADER_ARG("gc=0x%x texObj=0x%x face=%d level=%d x=%x y=%d",
                   gc, texObj, 0, level, x, y);

    gcmONERROR(gcChipCopyTexImage(gc, texObj, 0, level, x, y));
...
GLint xoffset)
{
    ...
    gcmHEADER_ARG("gc=0x%x texObj=0x%x face=%d level=%d x=%x y=%d width=%d hieght=%d xoffset=%d yoffset=%d",
                   gc, texObj, 0, level, x, y, width, 1, xoffset, 0);

    gcmONERROR(gcChipCopyTexSubImage(gc, texObj, 0, level, x, y, xoffset, 0, 0, width, 1));
...
GLboolean __glChipCompressedTexImage1D(__GLcontext* gc,
    __GLtextureObject *texObj,
    GLint level,
    const GLvoid* buf)
{
    ...
    )
{
    ...
    gcmHEADER_ARG("gc=0x%x texObj=0x%x face=%d level=%d xoffset=%d yoffset=%d"
                  "width=%d height=%d buf=0x%x size=%d",
                   gc, texObj, 0, level, xoffset, 0, width, 1, buf, size);

    gcmONERROR(gcChipCompressedTexSubImage(gc, texObj, 0, level, xoffset, 0, 0, width, 1, 1, buf, size));
            ...
            if (mipmap->formatInfo->stencilSize > 0)
            {
                if (!chipMipLevel->stencilOpt)
                {
                    gcmONERROR(gcoOS_Allocate(gcvNULL,
                                              ...
                                              (gctPOINTER*)&chipMipLevel->stencilOpt));
                    gcoOS_ZeroMemory(chipMipLevel->stencilOpt, numSlices * gcmSIZEOF(__GLchipStencilOpt));
                }
    ...
    )
{
    ...
    do
    {
        ...
        __GLTextureEnableState * es = gcvNULL;
        __GLtextureUnitState *tex = gcvNULL;
        __GLtextureEnvState *tes = gcvNULL;
#endif
...
#ifdef OPENGL40
    if (chipCtx->texture.sampler)
    {
        gcoOS_Free(gcvNULL, (gctPOINTER)chipCtx->texture.sampler);
        chipCtx->texture.sampler = (glsTEXTURESAMPLER *)gcvNULL;
    }
#endif
    gcmFOOTER();
    /* Return result. */
