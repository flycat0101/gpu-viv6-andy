...
arch_int32 * initOptimizeGraphInfo(
    arch_int32 operationCount
    )
{
    archSTATUS status = archSTATUS_OK;
    arch_int32 *pTMP = NULL;

    status = archAllocateMemory(sizeof(arch_int32) * operationCount, (archPOINTER *)&pTMP);
    if (archIS_ERROR(status)) goto error;
    memset(pTMP, -1, sizeof(arch_int32) * operationCount);
    return pTMP;

error:
    if (pTMP != NULL) {
        deInitOptimizeGraphInfo(pTMP);
    }
    archInfo("ERROR: initArchOpInfo() return out-of-memory\n");
    return NULL;
}

void deInitOptimizeGraphInfo(
    arch_int32 *pTMP
    )
{
    /* free every OpInfo, total count is "operationCount" */
    ...
    )
{
    arch_status vxStatus = ARCH_SUCCESS;
    struct _archModelInfo * archModel;
    arch_nn_config          *pArchNnConfig = NULL;
    arch_drv_option         *pArchOptions = NULL;
    archNN_DATABASE_FEATURE *pArchDataFeature = NULL;
    archHAL_CHIPIDENTITY    *pChipDentity = NULL;

    archSwLibContext *pArchSwLibContext = &gArchSwLibContext;

#ifdef DEBUG_VERIFY
    /* verify config */
    arch_uint32 result1 = 0, /*result2 = 0,*/ result3 = 0, result4 = 0;
    result1 = memcmp(pInArchNnConfig, pArchSwLibContext->pArchNnConfig, sizeof(arch_nn_config));
    pInArchOptions->flagTPFunc = NULL;
    pInArchOptions->typeTPFunc = NULL;
    //pInChipDentity->chipFlags = 0;
    /*result2 = memcmp(pInArchOptions, pArchSwLibContext->pArchOptions, sizeof(arch_drv_option));*/
    result3 = memcmp(pInArchDataFeature, pArchSwLibContext->pArchDataFeature, sizeof(archNN_DATABASE_FEATURE));
    result4 = memcmp(pInChipDentity, pArchSwLibContext->pChipIdentity, sizeof(archHAL_CHIPIDENTITY));
    assert(!result1 && !result3 && !result4);
#endif

    /* Check if the configration is input or need query */
    /* for driver path, need to query the configrations, for python path, need to input configrations */
    pArchNnConfig = pInArchNnConfig == NULL ? pArchSwLibContext->pArchNnConfig : pInArchNnConfig;
    pArchOptions = pInArchOptions == NULL ? pArchSwLibContext->pArchOptions : pInArchOptions;
    pArchDataFeature = pInArchDataFeature == NULL ? pArchSwLibContext->pArchDataFeature : pInArchDataFeature;
    pChipDentity = pInChipDentity == NULL ? pArchSwLibContext->pChipIdentity : pInChipDentity;

    /* configration not input from caller, need to query */
    if(pArchNnConfig == NULL || pArchOptions == NULL || pArchDataFeature == NULL || pChipDentity == NULL)
    {
        /* one of the configration is empty */
        archInfo("Configration is empty\n");
        assert(0);
    }
...
/***********************************************************************************
* Function:     initArchSwLibConfig/DeInitArchSwLibConfig
* Description:  Init/Deinit the configrations for the arch model SW lib
* Input:        :
* Ouput:        archSwLibContext *:
***************************************************************************************/
arch_status DeInitArchSwLibConfig(
...
archSwLibContext * initArchSwLibConfig()
{
    archSTATUS status = archSTATUS_OK;
    archSwLibContext *pArchSwLibContext = &gArchSwLibContext;
...
error:
    DeInitArchSwLibConfig(pArchSwLibContext);
    return NULL;
}
    ...
    )
{
    ...
    /* init configration */
    archSwLibContext *pArchSwLibContext = initArchSwLibConfig();
...
void printConfig(arch_nn_config *pArchNnConfig,arch_drv_option *pArchOptions, archHAL_CHIPIDENTITY *pChipIdentity,archNN_DATABASE_FEATURE *pArchDataFeature)
{
    ...
    /* new */
    archInfo("multiVIPnum is %d.\n",pArchNnConfig->fixedFeature.multiVIPnum);
    archInfo("preprocessImgBuf640BLimit is %d.\n",pArchNnConfig->fixedFeature.preprocessImgBuf640BLimit);
    ...
    archInfo("ddrKernelBurstSize is %d.\n",pArchNnConfig->customizedFeature.ddrKernelBurstSize);
    ...
    /* new */
    archInfo("splitZ is %d.\n",pArchNnConfig->unifiedFeature.splitZ);
    archInfo("SW_Alignment is %d.\n",pArchNnConfig->unifiedFeature.SW_Alignment);
    archInfo("HW_Alignment is %d.\n",pArchNnConfig->unifiedFeature.HW_Alignment);
    archInfo("BUG_2112 is %d.\n",pArchNnConfig->unifiedFeature.BUG_2112);
    archInfo("singlePortVipSram is %d.\n",pArchNnConfig->unifiedFeature.singlePortVipSram);
    archInfo("tileAccessCapbility is %d.\n",pArchNnConfig->unifiedFeature.tileAccessCapbility);
    archInfo("fastDp3Preprocessor is %d.\n",pArchNnConfig->unifiedFeature.fastDp3Preprocessor);
    archInfo("tensorAddMerge is %d.\n",pArchNnConfig->unifiedFeature.tensorAddMerge);
    archInfo("fastFirstPixelPooling is %d.\n",pArchNnConfig->unifiedFeature.fastFirstPixelPooling);
    ...
    /* new */
    archInfo("enableMPSplitZ is %d.\n",pArchOptions->enableMPSplitZ);
    archInfo("setCoreCount is %d.\n",pArchOptions->setCoreCount);
    archInfo("graphBatchCount is %d.\n",pArchOptions->graphBatchCount);
    archInfo("specificDDRLimitByBurst is %d.\n",pArchOptions->specificDDRLimitByBurst);
    archInfo("ddrReadSustainedBw64BBurst is %f.\n",pArchOptions->ddrReadSustainedBw64BBurst);
    archInfo("ddrReadSustainedBw128BBurst is %f.\n",pArchOptions->ddrReadSustainedBw128BBurst);
    archInfo("ddrReadSustainedBw256BBurst is %f.\n",pArchOptions->ddrReadSustainedBw256BBurst);
    archInfo("ddrMaskWriteSustainedBw64BBurst is %f.\n",pArchOptions->ddrMaskWriteSustainedBw64BBurst);
    archInfo("ddrMaskWriteSustainedBw128BBurst is %f.\n",pArchOptions->ddrMaskWriteSustainedBw128BBurst);
    archInfo("ddrMaskWriteSustainedBw256BBurst is %f.\n",pArchOptions->ddrMaskWriteSustainedBw256BBurst);
    archInfo("ddrNonMaskWriteSustainedBw64BBurst is %f.\n",pArchOptions->ddrNonMaskWriteSustainedBw64BBurst);
    archInfo("ddrNonMaskWriteSustainedBw128BBurst is %f.\n",pArchOptions->ddrNonMaskWriteSustainedBw128BBurst);
    archInfo("ddrNonMaskWriteSustainedBw256BBurst is %f.\n",pArchOptions->ddrNonMaskWriteSustainedBw256BBurst);
    ...
    /* new */
    archInfo("drJdDiffConditionForCacheLineModePreFix is %d.\n",pArchDataFeature->drJdDiffConditionForCacheLineModePreFix);
    archInfo("readReturnArbiterBubbleFix is %d.\n",pArchDataFeature->readReturnArbiterBubbleFix);
    archInfo("nerghborImageDataTransferNotEfficientFix is %d.\n",pArchDataFeature->nerghborImageDataTransferNotEfficientFix);
    archInfo("tpVipSramOt1Fix is %d.\n",pArchDataFeature->tpVipSramOt1Fix);
    archInfo("v8AccumulationBufRwConfictZeroSkipPerfFix is %d.\n",pArchDataFeature->v8AccumulationBufRwConfictZeroSkipPerfFix);
    archInfo("burstCollectDummyDataWasteCyclesFix is %d.\n",pArchDataFeature->burstCollectDummyDataWasteCyclesFix);
    archInfo("tpAccessVipSramOtIsOneFix is %d.\n",pArchDataFeature->tpAccessVipSramOtIsOneFix);
    archInfo("nnInTileDataIsAllPadFix is %d.\n",pArchDataFeature->nnInTileDataIsAllPadFix);
    ...
    archInfo("vipSramAsyncFifo is %d.\n",pArchDataFeature->vipSramAsyncFifo);
    archInfo("nnDDRBurstSize is %d.\n",pArchDataFeature->nnDDRBurstSize);
    archInfo("nnLargeBurstSize is %d.\n",pArchDataFeature->nnLargeBurstSize);
    archInfo("tpComp2pixelPerCycle is %d.\n",pArchDataFeature->tpComp2pixelPerCycle);
    archInfo("grachBatchCount is %d.\n",pArchDataFeature->grachBatchCount);
}
...
/***********************************************************************************
* Function:     updateAxiSram
* Description:  Since The AXI sram may set by insmod, C-Arch can not detect it.
                Need to set to C-Arch after query
* Input:        AXI Sram size
*
* Ouput:
***************************************************************************************/
void updateAxiSram(arch_uint32 size)
{
    archSwLibContext *pArchSwLibContext = &gArchSwLibContext;

    /* check valid */
    if(pArchSwLibContext->pArchNnConfig == NULL
        || pArchSwLibContext->pArchOptions == NULL
        || pArchSwLibContext->pArchDataFeature == NULL)
    {
        /* invalid handle */
        assert(0);
    }
    /* In FPGA/Chip run mode , the AXI sram may be queried by driver */
    pArchSwLibContext->pArchNnConfig->customizedFeature.axiSRAMSize = size;
}

/***********************************************************************************
* Function:     archSwLibDeInitFromDriver/archSwLibInitFromDriver
* Description:  Init/Deinit function of Arch Model Library Called from driver.
                Since the pArchSwLibContext point to a glocal var, do not need the deinit.
* Input:
* Ouput:        archSwLibHandle:
* TODO:         Intergrate this interface with python
***************************************************************************************/
void archSwLibDeInitFromDriver(archSwLibHandle *archSwLibHandle)
{
    archSwLibContext *pArchSwLibContext = (archSwLibContext *)archSwLibHandle;

    DeInitArchSwLibConfig(pArchSwLibContext);

}

archSwLibHandle archSwLibInitFromDriver(archHAL_CHIPIDENTITY *pChipIdentity,
                                        APMHandle apm)
{
    ...
    pArchSwLibContext = initArchSwLibConfig();

    /* fill configration */
    initConfigFromDataBase(pChipIdentity,
        pArchSwLibContext);

    /* init apm handle */
    pArchSwLibContext->apm = apm;

    /* temp flag for divide data from log or excel */
    pArchSwLibContext->flag = 0;

    /*archInfo("Arch Model Sw Lib Init from driver done.\n");*/
...
static arch_status optimizeGraph_updateDstInUpstreamLayers(
    archModelOpInfo *    srcOp,
    archModelOpInfo *    dstOp,
    int *        moveReverseMap
    )
{
    unsigned int i;
    memcpy(dstOp->upStreamLayer, srcOp->upStreamLayer, sizeof(arch_int32) * MAX_PARENT_CHILD_OP_NUM);
    for (i = 0; i < srcOp->upStreamLayerCount; i++)
    {
        if (srcOp->upStreamLayer[i] == -1)
        {
            dstOp->upStreamLayer[i] = -1;
        }
        else
        {
            dstOp->upStreamLayer[i] = moveReverseMap[srcOp->upStreamLayer[i]];
        }
    }
    return 0;
}

static arch_status optimizeGraph_updateDownStream(
    archModelOpInfo **    opInfo,
    arch_uint32            totalcount
    )
{
    arch_uint32 index = 0, j = 0, downCount_of_upLayer = 0;
    arch_int32 upLayerId = 0;

    /* calcute up/down stream based on upstream count and parent abs ID */

    for(index = 0; index < totalcount; index++)
    {
        memset(opInfo[index]->downStreamLayer, 0, sizeof(arch_int32) * MAX_PARENT_CHILD_OP_NUM);
        opInfo[index]->downStreamLayerCount = 0;
        if(opInfo[index]->upStreamLayerCount > 0)
        {
            /* calcute downstream info */
            for(j = 0; j != opInfo[index]->upStreamLayerCount; j++)
            {
                /* find the upstream layer */
                upLayerId = opInfo[index]->upStreamLayer[j];
                if(upLayerId < 0)
                    continue;
                /* update the downstream info */
                downCount_of_upLayer = opInfo[upLayerId]->downStreamLayerCount;
                opInfo[upLayerId]->downStreamLayer[downCount_of_upLayer] = index;
                opInfo[upLayerId]->downStreamLayerCount = opInfo[upLayerId]->downStreamLayerCount + 1;
                opInfo[upLayerId]->downOpCount    = opInfo[upLayerId]->downStreamLayerCount;
                opInfo[upLayerId]->downLayerCount = opInfo[upLayerId]->downStreamLayerCount;
...
/* archNNConfig has been moved to Arch Model */
    ...
    )
{
    ...
    int            moved, i;
    int            totalcount;
    archModelOpInfo * srcOp, *dstOp, *nextOp;
    totalcount = (int) *pTotalcount;
    archSwLibContext *pArchSwLibContext = &gArchSwLibContext;
    bool                enableNativeStride2 = pArchSwLibContext->pArchNnConfig->unifiedFeature.fastFirstPixelPooling;
    bool                enableTensorAddMerge = pArchSwLibContext->pArchNnConfig->unifiedFeature.tensorAddMerge;
    //initialize opGraphIdMap
    for (i = 0; i < totalcount; i++)
    {
        opGraphIdMap[i] = opInfo[i]->absoluteOperationID;
    }
    while (srcInd < totalcount)
    {
        moved = 0; // this is used to make sure a layer won't be merged twice.
        srcOp = opInfo[srcInd];
        dstOp = opInfo[dstInd];
        nextOp = opInfo[srcInd + 1];
        //Fast first pixel pooling
        if (enableNativeStride2 &&
            srcInd < totalcount - 1 &&
            moved == 0 &&
            srcOp->op == ARCHNNE_OPERATOR_RESHUFFLE &&
            nextOp->op == ARCHNNE_OPERATOR_CONVOLUTION &&
            nextOp->kx == 2 && nextOp->ky == 2 &&
            nextOp->pstride == 1 && nextOp->psize == 1 &&
            nextOp->upStreamLayer[0] == srcOp->absoluteOperationID && nextOp->upStreamLayerCount)
        {
            memcpy(dstOp, nextOp, sizeof(archModelOpInfo));
            dstOp->kz = dstOp->kz / 4;
            dstOp->kx = dstOp->kx * 2 - 1;
            dstOp->ky = dstOp->ky * 2 - 1;
            dstOp->op = ARCHNNE_OPERATOR_FIRST_PIXEL_POOLING;
            dstOp->perf.coefCompressRatio = archMIN(1, dstOp->perf.coefCompressRatio * 4);
            dstOp->perf.coefNonZeroRatio = archMIN(1, dstOp->perf.coefNonZeroRatio * 4);
            optimizeGraph_updateDstInUpstreamLayers(srcOp, dstOp, moveReverseMap);
            moveHistory[dstInd] = srcInd + 1;
            moveReverseMap[srcInd + 1] = dstInd;
            //optimizeGraph_updateUpStream(dstOp, opInfo);
            srcInd += 2;
            moved = 1;
        }

        //TensorAdd Merge
        if (enableTensorAddMerge && moved == 0 &&
            srcInd < (totalcount - 1) &&
            nextOp->op == ARCHNNE_OPERATOR_TENSOR_ADD &&
            srcOp->op == ARCHNNE_OPERATOR_CONVOLUTION &&
            nextOp->upStreamLayer[0] == srcOp->absoluteOperationID) /*&&
            issinglechildParent(opInfo, srcInd + 1, srcInd))*/
        {
            unsigned int j = 0;
            memcpy(dstOp, srcOp, sizeof(archModelOpInfo));

            //modify dstOp's upstream layer id
            for (j = 0; j < srcOp->upStreamLayerCount; j++)
            {
                dstOp->upStreamLayer[j] = moveReverseMap[srcOp->upStreamLayer[j]];

            }
            dstOp->op = ARCHNNE_OPERATOR_TENSOR_ADD_MERGE;
            moveHistory[dstInd] = srcInd + 1;
            moveReverseMap[srcInd + 1] = dstInd;
            srcInd += 2;
            moved = 1;
        ...
        if (moved == 0)
        {
            memcpy(dstOp, srcOp, sizeof(archModelOpInfo));
            optimizeGraph_updateDstInUpstreamLayers(srcOp, dstOp, moveReverseMap);
            moveHistory[dstInd] = srcInd;
            moveReverseMap[srcInd] = dstInd;
            //optimizeGraph_updateUpStream(dstOp, opInfo);
            srcInd++;
        }
        /*dstOp->absoluteOperationID = dstInd;*/
        dstInd++;
    }
    *pTotalcount = (arch_uint32) dstInd;
    optimizeGraph_updateDownStream(opInfo, dstInd);
    return 0;
}
...
{
    int            srcInd = 0; //this is the ID of non-optimized layer
    int            dstInd = 0; //this is the ID of optimized layer
    archModelOpInfo * emptyOp = NULL, *srcOp, *dstOp;
    archAllocateMemory(sizeof(archModelOpInfo), (archPOINTER *) &emptyOp);
    for (srcInd = nonOpt_totalcount - 1; srcInd >= 0; srcInd--)
    {
        srcOp = opInfo[srcInd];
        if (moveReverseMap[srcInd] == -1)
        {
            memcpy(srcOp, emptyOp, sizeof(archModelOpInfo));
            memset(srcOp, 0, sizeof(archModelOpInfo));
        }
        else
        {
            dstInd = moveReverseMap[srcInd];
            dstOp = opInfo[dstInd];
            memcpy(srcOp, dstOp, sizeof(archModelOpInfo));
        }
        srcOp->absoluteOperationID = opGraphIdMap[srcInd];
    }
    assert(nonOpt_totalcount >= 0);
    *totalCount = (arch_uint32) nonOpt_totalcount;
    archFreeMemory(emptyOp);
    return archSTATUS_OK;
}
