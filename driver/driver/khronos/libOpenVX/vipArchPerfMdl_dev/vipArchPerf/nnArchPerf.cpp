    ...
    )
{
    ...
    /*double kernelRepeatRead, imageRepeatRead;*/
    double /*imageRepeatSingleRead,*/ readBandWidth = 0/*, KernelNonIdealCache, KernelStorage*/;
                                             ...
                                             double *output_BurstCount_64B_Standalone, double *output_BurstCount_128B_Standalone, double *output_BurstCount_256B_Standalone)
{
    ...
    {
        t_output_BurstCount_256B_Standalone = Cal_StandAlone256BC(hgap, vgap, tile_x, tile_y, x, y, z, stride, slice) * factor;
    }
    ...
    )
{
    ...
    double convToVIPSramBW, trspToVIPSramBW;
    ...
    */
    ...
    if (dst_buf == SW_TILING_FROM_DDR)
    {
        ...
        if(specified_ddr_bw_limit_by_burst)
        {
            ...
            WriteBandWidth_By_BurstSize(tile_xsize, tile_ysize, x, y, z, k, dst_buf, output_data_size, image_compress_ratio, usc_cache_size, pooling_stride, zdp, out_image_stride, out_image_slice,is_nn_write_without_usc, &result_tmp);
    ...
    {
        convToVIPSramBW = WriteBandWidth(tile_xsize, tile_ysize, x, y, z, output_data_size, 1, usc_cache_size, pooling_stride, out_image_stride, out_image_slice, is_nn_write_without_usc, SW_TILING_FROM_VIP_SRAM, 0); /* burst size set to 0 for not used */
        vipWriteBW = vipWriteBW + convToVIPSramBW;
    }
    ...
    if(pContext->pInPerfParams->cmdInfo.TrspInterleaveCh_out > 1)
    {
        if(dst_buf == SW_TILING_FROM_DDR)
        {
            trspToVIPSramBW = ddrWriteBW;
        }
        else if(dst_buf == SW_TILING_FROM_AXI_SRAM)
        {
            trspToVIPSramBW = axiWriteBW;
        }
        vipWriteBW = vipWriteBW + trspToVIPSramBW;
    }
    ...
    if (pContext->bf.Accumulation_Read_Out_Bubbles_BUG_2044)
    {
        internalWriteCycleCount = internalWriteCycleCount + ceilf((float)z / (k * cores)) * (cores - 1) * 5 * ceilf((float)x / tile_xsize) * ceilf((float)y / tile_ysize) * (input_data_size * input_data_size) / (8 * 8);
    }
    if (pContext->bf.Burst_Collect_Dummy_Data_Waste_Cycles_BUG_2111)
    {
        double dummyBurstWasteCycleCount = (convToVIPSramBW + trspToVIPSramBW) / l2cache_width /* original write cycles to VIPSRAM from internal */
                                            * (vip_sram_acc_unit_size_in_byte / 16);                 /* 2111 effect */
        internalWriteCycleCount = max(internalWriteCycleCount, dummyBurstWasteCycleCount);
    }
    ...
    )
{
    ...
    unsigned int  zrl_bits = pHwInfo->pFeatures->NN_ZRL_BITS;
...
void initBugStatus(APM_BUG_FEATURE_TYPE * pBf)
{
    ...
    pBf->Accumulation_Read_Out_Bubbles_BUG_2044 = !pContext->p_hwInfo->pFeatures->V8_ACCUMLATION_READ_OUT_HAS_BUBBLES_PERF_FIX;
    ...
    pBf->Burst_Collect_Dummy_Data_Waste_Cycles_BUG_2111 = !pContext->p_hwInfo->pFeatures->BURST_COLLECT_DUMMY_DATA_WASTE_CYCLES_FIX;
}
