...
static archModelOpInfo ** archTransferParam(vx_graph graph, vx_uint32 *totalCount);
...
static archModelOpInfo ** archTransferParam(vx_graph graph, vx_uint32 *totalCount)
{
    ...
    bool featureDEC400Enable = gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_VIP_DEC400);
    ...
    for (i = 0; i < graph->nodeCount; i++)
    {
        ...
        for (j = 0; j < node->layer->num_operations; j++)
        {
            ...
            if (isValidNN(operation))
            {
                ...
                /* plan to move this judgement to Arch Model inside */
                opInfo[count]->perf.imageCompressRatio = (featureDEC400Enable && context->options.enableVIPDEC400) ? 0.700000000000000f : 1.0f;
            ...
            else if (isValidTP(operation))
            {
                ...
                if (operation->operatorType == VXNNE_OPERATOR_TENSOR_TRANS)
                {
                    opInfo[count]->calcinx = opInfo[count]->kz;
                }
                else
                {
                    opInfo[count]->calcinx = TENSOR_VIEW_SIZE_INDEX(tpOp->input, 0); //This calcinx is actually used as output x in c-arch.
                }
                ...
                else
                {
                    ...
                    opInfo[count]->calcinx = opInfo[count]->origoutx;
                }
                opInfo[count]->pix = (vx_uint32)ceilf((vx_float32)(opInfo[count]->xsize - opInfo[count]->p3) / opInfo[count]->pstride);
                opInfo[count]->piy = (vx_uint32)ceilf((vx_float32)(opInfo[count]->ysize - opInfo[count]->p3) / opInfo[count]->pstride);
                opInfo[count]->sbuf = SW_TILING_FROM_AXI_SRAM;
            ...
            else if (isValidFC(operation))
            {
                ...
                opInfo[count]->perf.coefNonZeroRatio  = WB_NON_ZERO_RATIO(wb);
                opInfo[count]->perf.coefCompressRatio = WB_COMPRESS_RATIO(wb);
                /* plan move this judgement to Arch Model inside */
                opInfo[count]->perf.imageCompressRatio = (featureDEC400Enable && context->options.enableVIPDEC400) ? 0.700000000000000f : 1.0f;
                opInfo[count]->perf.imageNonZeroRatio  = 0.300000000000000;
...
/***********************************************************************************
* Function:     updateSpecifiedConfigration to C-Arch
* Description:  Since some of the configration can not be detected by C-Arch, such as
                set in insmod, need to set these configration to C-Arch
* Input:
*

* Ouput:
***************************************************************************************/
void setAxiSramToCArch(vx_uint32 size)
{
    updateAxiSram(size);
    return;
}
...
#ifdef DEBUG_VERIFY
/***********************************************************************************
* Function:     initConfigration for run mode only
...
static vx_status initConfigration(arch_nn_config *pArchNnConfig,arch_drv_option *pArchOptions,vx_context context)
{
    ...
    pArchNnConfig->fixedFeature.multiVIPnum = pArchNnConfig->fixedFeature.vipCoreCount;
    ...
    pArchNnConfig->customizedFeature.ddrKernelBurstSize = pContextNnConfig->customizedFeature.ddrKernelBurstSize;
    ...
    /* add new configration from feature database */
    pArchNnConfig->unifiedFeature.singlePortVipSram = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_USE_SINGLE_PORT_VIPSRAM);
    pArchNnConfig->unifiedFeature.tileAccessCapbility = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_TILE_ACCESS_CAPABILITY);
    pArchNnConfig->unifiedFeature.fastDp3Preprocessor = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_FAST_DP3_PREPROCESSOR);
    pArchNnConfig->unifiedFeature.tensorAddMerge = gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_NN_TENSOR_ADD);
    pArchNnConfig->unifiedFeature.fastFirstPixelPooling = gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_NN_NATIVE_STRIDE_TWO);
    ...
    pArchOptions->enableSubnetworkSplitting = 0;
    pArchOptions->enableMPSplitZ = pContextOptions->enableMPSplitZ;
    if(pContextOptions->setCoreCount != 0)
    {
        pArchNnConfig->fixedFeature.multiVIPnum = pContextOptions->setCoreCount;
    }
    return 0;
}

/***********************************************************************************
* Function:     updateConfigration for run mode only
...
static vx_status updateConfigration(archNN_DATABASE_FEATURE *pArchDataFeature,arch_nn_config *pArchNnConfig, vx_context context)
{
    ...
    pArchDataFeature->nerghborImageDataTransferNotEfficientFix = gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_DEPTHWISE_NEIGHBOR_IMG_DATA_TRANSFER_NOT_EFFICIENT_FIX);  /* 2045 */
    pArchDataFeature->tpVipSramOt1Fix = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_TP_ACCESS_VIPSRAM_OT_IS_ONE_FIX);                        /* 2050 */
    ...
    /* bug2112 */
    pArchNnConfig->unifiedFeature.BUG_2112 = !gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_BIT_INIMG_NOT_64BYTE_ALIGN_CACHELINE_MODE_FIX);

    /* 2044 */
    pArchDataFeature->v8AccumulationBufRwConfictZeroSkipPerfFix = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_V8_ACCUMLATION_READ_OUT_HAS_BUBBLES_PERF_FIX);
    /* 2111 */
    pArchDataFeature->burstCollectDummyDataWasteCyclesFix = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_BURST_COLLECT_DUMMY_DATA_WASTE_CYCLES_FIX);
    /* 2050 */
    pArchDataFeature->tpAccessVipSramOtIsOneFix = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_TP_ACCESS_VIPSRAM_OT_IS_ONE_FIX);
    pArchDataFeature->nnInTileDataIsAllPadFix = gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_NN_IN_TILE_DATA_IS_ALL_PAD_FIX);
    ...
    /* Burst size */
    pArchDataFeature->nnDDRBurstSize = pArchNnConfig->customizedFeature.ddrKernelBurstSize;
...
VX_INTERNAL_API vx_status archGraphPredictPerf(vx_graph graph)
{
    ...
    vx_context              context    = vxoContext_GetFromReference(&graph->base);
#ifdef DEBUG_VERIFY
    arch_nn_config          archNNConfig = {0};             /* configration for specific case */
    arch_drv_option         archDrvOption = {0};            /* options defined in env */
    archHAL_CHIPIDENTITY    chipIdentity = {0};             /* Chip identify for HW info */
    archNN_DATABASE_FEATURE archDataFeature = {0};    /* case feature definition */
#endif
    archModelOpInfo         **opInfo   = NULL;        /* Base opInfo for Arch Model */
    int * moveHistory = NULL, *moveReversedMap = NULL, *opGraphIdMap = NULL;
    int nonOpt_totalcount = 0;
    ...
    opInfo = archTransferParam(graph, &totalCount);
    if(opInfo == NULL)
    {
        return ARCH_FAILURE;
    }
    if(totalCount == 0)
    {
        ...
        // do not have NN or TP just return
        return VX_SUCCESS;
    }
    ...
    /* OptimizeGraph */
    nonOpt_totalcount = totalCount;
    moveHistory = initOptimizeGraphInfo(totalCount);
    moveReversedMap = initOptimizeGraphInfo(totalCount);
    opGraphIdMap = initOptimizeGraphInfo(totalCount);
    optimizeGraph(opInfo, &totalCount, moveHistory, moveReversedMap, opGraphIdMap);
    /* Call predict function do the performance analysing */
    /* do not need to pass the configration to Arch Model, Arch Model will get it by itself */
#ifndef DEBUG_VERIFY
    vxStatus = archPredictPerf(context->globalData->apm, opInfo, totalCount, NULL,
                               NULL, NULL, NULL);
#else
    /* for verify if the config is correct */
    vxStatus = archPredictPerf(context->globalData->apm, opInfo, totalCount, &archNNConfig,
                               &archDrvOption, &archDataFeature,&chipIdentity);
#endif
    /* OptimizeGraph recover */
    optimizeGraph_recoverPosition(opInfo, moveReversedMap, nonOpt_totalcount, &totalCount, opGraphIdMap);
    deInitOptimizeGraphInfo(moveHistory);
    deInitOptimizeGraphInfo(moveReversedMap);
    deInitOptimizeGraphInfo(opGraphIdMap);
    /* OptimizeGraph End */
    /* release OpInfo */
    deInitArchOpInfo(opInfo, totalCount);
#else
    ...
    vxnne_operator_e op_type)
{
    ...
    arch_nn_config *pArchNnConfig =  &archNNConfig;
    arch_drv_option *pArchOptions = &archDrvOption;
    arch_uint32 result1 = 0,/* result2 = 0, */result3 = 0;
#endif
//    vx_uint32 coreIndex = 0,
    ...
    vx_int32 inputDataSize, outputDataSize;
    vx_op_param conv_cmd = &op_command->parameter;
    vxnne_operation operation = op_command->operation;
    arch_bool axiSramOnlySWTiling = 0;


    archSwLibContext *pArchSwLibContext = (archSwLibContext *)context->globalData->archSwLibHandle;
    vx_uint32 archVipSramSize = 0;
    bool featureDEC400Enable = gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_VIP_DEC400);
...
#ifdef DEBUG_VERIFY
    /* verify config */
    result1 = memcmp(pArchNnConfig, pArchSwLibContext->pArchNnConfig, sizeof(arch_nn_config));
    pArchOptions->flagTPFunc = NULL;
    pArchOptions->typeTPFunc = NULL;
    /*result2 = memcmp(pArchOptions, pArchSwLibContext->pArchOptions, sizeof(arch_drv_option));*/
    result3 = memcmp(&archDataFeature, pArchSwLibContext->pArchDataFeature, sizeof(archNN_DATABASE_FEATURE));
    assert(!result1 && !result3);
#endif
    axiSramOnlySWTiling = context->nnConfig.unifiedFeature.axiSramOnlySWTiling ? arch_true_e : arch_false_e;
    ...
    perf->info.xOffSet = (-1) * conv_cmd->pad_x_left;
    perf->info.yOffSet = (-1) * conv_cmd->pad_y_top;
    perf->info.xOffset_right = (-1) * conv_cmd->pad_x_right;
    perf->info.yOffset_bottom = (-1) * conv_cmd->pad_y_bottom;
    ...
    archVipSramSize = ((context->nnConfig.customizedFeature.vipSRAMSize <= (arch_uint32)ARCH_VIP_SRAM_IMAGE_STREAM_SIZE) ?
                                            context->nnConfig.customizedFeature.vipSRAMSize :
                                            (context->nnConfig.customizedFeature.vipSRAMSize - ARCH_VIP_SRAM_IMAGE_STREAM_SIZE));

    perf->swTilingInfo.cacheSpace = archVipSramSize;
    ...
    if(wb != NULL)
    {
        ...
        perf->imageCompressRatio = (featureDEC400Enable && context->options.enableVIPDEC400) ? 0.700000000000000f : 1.0f;
    ...
    /* adjust NN transpose channel based on the buff */
    if(perf->swTilingInfo.srcBuf != SW_TILING_FROM_DDR)
    {
       perf->swTilingInfo.trspIvLayerChsIn = 0;
    }
    if(perf->swTilingInfo.dstBuf != SW_TILING_FROM_DDR)
    {
        perf->swTilingInfo.trspIvLayerChsOut = 0;
    }
#ifdef DEBUG_VERIFY
...
#else
    archCalculateArchPerf(context->globalData->apm, pArchSwLibContext->pArchNnConfig,pArchSwLibContext->pArchOptions, pArchSwLibContext->pArchDataFeature, (arch_perf)perf, (archnne_operation_target_e)op_target, (archnne_operator_e)op_type);
#endif
}
    ...
    )
{
    ...
    if (gOrigShowType != (vx_int8)context->options.collectPerfType)
    {
        ...
        vxInfo("\nArchModelVersion: %s\nSWTilingVersion: %s\nProfileMode: %d\nHWFinalRelease: %d\n"
               ...
               vxoContext_IsFeatureAvailable(context, VX_NN_FEATURE_VIP_FINAL_RELEASE) ? 1 : 0,
               pArchSwLibContext->pChipIdentity->chipModel,
               pArchSwLibContext->pChipIdentity->chipRevision,
               pArchSwLibContext->pChipIdentity->productID,
               pArchSwLibContext->pChipIdentity->customerID,
               pArchSwLibContext->pChipIdentity->ecoID,
        ...
        vxInfo("DDR_ALIGN: %d\nIN_LINES_PER_CYCLE: %d\n",
            pArchSwLibContext->pArchDataFeature->ddrAlign,
            pArchSwLibContext->pArchDataFeature->inlinesPerCycle
            );

        vxInfo("NN_SLOW_OUTPUT: %d\nNO_NARROW_POST_PROCESS_PIPE: %d\nNN_SMALLBATCH_PHASE1: %d\n",
            pArchSwLibContext->pArchDataFeature->nnSlowOutput,
            pArchSwLibContext->pArchDataFeature->noNarrowPostProcessPipe,
            pArchSwLibContext->pArchDataFeature->prefetchNNCommandKernelHeader
            );
        ...
        vxInfo("NN_KERNEL_SIZE_WASTE_IN_PARTIAL_MODE_FIX: %d\n",
            pArchSwLibContext->pArchDataFeature->partialKernelCacheInternalFix
            );

        vxInfo("KERNEL_VIP_SRAM_READ_BW_LIMITATION_FIX: %d\n",
            pArchSwLibContext->pArchDataFeature->internalKernelReadBottleneckFix
            );

        vxInfo("IMG_POP_PIPELINE_PAUSE_FIX: %d\n",
            pArchSwLibContext->pArchDataFeature->ImgPopPipelinePauseFix
            );

        vxInfo("FULL_CACHE_INTERVAL_FIX: %d\n",
            pArchSwLibContext->pArchDataFeature->fullCacheIntervalFix
            );

        vxInfo("NN_Transpose: %d\nSPECIFIED_DDR_BW_LIMIT_BY_BURST: %d\n",
            pArchSwLibContext->pArchDataFeature->nnTranspose, pArchSwLibContext->pArchDataFeature->specificDDRLimitByBurst
            );

        vxInfo("DR_JD_Diff_For_Cacheline_Mode_Fix: %d\n",
            pArchSwLibContext->pArchDataFeature->drJdDiffConditionForCacheLineModePreFix
            );

        vxInfo("TP_ACCESS_VIPSRAM_OT_IS_ONE_FIX: %d\n",
            pArchSwLibContext->pArchDataFeature->tpAccessVipSramOtIsOneFix
            );

        vxInfo("SINGLE_PORT_VIPSRAM: %d\nTILE_ACCESS_CAPBILITY: %d\nFAST_DP3PREPROCESS: %d\n",
            pArchSwLibContext->pArchNnConfig->unifiedFeature.singlePortVipSram,
            pArchSwLibContext->pArchNnConfig->unifiedFeature.tileAccessCapbility,
            pArchSwLibContext->pArchNnConfig->unifiedFeature.fastDp3Preprocessor
            );
        ...
        vxInfo("DDR_READ_SUSTAINED_BW_64B_BURST: %f\nDDR_READ_SUSTAINED_BW_128B_BURST: %f\nDDR_READ_SUSTAINED_BW_256B_BURST: %f\n",
            pArchSwLibContext->pArchDataFeature->ddrReadSustainedBw64BBurst,
            pArchSwLibContext->pArchDataFeature->ddrReadSustainedBw128BBurst,
            pArchSwLibContext->pArchDataFeature->ddrReadSustainedBw256BBurst
            );
        vxInfo("DDR_WRITE_SUSTAINED_BW_64B_MASK_BURST: %f\nDDR_WRITE_SUSTAINED_BW_128B_MASK_BURST: %f\nDDR_WRITE_SUSTAINED_BW_256B_MASK_BURST: %f\n",
            pArchSwLibContext->pArchDataFeature->ddrMaskWriteSustainedBw64BBurst,
            pArchSwLibContext->pArchDataFeature->ddrMaskWriteSustainedBw128BBurst,
            pArchSwLibContext->pArchDataFeature->ddrMaskWriteSustainedBw256BBurst
            );
        vxInfo("DDR_WRITE_SUSTAINED_BW_64B_NONMASK_BURST: %f\nDDR_WRITE_SUSTAINED_BW_128B_NONMASK_BURST: %f\nDDR_WRITE_SUSTAINED_BW_256B_NONMASK_BURST: %f\n",
            pArchSwLibContext->pArchDataFeature->ddrNonMaskWriteSustainedBw64BBurst,
            pArchSwLibContext->pArchDataFeature->ddrNonMaskWriteSustainedBw128BBurst,
            pArchSwLibContext->pArchDataFeature->ddrNonMaskWriteSustainedBw256BBurst
            );

        /* new configration */
        vxInfo("VIPSRAM_ASYNC_FIFO: %d\nreadReturnArbiterBubbleFix: %d\nnerghborImageDataTransferNotEfficientFix: %d\ntpVipSramOt1Fix: %d\n",
            pArchSwLibContext->pArchDataFeature->vipSramAsyncFifo,
            pArchSwLibContext->pArchDataFeature->readReturnArbiterBubbleFix,          /* 2038 */
            pArchSwLibContext->pArchDataFeature->nerghborImageDataTransferNotEfficientFix,          /* 2045 */
            pArchSwLibContext->pArchDataFeature->tpVipSramOt1Fix           /* 2050 */
            );

        /* Burst size */
        vxInfo("NN_DDR_BURST_SIZE: %d\nTP_COMP_2PIXEL_PER_CYCLE: %d\n",
            pArchSwLibContext->pArchDataFeature->nnDDRBurstSize,
            pArchSwLibContext->pArchDataFeature->tpComp2pixelPerCycle
            );
    ...
    vxInfo("ReadBW: %llu\nWriteBW: %llu\nCycleCount: %llu\n\n",
             ...
             (vx_uint64)(perf->resultInfo.perfCycleCount  + 0.5f));
