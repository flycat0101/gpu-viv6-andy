    ...
    case VX_KERNEL_CONVOLUTION_LAYER:
        {
            ...
            if(VX_TENSOR_LIFE_TIME_STATIC == TENSOR_DATA_LIFETIME(weight) &&
                ...
                SCALAR_VALUE(node->paramTable[PARAM_CONV_PAD_INDEX], u32) < 7 &&
                SCALAR_VALUE(node->paramTable[PARAM_CONV_PAD_INDEX + 2], u32) < 7
                )
            {
                if(SCALAR_VALUE(node->paramTable[PARAM_CONV_DEPTH_MULTIPLIER_INDEX], u32) == 0)
                {
                    if((TENSOR_SIZE_INDEX(weight, 0) == TENSOR_SIZE_INDEX(weight, 1) || TENSOR_SIZE_INDEX(weight, 0) == 1 ) )
                        nodeOpType = OP_CONVOLUTION;
                    else if(vxoGraphOptimization_nnHalSupport(weight))
    ...
    case VX_KERNEL_NN_FULLY_CONNECTED_LAYER:
        {
                ...
                VX_TENSOR_RANK_SN == TENSOR_RANK((vx_tensor)node->paramTable[1]) )
            {
                ...
                VX_TENSOR_LIFE_TIME_STATIC == TENSOR_DATA_LIFETIME((vx_tensor)node->paramTable[2]) &&
...
VX_INTERNAL_API vx_status vxoGraph_Optimization_LayerSwaping(vx_graph graph)
{
    ...
    for (nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
    {
...

        {
            ...
            vx_reference maxpoolInput   = maxPoolNode->paramTable[0];
            vxoNode_SetParameter(leakyReluNode, 0, maxpoolInput);
            vxoNode_SetParameter(maxPoolNode, 0, leakyInput);
        }

        {
            vx_reference leakyOutput     = leakyReluNode->paramTable[leakyReluNode->numParameters - 1];
            ...
            vxoNode_SetParameter(maxPoolNode, maxPoolNode->numParameters - 1, leakyOutput);
        }
    }
...
VX_PRIVATE_API  void vxoGraph_MergeConvolutionNodes_GetParmFromConv(vx_node convNode, vx_tensor *weight, vx_tensor *bias,
                                                                    ...
                                                                    vx_enum *rounding_policy, vx_enum *down_scale_size_rounding)
{
...
VX_PRIVATE_API  void vxoGraph_MergeConvolutionNodes_GetParmFromConvRelu(vx_node convNode, vx_tensor *weight, vx_tensor *bias,
                                                                    vx_uint32 pad[4], vx_uint32 stride[2],
                                                                    vx_uint8 *accumulator_bits, vx_enum *overflow_policy, vx_enum *rounding_policy,
                                                                    vx_enum *down_scale_size_rounding, vx_bool *enable_relu)
{
    vx_weights_biases_parameter wb = (vx_weights_biases_parameter)convNode->paramTable[PARAM_CONV_RELU_WEIGHTED_BIAS_INDEX];
    *weight                      = wb->wb_base->origWeight;
    *bias                        = wb->wb_base->origBias;
    stride[0]                   = wb->wb_base->strideX;
    stride[1]                   = wb->wb_base->strideY;
    pad[0]                      = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_PAD_X_INDX], u32);
    pad[1]                      = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_PAD_X_INDX], u32);
    pad[2]                      = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_PAD_Y_INDX], u32);
    pad[3]                      = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_PAD_Y_INDX], u32);
    *enable_relu                = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_ENABLE_RELU_INDEX], b);
    *accumulator_bits           = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_ACCUMULATOR_BITS_INDEX], u8);
    *overflow_policy            = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_OVERFLOW_INDEX], u32);
    *rounding_policy            = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_ROUNDING_INDEX], u32);
    *down_scale_size_rounding   = SCALAR_VALUE(convNode->paramTable[PARAM_CONV_RELU_DOWN_SCALE_SIZEROUNDING_INDEX], u32);
}
...
VX_INTERNAL_API vx_status vxoGraph_MergeConvolutionNodes(vx_node nodes[], vx_uint32 nodeCount)
{
    ...
    vx_uint32 i = 0, lastNodeOutputIndex = nodes[0]->numParameters - 1;
    ...
    for(i = 0; i < nodeCount; i++){
        ...
        switch (nodes[i]->kernel->enumeration){
            ...
            case VX_KERNEL_NN_CONVOLUTION_RELU_LAYER:{
                if(nodeCount ==1)
                    break;
                    vxoGraph_MergeConvolutionNodes_GetParmFromConvRelu(
                        nodes[i], &weight, &bias, pad,stride,
                        &accumulator_bits, &overflow_policy, &rounding_policy,
                        &down_scale_size_rounding, &enable_relu
                        );
                    break;
                }
            case VX_KERNEL_NN_CONVOLUTION_RELU_POOLING_LAYER:{
            ...
            default:
                vxError("wrong node to be merged\n");
                break;
        }
    }/*for(i = 0; i < nodeCount; i++)*/
    ...
    /*viv:for conv_relu case; do not create new node and keep the output shape of conv */
    if(VX_KERNEL_CONVOLUTION_LAYER != nodes[0]->kernel->enumeration && nodeCount == 2 && VX_KERNEL_ACTIVATION_LAYER == nodes[1]->kernel->enumeration)
    {
        vx_bool diff = vx_false_e;
        vx_scalar relu = vxCreateScalar(context, VX_TYPE_BOOL, &enable_relu);
        vx_uint32 relu_index = VX_KERNEL_NN_CONVOLUTION_RELU_POOLING_LAYER2 == nodes[0]->kernel->enumeration ? 12 : 8;
        vxSetParameterByIndex(nodes[0], relu_index, (vx_reference)relu);

        {
            vx_uint32 i = 0;
            for(i = 0; i < gcmMAX(TENSOR_DIM_NUM(convOutputTensor), TENSOR_DIM_NUM(finalOutTensor)); i++)
                if(TENSOR_SIZE_INDEX(convOutputTensor, i) != TENSOR_SIZE_INDEX(finalOutTensor, i))
                { diff = vx_true_e; break;}
        ...
        vxSetParameterByIndex(nodes[0], nodes[0]->numParameters - 1, (vx_reference)finalOutTensor);
        vxSetParameterByIndex(nodes[1], nodes[1]->numParameters - 1, (vx_reference)convOutputTensor);

        if(diff)
            vxReleaseTensor(&finalOutTensor);
        vxReleaseScalar(&relu);
    }
    else if(weight &&
        ((lastNode > 0 ) ||
            (lastNode == 0 && nodes[0]->kernel->enumeration == VX_KERNEL_CONVOLUTION_LAYER))
        )
    {
        vx_context context = vxGetContext((vx_reference)nodes[0]);
        ...
        vx_nn_convolution_relu_pooling_params_ext2_t wb_params =
        {
            ...
            0, VX_TENSOR_RANK_WHCN, TENSOR_DATA_TYPE(finalOutTensor)
        };

        vx_weights_biases_parameter_optimizations_ext_t opt = {-1, TENSOR_DATA_TYPE(finalOutTensor),
            TENSOR_TF_ZEROPOINT(inputTensor), TENSOR_DIM_NUM(inputTensor), TENSOR_DIM_NUM(finalOutTensor) };
        vx_weights_biases_parameter wb = vxCreateWeightsBiasesParameterFromTensors3(
            VX_NN_CONVOLUTION_LAYER,
            TENSOR_SIZES(inputTensor), TENSOR_SIZES(convOutputTensor), TENSOR_SIZES(finalOutTensor),
            (vx_nn_convolution_relu_pooling_params_t *)&wb_params, sizeof(wb_params), (vx_weights_biases_parameter_optimizations_t *)&opt, sizeof(opt), weight, bias);
        CHECK_NULL(wb);

        nodes[0]->merged = vx_true_e;
        newNode = vxConvolutionReluPoolingLayer2(nodes[0]->graph, inputTensor, wb, (vx_nn_convolution_relu_pooling_params)&wb_params,
            sizeof(wb_params), finalOutTensor);
...
VX_INTERNAL_API vx_status vxoGraph_MergeFullyConnectedNodes(vx_node nodes[], vx_uint32 nodeCount)
{
    ...
    {
        vx_node newNode = vxoNode_TransferFC2FCRelu(nodes[0]);
        nodes[0]->merged = vx_true_e;
        nodes[0] = newNode;
...
VX_INTERNAL_API vx_status vxoGraph_MergeWithChildNodes(vx_node node)
{
    ...
    if(!vxoNode_IsConvNode(node) && !vxoNode_IsFCNode(node) )
    {
        return VX_SUCCESS;
    }
        ...
        nodeCount++)
    {
        ...
        if((opType & OP_CONVOLUTION ) &&
            ((currNodeType & OP_FULLYCONNECTED) != 0  ||
              ((currNodeType & OP_FULLYCONNECTED) == 0 && ((currNodeType & (OP_RELU | OP_POOLING) ) == 0) )
            )
          )
            break;

        if((opType & OP_FULLYCONNECTED) &&
            ((opType & OP_CONVOLUTION) != 0 ||
              ((opType & OP_CONVOLUTION) == 0 && (currNodeType & OP_RELU) == 0 ) )
           )
            break;
...
VX_PRIVATE_API vx_tensor vxoGraph_Optimization_WAR_convert1x1x1weight(vx_graph graph, vx_tensor weight)
{
        ...
        orgDims[0] == 1 && orgDims[1]== 1 && orgDims[2]== 1))
    {
        ...
        dims[1] = 2;
        {
            ...
            bigWeight = vxCreateTensor2(vxGetContext((vx_reference)graph), &param, sizeof(vx_tensor_create_params_t));
...
VX_PRIVATE_API vx_weights_biases_parameter vxoGraph_Optimization_GetWBfromFC(vx_node FCNode, vx_uint32 *convInputDims, vx_uint32 *convOutputDims,
                                                                             vx_nn_convolution_relu_pooling_params_t * params, vx_uint32 sizeOfParam)
{
    vx_enum opType = FCNode->kernel->enumeration;
    vx_tensor weight, bias;
    vx_tensor shareWeight, bigWeight;
    vx_weights_biases_parameter_optimizations_t wbop;
    vx_weights_biases_parameter newWB;
    vx_nn_convolution_relu_pooling_params_ext2_t *wbParam = (vx_nn_convolution_relu_pooling_params_ext2_t *)params;
    if(VX_KERNEL_NN_FULLY_CONNECTED_RELU_LAYER == opType)
    {
        vx_weights_biases_parameter weight_biases = (vx_weights_biases_parameter)FCNode->paramTable[1];
        weight = weight_biases->wb_base->origWeight;
        bias   = weight_biases->wb_base->origBias;

        wbParam->ext.base.pad_x_left = weight_biases->wb_base->pad_x_left;
        wbParam->ext.base.pad_x_right = weight_biases->wb_base->pad_x_right;
        wbParam->ext.base.pad_y_top = weight_biases->wb_base->pad_y_top;
        wbParam->ext.base.pad_y_bottom = weight_biases->wb_base->pad_y_bottom;
        wbParam->ext.base.down_scale_size_rounding = weight_biases->wb_base->down_scale_size_rounding;
        wbParam->ext.stride_x = weight_biases->wb_base->strideX;
        wbParam->ext.stride_y = weight_biases->wb_base->strideY;
    }
    else
    {
        weight = (vx_tensor)FCNode->paramTable[1];
        bias   = (vx_tensor)FCNode->paramTable[2];
    }

    shareWeight = vxoGraph_Optimization_GetReshapeWeightTensor(weight);
    bigWeight = vxoGraph_Optimization_WAR_convert1x1x1weight(FCNode->graph, shareWeight);


    wbop.inputZeroPoint =  TENSOR_TF_ZEROPOINT((vx_tensor)FCNode->paramTable[0] );
    wbop.outputFormat = TENSOR_DATA_TYPE((vx_tensor)FCNode->paramTable[FCNode->numParameters - 1]);
    wbop.zrl = -1;
    newWB = vxCreateWeightsBiasesParameterFromTensors2(VX_NN_CONVOLUTION_LAYER,
                                                                                             weight->dimCount,
                                                                                             convInputDims,
                                                                                             convOutputDims,
                                                                                             convOutputDims,
                                                                                             TENSOR_DATA_TYPE((vx_tensor)FCNode->paramTable[FCNode->numParameters-1]),
                                                                                             (vx_nn_convolution_relu_pooling_params)params,
                                                                                             sizeof(vx_nn_convolution_relu_pooling_params_ext2_t),
                                                                                             &wbop,
                                                                                             bigWeight,
                                                                                             bias);
    if(bigWeight != shareWeight)
        vxReleaseTensor(&bigWeight);
    if(shareWeight != weight)
        vxReleaseTensor(&shareWeight);
...
VX_INTERNAL_API vx_status vxoGraph_Optimization_ConvertBatchFCtoConv(vx_graph graph)
{
    ...
    ********************************************************************************/
    ...
    for (nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
    {
        ...
        CHECK_NULL(vxTensorPermuteNode(graph, shareInputTensor, convInputTensor, permuteIdx,3) );
        ...
        /*if(vxoGraphOptimization_nnHalSupport(convInputTensor))*/
        {
            ...
            vx_nn_convolution_relu_pooling_params_ext2_t params = {
                        {
                            { 0, 0, 0, 0, 0, 0,
                                SCALAR_VALUE(node->paramTable[4], u8),
                                SCALAR_VALUE(node->paramTable[5], u32),
                                SCALAR_VALUE(node->paramTable[6], u32),
                                SCALAR_VALUE(node->paramTable[7], u32),
            ...
            vx_weights_biases_parameter weights_biases = vxoGraph_Optimization_GetWBfromFC(node, TENSOR_SIZES(convInputTensor), TENSOR_SIZES(convOutputTensor),
                (vx_nn_convolution_relu_pooling_params_t *)&params, sizeof(params));
        ...
        /*else
        {
            vx_tensor weight = node->kernel->enumeration == VX_KERNEL_NN_FULLY_CONNECTED_RELU_LAYER ? ((vx_weights_biases_parameter)node->paramTable[1])->wb_base->origWeight : (vx_tensor)node->paramTable[1];
            vx_tensor bias = node->kernel->enumeration == VX_KERNEL_NN_FULLY_CONNECTED_RELU_LAYER ? ((vx_weights_biases_parameter)node->paramTable[1])->wb_base->origBias: (vx_tensor)node->paramTable[2];
            vx_nn_convolution_params_t param = {0,0,
                SCALAR_VALUE(node->paramTable[6], u32),
                SCALAR_VALUE(node->paramTable[4], u32),
                0,0,
                SCALAR_VALUE(node->paramTable[5], u32)};

            vx_tensor tmpTensor = VX_NULL;
            vx_bool reluflag = node->kernel->enumeration == VX_KERNEL_NN_FULLY_CONNECTED_RELU_LAYER ? SCALAR_VALUE(node->paramTable[7], b): vx_false_e;
            if(reluflag)
            {
                vx_enum quant_format = TENSOR_DATA_TYPE(convOutputTensor) == VX_TYPE_UINT8 ? VX_QUANT_AFFINE_SCALE : VX_QUANT_DYNAMIC_FIXED_POINT;
                vx_tensor_create_params_t tensorParam = {TENSOR_DIM_NUM(convOutputTensor), (vx_int32_ptr)(TENSOR_SIZES(convOutputTensor)), TENSOR_DATA_TYPE(convOutputTensor), quant_format, 0};
                if(TENSOR_DATA_TYPE(convOutputTensor)  == VX_TYPE_UINT8)
                {
                    tensorParam.quant_data.affine.scale     = TENSOR_TF_SCALE(convOutputTensor);
                    tensorParam.quant_data.affine.zeroPoint = TENSOR_TF_ZEROPOINT(convOutputTensor);
                }
                else
                {
                    tensorParam.quant_data.dfp.fixed_point_pos = TENSOR_POS(convOutputTensor);
                }
                tmpTensor = vxCreateVirtualTensor2(graph, &tensorParam, sizeof(tensorParam));
                vxConvolutionLayer(graph, convInputTensor, weight, bias, &param, sizeof(param), tmpTensor);
                vxActivationLayer(graph, tmpTensor, VX_CONVOLUTIONAL_NETWORK_ACTIVATION_RELU, 0, 0, convOutputTensor);

                vxReleaseTensor(&tmpTensor);
            }
            else
                vxConvolutionLayer(graph, convInputTensor, weight, bias, &param, sizeof(param), convOutputTensor);
        }*/

        CHECK_NULL(vxTensorPermuteNode(graph, convOutputTensor, shareOutputTensor, permuteIdx,3) );
...
VX_INTERNAL_API vx_status vxoGraph_Optimization_WAR_convert1x1x1KernelsizeInConv(vx_graph graph)
{
    vx_int32 nodeIndex;
    vx_int32 nodeCount = graph->nodeCount;
    vx_node* nodeTable = graph->nodeTable;
    vx_weights_biases_parameter weight_biases = NULL;
    vx_tensor weight =  VX_NULL;
    vx_tensor bigWeight = VX_NULL;
    vx_tensor bias = VX_NULL;
    ...
    for (nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
    {
        ...
        if(opType != VX_KERNEL_NN_CONVOLUTION_RELU_POOLING_LAYER2)
            continue;

        weight_biases = (vx_weights_biases_parameter)node->paramTable[1];
        weight =  weight_biases->wb_base->origWeight;
        bias = weight_biases->wb_base->origBias;

        if(TENSOR_DIM_NUM(weight) >= 3 && (
            TENSOR_SIZE_INDEX(weight,0) != 1 || TENSOR_SIZE_INDEX(weight,1) != 1 || TENSOR_SIZE_INDEX(weight,2) != 1))
            continue;

        bigWeight = vxoGraph_Optimization_WAR_convert1x1x1weight(graph, weight);
        {
            vx_nn_convolution_relu_pooling_params_ext2_t params = {
                        {
                                { 0, 0,
                                    weight_biases->wb_base->pad_x_left,
                                    weight_biases->wb_base->pad_x_right,
                                    weight_biases->wb_base->pad_y_top,
                                    weight_biases->wb_base->pad_y_bottom,
                                    0,
                                  VX_CONVERT_POLICY_SATURATE,
                                  VX_ROUND_POLICY_TO_ZERO,
                                  weight_biases->wb_base->down_scale_size_rounding,
                                  vx_false_e,
                                  0, 0, 0, VX_PAD_CONSTANT, 0 },
                                weight_biases->wb_base->strideX,
                                weight_biases->wb_base->strideY
                        },
                        0,
                        VX_TENSOR_RANK_WHCN,
                        weight_biases->wb_base->weights_data_format
                };
          vx_weights_biases_parameter newWeightBias = vxCreateWeightsBiasesParameterFromTensors2(VX_NN_CONVOLUTION_LAYER,
                                                        weight->dimCount,
                                                        TENSOR_SIZES((vx_tensor)node->paramTable[0]),
                                                        TENSOR_SIZES((vx_tensor)node->paramTable[node->numParameters - 1]),
                                                        TENSOR_SIZES((vx_tensor)node->paramTable[node->numParameters - 1]),
                                                        TENSOR_DATA_TYPE((vx_tensor)node->paramTable[node->numParameters - 1]),
                                                        (vx_nn_convolution_relu_pooling_params)&params,
                                                        sizeof(vx_nn_convolution_relu_pooling_params_ext2_t),
                                                        ...
                                                        bigWeight,
                                                        bias);
          /*vxReleaseWeightsBiasesParameter(&weight_biases);*/
          vxoNode_SetParameter(node, 1, (vx_reference)newWeightBias );
        }
    }
    return VX_SUCCESS;
}
...
VX_INTERNAL_API vx_tensor vxoGraph_Optimization_ConvertAvgPool2Conv_createWeight(vx_tensor input, vx_uint32 weight_dims[4])
{
    ...
    if(TENSOR_DATA_TYPE(input) == VX_TYPE_FLOAT16)
    {
        ...
        vx_int16 quantized_data = Fp32toFp16(fill_data);
        vx_int16 *ptr = (vx_int16 *)weight_memory;

        for(i = 0; i < weight_size; i++)
            ptr[i] = quantized_data;
        parm.quant_format = VX_QUANT_DYNAMIC_FIXED_POINT;
    }
    else if(TENSOR_DATA_TYPE(input) == VX_TYPE_UINT8)
    {
        float scale = fill_data/255.0f;
        vx_int32 zp = 0;
        vx_uint8 quantized_data = (vx_uint8) roundRTNE(fill_data / scale + zp);

        parm.quant_format = VX_QUANT_AFFINE_SCALE;
        parm.quant_data.affine.scale = fill_data/255.0f;
        parm.quant_data.affine.zeroPoint = 0;

        memset(weight_memory, quantized_data, weight_size);
    }
    else if(TENSOR_DATA_TYPE(input) == VX_TYPE_INT8)
    {
        vx_int8 *ptr = (vx_int8 *)weight_memory;
        vx_int8 fl =(vx_int8) gcmMIN(12, 8 - ceilf((float)log(fill_data) + 1));
        vx_int8 quantized_data = 0;
        float tmp = fill_data;

        for(i = 0; (vx_int8)i < fl-1; i++)
            tmp *= fill_data;
        quantized_data  = (vx_int8)tmp;

        parm.quant_format = VX_QUANT_DYNAMIC_FIXED_POINT;
        parm.quant_data.dfp.fixed_point_pos = fl;

        for(i = 0; (vx_uint32)i < weight_size; i++)
            ptr[i] = quantized_data;
    }
...
VX_INTERNAL_API vx_status vxoGraph_Optimization_TensorAdd2Conv_createWeight_fp16(vx_tensor tensorIn[2], vx_uint32 coreNum, vx_tensor *weight, vx_int32 factor)
{
    ...
    vx_int16  weightDatai16 = Fp32toFp16(1.f);
    vx_int16  weightDatai16_1 = Fp32toFp16(1.f * factor);
    vx_size wDims[4] = {1,1,2 * coreNum,coreNum};

    vx_int16 *weightData = (vx_int16 *)vxAllocateAndZeroMemory(4* coreNum * coreNum);
    vx_uint32 i = 0;
    for(i = 0; i < coreNum; i++)
    {
        vx_int32 offset = i * 2 * coreNum;
        weightData[offset + i] = weightDatai16;
        weightData[offset + i + coreNum] = weightDatai16_1;
    }
...
VX_PRIVATE_API vx_status vxoGraph_Optimization_TensorAdd2Conv_copyData2Weight_i8(vx_tensor *weight, vx_uint32 coreNum, vx_int8 quantedData[2])
{
    vx_uint32 i = 0;
...
VX_PRIVATE_API vx_status vxoGraph_Optimization_TensorAdd2Conv_createWeight_asymmetic(vx_tensor tensorIn[2], vx_uint32 coreNum,
                                                                               vx_tensor *weight, vx_tensor *bias, vx_int32 factor)
{
    vx_context context = vxGetContext((vx_reference)tensorIn[0]);
    {
        ...
        vxoGraph_Optimization_TensorAdd2Conv_createWeight(context, weight, coreNum, VX_TYPE_UINT8,VX_QUANT_AFFINE_SCALE, scale, zp, 0);
        ...
        vxoGraph_Optimization_TensorAdd2Conv_copyData2Weight_i8(weight, coreNum, quantedData);
        }
    }
    {
...
VX_PRIVATE_API vx_status vxoGraph_Optimization_TensorAdd2Conv_createWeight_dfp(vx_tensor tensorIn[2], vx_uint32 coreNum,
                                                                               vx_tensor *weight, vx_tensor *bias, vx_int32 factor)
{
    vx_int8 fl_delta = TENSOR_POS(tensorIn[1]) - TENSOR_POS(tensorIn[0]);
    vxoGraph_Optimization_TensorAdd2Conv_createWeight(vxGetContext((vx_reference)tensorIn[0]), weight, coreNum,
        VX_TYPE_INT8,VX_QUANT_DYNAMIC_FIXED_POINT,0, 0, gcmMAX(0, fl_delta));
    {
        ...
        vxoGraph_Optimization_TensorAdd2Conv_copyData2Weight_i8(weight, coreNum, (vx_int8 *)weightValue);
    }
    return VX_SUCCESS;
}
    ...
    for (nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
    {
        ...
        if(vxoGraph_getKernelType(node) == OP_ADD_SUB)
        {
            ...
            }
            {
                ...
                /*vxoGraph_Optimization_replaceOldParamter(graph, node->childNodes, node->numChildren,
                        (vx_reference)output, (vx_reference)convOutTensor);*/
            }

            switch (TENSOR_DATA_TYPE(tensorIn[0]))
            {
                case VX_TYPE_FLOAT16:
                ...
                case VX_TYPE_UINT8:
                    ...
                    {
                        vxoGraph_Optimization_TensorAdd2Conv_createWeight_dfp(tensorIn, core, &weight, &bias, factor);
                    }
                    else
                    {
                        vxoGraph_Optimization_TensorAdd2Conv_createWeight_asymmetic(tensorIn, core, &weight, &bias, factor);
                    }
                    break;
                default:
                    break;
...

            {
                ...
                vx_nn_convolution_relu_pooling_params_ext2_t param = {
                    ...
                    0, TENSOR_RANK(convInTensor), TENSOR_DATA_TYPE(convOutTensor)
                };

                vx_weights_biases_parameter_optimizations_ext_t opt = {-1, TENSOR_DATA_TYPE(convOutTensor),
                    TENSOR_TF_ZEROPOINT(convInTensor), TENSOR_DIM_NUM(convInTensor), TENSOR_DIM_NUM(convOutTensor)};

                vx_weights_biases_parameter wb = vxCreateWeightsBiasesParameterFromTensors3(
                    VX_NN_CONVOLUTION_LAYER,
                    TENSOR_SIZES(convInTensor),
                    TENSOR_SIZES(convOutTensor),
                    TENSOR_SIZES(convOutTensor),
                    (vx_nn_convolution_relu_pooling_params)&param, sizeof(param),
                    (vx_weights_biases_parameter_optimizations_t *)&opt, sizeof(opt),
                    weight, bias);
                CHECK_NULL(wb);

                convNode  = vxConvolutionReluPoolingLayer2(node->graph, convInTensor, wb,
                    (vx_nn_convolution_relu_pooling_params)&param, sizeof(param), convOutTensor);
                CHECK_NULL(convNode );
        ...
        if(vxoGraph_getKernelType(node) == OP_CONVOLUTION_RELU_POOLING || vxoGraph_getKernelType(node) == OP_CONVOLUTION_POOLING)
        {
            ...
            if(pool_size[0] == 3 && pool_size[1] == 3)
            {
                ...
                convOutTensor = vxCreateVirtualTensor2(graph, &convOuttensorP, sizeof(convOuttensorP));
                {
                    ...
                    vx_weights_biases_parameter_optimizations_ext_t opt = {-1, TENSOR_DATA_TYPE(convOutTensor),
                        TENSOR_TF_ZEROPOINT(convInTensor), TENSOR_DIM_NUM(convOutTensor), TENSOR_DIM_NUM(convOutTensor) };
                    vx_weights_biases_parameter wb = vxCreateWeightsBiasesParameterFromTensors3(
                        VX_NN_CONVOLUTION_LAYER,
                        TENSOR_SIZES(convInTensor), TENSOR_SIZES(convOutTensor), TENSOR_SIZES(convOutTensor),
                        (vx_nn_convolution_relu_pooling_params_t *)&wb_params, sizeof(wb_params),
                        (vx_weights_biases_parameter_optimizations_t *)&opt, sizeof(opt), weight, bias);
                    CHECK_NULL(wb);
    ...
    for (nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
    {
        ...
        if(vxoGraph_getKernelType(node) == OP_CONVOLUTION_NxM)
        {
            ...
            vx_enum overflow_policy,rounding_policy,down_scale_size_rounding;
             ...
             {
            vx_uint32 padData = 0;
            vx_scalar padconst = vxCreateScalar(context, VX_TYPE_UINT32, (void *)&padData);
            vx_nn_convolution_params_ext2_t params = {
            {
                {
                    (vx_size)pad[0], (vx_size)pad[2], overflow_policy, rounding_policy, down_scale_size_rounding, dilation[0], dilation[1]
                },
                (vx_size)pad[1], (vx_size)pad[3], VX_PAD_CONSTANT, padconst
                },
...
VX_INTERNAL_API vx_status vxoGraph_Optimization_unrollDWConv(vx_graph graph)
{
    ...
    for (nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
    {
        ...
        if(vxoGraph_getKernelType(node) == OP_CONVOLUTION_DW)
        {
            ...
            /*if(graph->base.context->nnConfig.customizedFeature.depthWiseSupport != 0)
                if(depth_mult == 1)
                    continue;
                else
                {
...
*/
            ...
            {
                vx_uint32 padv = 0;
                vx_scalar sPad = vxCreateScalar(graph->base.context, VX_TYPE_UINT32, &padv);
                vx_nn_convolution_params_ext2_t params = {
                    {
                        {
                        ...
                        (vx_size)pad[1], (vx_size)pad[3], VX_PAD_CONSTANT, sPad
