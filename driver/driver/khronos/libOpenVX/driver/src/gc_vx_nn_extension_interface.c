...
vx_status vxnneExecutionLayer_GenerateCommands(vx_context context, vxnne_layer layer)
{
    ...
    for (i = 0; i < executionLayer->opIndicesNum; i++)
    {
        ...
        /*used by swap handel*/
    vxo_insertHandle(executionLayer);
...
VX_PRIVATE_API vx_status vxnneOperation_ExecuteYUVScalerCommand(vx_node node, vxnne_operation operation)
{
    ...
    {
        vxmONERROR(vxoBinaryGraph_SaveScalerLayer(node, operation));
    }
...
vx_status vxnneExecutionLayer_Execute(vxnne_layer layer)
{
    ...
    vxnne_execution_layer   executionLayer = (vxnne_execution_layer)layer;
    vxnne_operation         operation;
    vxnne_operation_target_e defaultChannel = VXNNE_OPERATION_TARGET_SH;
    vx_graph graph = executionLayer->graph;
    vx_node node = VX_NULL;

    vxnneMultiChannel_SetCurrentChannel(defaultChannel);
    /* Program SRAM REMAP registers. */
    if (gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_SWTILING_PHASE3))
    {
        gcoVX_SetRemapAddress(graph->base.context->vipSRAM.physBase, 0, gcvVX_SRAM_REMAP);
    }
    ...
    for (i = 0; i < executionLayer->opIndicesNum; i++)
    {
        ...
        if (operation->layer && operation->layer->node->base.context->options.enableNNLayerDump && executionLayer->opIndices[i].operation->mGpuSync)
        {
            gcfVX_Flush(gcvTRUE);
        ...
        if (executionLayer->graph->base.context->options.enableCNNPerf && executionLayer->opIndices[i].operation->mGpuSync)
        {
            vxInfo("layer id: %d layer name:%s operation[%d]:%s target:%s.\nuid: %d\nop_abs_id: %d\n",
                ...
                executionLayer->opIndices[i].operation->absoluteOperationID);
            if (operation->target == VXNNE_OPERATION_TARGET_SH)
            {
                vxInfo("shader kernel name: %s\n",
                    ((vxnne_shader_operation)operation)->shaderExecutable->kernelShader->name);
            }
            gcfVX_Flush(gcvTRUE);

            if (operation->layer && operation->layer->node->base.context->options.enableBlockDump &&
                !operation->layer->node->base.context->options.enableNNLayerDump &&
                executionLayer->opIndices[i].outputTile.sRAM == SW_TILING_FROM_DDR)
            {
                vxnneOperation_NodeDump(&executionLayer->opIndices[i]);
            }
            ...
            if (operation->layer && operation->layer->node->base.context->options.enableNNLayerDump
