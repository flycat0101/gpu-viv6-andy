...
vx_status vxnneExecuteSWL2Normalize(struct _vxnne_operation_s *operation)
{
    ...
    vx_int32  axis              = l2normalizeOperation->axis;
    ...
    gctPOINTER input_data_ptr;
    gctPOINTER output_data_ptr;
    vx_status status = VX_SUCCESS;
    vx_uint32 outerSize = 1;
    vx_uint32 axisSize  = TENSOR_VIEW_SIZE_INDEX(inputs, axis);
    vx_uint32 innerSize = 1;

    vxoTensor_GetTensorBatchArrayViewMemory(inputs, operation->currBatchIndex, (gctPOINTER *)&input_data_ptr, VX_NULL);
    vxoTensor_GetTensorBatchArrayViewMemory(outputs, operation->currBatchIndex, (gctPOINTER *)&output_data_ptr, VX_NULL);

    for (i = 0; i < (vx_uint32)axis; i++)
    {
        innerSize *= TENSOR_VIEW_SIZE_INDEX(inputs, i);
    }

    for (i = axis + 1; i < dims; i++)
    {
        outerSize *= TENSOR_VIEW_SIZE_INDEX(inputs, i);
    }
    ...
    for (i = 0; i < outerSize; i++)
    {
        for (j = 0; j < innerSize; j++)
        {
            ...
            {
                vx_float32 data = vxnneGetDataExt(inputFormat, TENSOR_QUANT_TYPE(inputs), inner, (vx_uint8_ptr)input_data_ptr, inputFixedPoint, TENSOR_TF_ZEROPOINT(inputs), TENSOR_TF_SCALE(inputs));
                sum += data * data;
            }
            ...
            for (inner = begin ; inner < end; inner += innerSize)
            {
                vx_float32 val = vxnneGetDataExt(inputFormat, TENSOR_QUANT_TYPE(inputs), inner, (vx_uint8_ptr)input_data_ptr, inputFixedPoint, TENSOR_TF_ZEROPOINT(inputs), TENSOR_TF_SCALE(inputs));

                val = val * rsqrt;

                vxnneSaveDataExt(outputFormat, TENSOR_QUANT_TYPE(outputs), inner, val, (vx_uint8_ptr)output_data_ptr, outputFixedPoint, TENSOR_TF_ZEROPOINT(outputs), TENSOR_TF_SCALE(outputs), TENSOR_ROUNDING_MODE(outputs));
            }
        }
    }
...
VX_PRIVATE_API vx_status vxoNNL2NormalizeLayer_SW_Initialize(vxnne_layer ops_layer, const vx_reference parameters[], vx_uint32 num, vxnne_register_param reg_param)
{
    ...
    l2normalizeLayer->l2normalize_sw_operation.inputs           = src;
    l2normalizeLayer->l2normalize_sw_operation.outputs          = dst;
    l2normalizeLayer->l2normalize_sw_operation.axis             = out_axis;

    vxmONERROR(vxnneOperation_AddReference(&l2normalizeLayer->l2normalize_sw_operation.base, (vx_reference)src, VXNNE_OPERATION_REFENRENCE_INPUT));
    vxmONERROR(vxnneOperation_AddReference(&l2normalizeLayer->l2normalize_sw_operation.base, (vx_reference)dst, VXNNE_OPERATION_REFENRENCE_OUTPUT));
