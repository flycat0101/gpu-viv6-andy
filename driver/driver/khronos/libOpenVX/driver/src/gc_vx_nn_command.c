    ...
    for(k = 0; k < zcount; k++)
    {
        for(j = 0; j < ycount; j++)
        {
            ...
            for(i = 0; i < xcount; i++)
            {
                ...
                {
                    sinfoArray[index].vx_nn_general_cmd_split_info.inImageAddress  = sinfoArray[index - xcount].vx_nn_general_cmd_split_info.inImageAddress + (i_y_size  - conv_cmd_ptr->kernel_y + 1) * input->yStride;
                ...
                {
                    sinfoArray[index].vx_nn_general_cmd_split_info.inImageAddress  = sinfoArray[index - 1].vx_nn_general_cmd_split_info.inImageAddress + (i_x_size - conv_cmd_ptr->kernel_x + 1) * vxnneGetTypeSize(inDataFormat);
    ...
    )
{
    ...
    for (i = 0; i < split_count; i++)
    {
        ...
        vx_uint32 pool_size_x = parameter->pool_size_x;
        vx_uint32 pool_size_y = parameter->pool_size_y;

        if(parameter->pool_size_x != parameter->pool_size_y )/*HW only support 2*2 & 3*3*/
        {
            if(parameter->pool_size_x > parameter->pool_size_y)
            {
                pad_y_bottom = parameter->pool_size_x- parameter->pool_size_y;
                pool_size_y = parameter->pool_size_x;
            }
            else
            {
                pad_x_right = parameter->pool_size_y- parameter->pool_size_x;
                pool_size_x = parameter->pool_size_y;
            }
        }
        ...
        if (parameter->pool_size_x != 1)
        {
            ...
            info_array[i].vx_tp_general_cmd_split_info.inTileSequence = 0x0;
            info_array[i].vx_tp_general_cmd_split_info.inImageBaseAddress = inputBase + inZStride * split_offsets[i];
            info_array[i].vx_tp_general_cmd_split_info.inTileXSize = outTileXSize * poolingStride + pool_size_x - poolingStride;
        ...
        if (parameter->orig_no_pad &&
            ((inXSize - pool_size_x) % parameter->pool_stride ||
    ...
    )
{
    ...
    for (i = 0; i < split_count; i++)
    {
        ...
        info_array[i].vx_tp_general_cmd_split_info.outLoop5Inc   = outXSize;
    ...
    )
{
    ...
    switch (tp_type)
    {
        ...
        case TP_LSTM_STATE_OUT:
        {
            ...
            info->vx_nn_tp_cmd_info.aluPwlSignSupport   = 0;
            info->vx_nn_tp_cmd_info.aluReluEnable       = 0;
            info->vx_nn_tp_cmd_info.aluI2FEnable        = (tfQuant && hasInputQuant) ? 1 : (tpBF16 ? 1 : (tpRealInt16 ? (inFormat == VX_TYPE_FLOAT16 ? 0 : 1) : (inFormat == outFormat ? 0 : 1)));
            info->vx_nn_tp_cmd_info.aluF2IEnable        = (tfQuant && hasOutputQuant) ? 1 : (tpBF16 ? 1 : (tpRealInt16 ? (outFormat == VX_TYPE_FLOAT16 ? 0 : 1) : (inFormat == outFormat ? 0 : 1)));
