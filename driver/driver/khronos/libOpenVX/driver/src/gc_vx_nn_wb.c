    ...
    )
{
    ...
    {
        vx_uint32 coreCount = getTPCoreCount(context, TP_SINGLE_FC);
        ...
        for (j = 0; j < WB_KERNEL_Z_SLICE_NUM(wb); j++)
        {
            ...
            if (WB_IS_TP_COMPRESS(wb))
            {
                if (vxoContext_IsFeatureAvailable(context, VX_NN_FEATURE_TP_COMPRESSION_ENHANCEMENT))
                {
                    ...
                    /* for real compress, isCalculate should be set as false */
                ...
                if(fillSize > min_kernel_buffer_sizes[index])
                    vxmASSERT(0);
    ...
    )
{
    ...
    else if(vxoContext_IsFeatureAvailable(context, VX_NN_FEATURE_TF_QUANT) && (WB_WEIGHT_QUANT_FORMAT(wb)  == VX_QUANT_AFFINE_SCALE) && (WB_WEIGHT_DATA_FORMAT(wb) == VX_TYPE_UINT8 || WB_WEIGHT_DATA_FORMAT(wb) == VX_TYPE_INT8) )
    {
        vx_uint32 i= 0;
...
VX_PRIVATE_API vx_status vxoWeightBias_FreeCompressed(
    vx_weights_biases_parameter wb
    )
{
    if (wb == VX_NULL) return VX_ERROR_INVALID_PARAMETERS;
    ...
    )
{
    ...
    case VXNNE_OPERATION_TARGET_TP:
        tpCoreCount = getTPCoreCount(context, TP_SINGLE_FC);
    ...
    /* copy original weight and bias data from non-cache video memory to CPU memory to speed up */
    {
        ...
        {
            vxoTensor_GetTensorViewMemory((vx_tensor)(WB_ALPHA_REF(wb)), (gctPOINTER*)&alphaOrigPtr, VX_NULL);
    ...
    )
{
    ...
    wb->free_compressed        = vxoWeightBias_FreeCompressed;
    ...
    )
{
    ...
    vx_bool nnSupportFormat = vxnneIsNNSupportFormat(context, VX_NULL, weights, VX_NULL, VX_NULL);
    ...
    else if (vxoContext_IsFeatureAvailable(context, VX_NN_FEATURE_TF_QUANT) && (weightQuantType == VX_QUANT_AFFINE_SCALE) && (weightDataType == VX_TYPE_UINT8 || weightDataType == VX_TYPE_INT8))
    {
        ...
        )
    {
        ...
        if ((nonZeroRatio * weights->dims[0] * weights->dims[1] < 6.3 ||
            ...
            && !(!isV8 && layer_type == VX_NN_CONVOLUTION_LAYER &&
                    weights->dims[0] != weights->dims[1] && weights->dims[0] != 1) /*V7 didn't support MxN, only supoort 1xN*/
            )
        {
    ...
    {
        if(!gcoHAL_IsFeatureAvailable(gcvNULL, gcFEATURE_BIT_DEPTHWISE_16BIT_FORMAT) && weightDataType != VX_TYPE_UINT8 && weightDataType != VX_TYPE_INT8)
        {
            /*HW limilation: depthwise only support U8 and INT8*/
            vxError("[Not support]vxoCreateWeightsBiasesParameterFromTensors: depthwise only support U8 and INT8\n");
            goto exit;
        }
    ...
    )
{
    ...
    if (vxoContext_IsFeatureAvailable(context, VX_NN_FEATURE_TF_QUANT) && weights_quant_format == VX_QUANT_AFFINE_SCALE && (weights_data_format == VX_TYPE_UINT8 || weights_data_format == VX_TYPE_INT8))
    {
        skipValue[0] = 1;
    }
    ...
    vx_tensor   alpha)
{
    ...
    {
        vx_nn_convolution_relu_pooling_params_ext3_t conv_ext3 = *((vx_nn_convolution_relu_pooling_params_ext3_t*)(convolution_relu_pooling_params));
        stride_x = conv_ext3.ext2.ext.stride_x;
        stride_y = conv_ext3.ext2.ext.stride_y;
        rank_mode = conv_ext3.ext2.src_rank_mode;
        convert_format = conv_ext3.ext2.convert_dst_format;

        if (conv_ext3.ext2.depth_multiplier == 1 &&
            (TENSOR_DATA_TYPE(weights) == VX_TYPE_INT8 || TENSOR_DATA_TYPE(weights) == VX_TYPE_UINT8) &&
            vxoContext_IsFeatureAvailable(context, VX_NN_FEATURE_NN_DEPTHWISE_SUPPORT))
            layer = VX_NN_DEPTH_WISE_CONVOLUTION_LAYER;
    }
    ...
    vx_tensor   biases)
{
    ...
    else if (size_of_convlution_relu_pooling_params == sizeof(vx_nn_convolution_relu_pooling_params_ext3_t))
    {
        vx_nn_convolution_relu_pooling_params_ext3_t conv_ext3 = *((vx_nn_convolution_relu_pooling_params_ext3_t*)(convolution_relu_pooling_params));
        stride_x = conv_ext3.ext2.ext.stride_x;
        stride_y = conv_ext3.ext2.ext.stride_y;
