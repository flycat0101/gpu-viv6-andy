...
struct _VGContext
{
    ...
    char                        chipName[vgdCHIP_NAME_LEN];
#endif
...
#define NEWOBJ(type, os, objPtr) \
    ...
    {\
        ...
        {\
            gcoOS_ZeroMemory((gctPOINTER)(objPtr), sizeof(type));\
            type##Ctor(os, objPtr); \
        }\
    }while(0)
...
#define OVG_MALLOC(os, mem, size) \
    do { \
        ...
        gcoOS_ZeroMemory((gctPOINTER)(mem), size);\
    } while (0)
...
#define OVG_MEMCOPY(dst, src, size)         \
    ...
    { \
        ...
        gcmASSERT((size) > 0); \
        gcoOS_MemCopy(dst, src, size); \
    } \
    while (gcvFALSE)

#define OVG_MEMSET(mem, value, size) \
    gcoOS_MemFill(mem, value, size)
