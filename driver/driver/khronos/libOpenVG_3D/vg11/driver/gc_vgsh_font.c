...
#define _GC_OBJ_ZONE                gcdZONE_VG3D_FONT
...
Error:
    vgmGetApiEndTime(context);
    gcmFOOTER_ARG("return=%d", (font ? font->object.name : VG_INVALID_HANDLE));
    return (font ? font->object.name : VG_INVALID_HANDLE);
}
                  ...
                  VGboolean            allowAutoHinting)
{
    ...
    if (paintModes & VG_STROKE_PATH)
        vgmPROFILE(context, VG_PROFILER_STROKE, (gctUINTPTR_T)glyphCount);
    if (paintModes & VG_FILL_PATH)
        vgmPROFILE(context, VG_PROFILER_FILL, (gctUINTPTR_T)glyphCount);
