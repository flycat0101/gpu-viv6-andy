...
#define _GC_OBJ_ZONE                gcdZONE_VG3D_TESSELLATOR
...
{
    _VGTessPoint    *points        = gcvNULL;
    gctINT32        *counts        = gcvNULL;
    gctINT32        numSubs;
    gctINT32        numTotalPoints;
    gctINT32        i, j, k, l;
    _VGTessPoint    *tempPoint     = gcvNULL;
    _VGTessPoint    *tempPoint2    = gcvNULL;

    _VGTesstype     dashPhaseNext;
    _VGTessPoint    **dashedPoints = gcvNULL;
    gctINT32        **dashedCounts = gcvNULL;
    gctBOOL         isDashed;
    gctBOOL         *subPathClosed = gcvNULL;
    ...
    if (lineWidth * scale <= 0.99f)
    {
        ...
        _VGTessPoint *tempPoints = gcvNULL;
        gctINT32     *tempCounts = gcvNULL;
        gctINT32      oCounter   = 0;
        gctINT32      merged     = 0;
        base = 0;
        toMerge = 1;
    ...
    {
        gctINT32        *numDashSegs         = gcvNULL;
        gctINT32        *numDashPoints       = gcvNULL;
        gctINT32        *numPointsInDashSegs = gcvNULL;
        gctINT32        numTotalDashPoints;
        _VGTessPoint    *dashPoints          = gcvNULL;
        _VGTesstype     *pattern             = gcvNULL;
    ...
    )
{
    ...
    _VGVector2      *points = gcvNULL;
    gctINT32        *numPointsInSubPaths = gcvNULL;
    gctINT32        numSubPaths;
    gctINT32        numPoints;
    gctINT32        i, j, k;
    _VGint16        *triangleIndices = gcvNULL;
    gctINT32        triangleIndicesArrayLength;
    gctINT32        triangleCount;
    _VGVector2      *copyToPoint = gcvNULL, *copyFromPoint = gcvNULL;
    ...
    )
{
    ...
    _VGVector2        mp1;
    _VGVector2        mp2;
    _VGTesstype       normStartAngle;
    _VGTesstype       normEndAngle;
    gctINT32          startQuadrant;
    gctINT32          endQuadrant;
    ...
    *a2 -= a;
    ...
    normStartAngle = TESS_MUL(TESS_MUL(*a1, TESS_CONST_2), TESS_PI_RCP);
    normEndAngle   = TESS_MUL(TESS_MUL(*a2, TESS_CONST_2), TESS_PI_RCP);
    startQuadrant  = TESS_INT(TESS_FLOOR(normStartAngle)) + 1;
    endQuadrant    = TESS_INT(TESS_CEIL(normEndAngle));

    if ((mp1.y == mp2.y) && ((endQuadrant-startQuadrant) == 2))
    {
        center.x = (mp1.x + mp2.x)/2;
        center.y = (mp1.y + mp2.y)/2;
    }
    ...
    )
{
    ...
    _VGTesstype         t0 = 0, t1 = 0, t2 = 0;

    _VGTessPoint        **points = gcvNULL;
    gctINT32            **numPointsInSubPaths = gcvNULL;
    gctINT32            *numSubPaths = gcvNULL;
    gctBOOL             **isClosed = gcvNULL;
    ...
    *numSubPaths    = 0;
        ...
        case VG_MOVE_TO:        /* Indicate the beginning of a new sub-path. */
            {
                ...
                if (pathQuery)
                {
                    ...
                    if (!queryBegin)
                    {
                        path->tessellateResult.bound.x = path->tessellateResult.bound.width = sPoint.x;
                        path->tessellateResult.bound.y = path->tessellateResult.bound.height = sPoint.y;
                        queryBegin = gcvTRUE;
                    }
                    else
                    {
                        path->tessellateResult.bound.x = gcmMIN(path->tessellateResult.bound.x, sPoint.x);
                        path->tessellateResult.bound.y = gcmMIN(path->tessellateResult.bound.y, sPoint.y);
                        path->tessellateResult.bound.width = gcmMAX(path->tessellateResult.bound.width, sPoint.x);
                        path->tessellateResult.bound.height = gcmMAX(path->tessellateResult.bound.height, sPoint.y);
                    }
                }
            }
            break;
    ...
    if (numPoints < 1 || currentSegment < 0 || currentSubPath < 0)
    {
        ...
        if (pathQuery)
        {
            path->tessellateResult.bound.width -= path->tessellateResult.bound.x;
            path->tessellateResult.bound.height-= path->tessellateResult.bound.y;
            return -1;
        }
        else
        {
    ...
    )
{
    ...
    gctINT32        numLines;
    _VGTessPoint    *tempPoints    = gcvNULL;
    _VGTessPoint    *pointsCopy    = gcvNULL;
    ...
    )
{
    ...
    _VGVector2        nVec;
    _VGTessPoint    *points      = gcvNULL;
     SIMPLE_LIST    *headNode    = gcvNULL;
     SIMPLE_LIST    *currentNode = gcvNULL;
        ...
        while (lineLength > 0)
        {
            if ( (dashingIndex & 0x00000001) == 0 )     /* If the current dash phase lies in an "on" dash. */
            {
                ...
                if (lineLength >= dashPattern[dashingIndex] - dashingPhase)     /* If the current dash lies within the current line. */
                {
                    ...
                    if ((dashingPhase == 0) || VECTOR_EQUAL(currentNode->points[0].coord, points[i].coord))
                    {
...
void    *TA_Init(gcoOS os, int size, int levels)
{
    ...
    /* The extra 'int' is for counting the size of memory. */
