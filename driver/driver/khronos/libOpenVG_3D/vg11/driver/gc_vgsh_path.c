...
Error:
    ...
    return (VGPath)(path ? path->object.name : VG_INVALID_HANDLE);
}
...
void  vgDestroyPath(VGPath path)
{
    ...
    /* PSC. */
#if vgvUSE_PSC
    _PSCManagerRelease(&context->pscm, (VGint)path);
#endif
...
gctBOOL TessFlatten(_VGContext * context, _VGPath* path, _VGMatrix3x3 *matrix, gctFLOAT strokeWidth)
{
    ...
    if (IsPathDirty(path, phase))
    {
        ...
        result = _FlattenPath(context, path, TESSTYPE(strokeWidth), flattened);
        if (result == 0)
        {
...
gceSTATUS TessFillPath(_VGContext * context, _VGPath* path, _VGMatrix3x3 *matrix)
{
    ...
    if (IsPathDirty(path, VGTessPhase_Fill))
    {
        ...
        status = Tessellate(context, path, matrix);
...
void  vgDrawPath(VGPath path, VGbitfield paintModes)
{
    ...
    if (paintModes & VG_STROKE_PATH)
        vgmPROFILE(context, VG_PROFILER_STROKE, (gctUINTPTR_T)1);
    if (paintModes & VG_FILL_PATH)
        vgmPROFILE(context, VG_PROFILER_FILL, (gctUINTPTR_T)1);
    ...
    if(masking == VG_TRUE && context->maskImage.surface == gcvNULL)
    {
...
VGboolean  vgInterpolatePath(VGPath dstPath, VGPath startPath, VGPath endPath, VGfloat amount)
{
    ...
    if(start.data.size != end.data.size || start.segments.size != end.segments.size ||
       start.segments.items == gcvNULL || end.segments.items == gcvNULL)
    {
