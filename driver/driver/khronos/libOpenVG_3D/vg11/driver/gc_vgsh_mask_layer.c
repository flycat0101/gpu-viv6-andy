...
#define _GC_OBJ_ZONE                gcdZONE_VG3D_MASK
             ...
             VGint width, VGint height)
{
    ...
    object = vgshFindObject(context, (VGuint)mask);
...
VGMaskLayer vgCreateMaskLayer(VGint width, VGint height)
{
    ...
    OVG_IF_ERROR(width > context->image_max_width ||
                 ...
                 width * height > context->image_max_pixels, VG_ILLEGAL_ARGUMENT_ERROR, VG_INVALID_HANDLE);
...
Error:
    vgmGetApiEndTime(context);
    gcmFOOTER_ARG("return=%d", (maskLayerObj ? maskLayerObj->object.name : VG_INVALID_HANDLE));
    return (maskLayerObj ? maskLayerObj->object.name : VG_INVALID_HANDLE);
