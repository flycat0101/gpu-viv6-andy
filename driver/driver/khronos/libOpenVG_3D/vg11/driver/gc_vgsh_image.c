...
void _VGImageCtor(gcoOS os, _VGImage *image)
{
    ...
    gcoTEXTURE_InitParams(gcvNULL, &image->texStates);
...
void _VGImageDestroy(_VGContext *context, gcoOS os, _VGImage *image)
{
     gcmHEADER_ARG("context=0x%x os=0x%x image=0x%x",context, os, image);

     if (image == context->hardware.currentImage)
    {
        if ((context->hardware.currentImage != gcvNULL) &&
            (context->hardware.currentImage->object.type == VGObject_Image))
        {
            VGObject_Release(context->os, (_VGObject*)context->hardware.currentImage);
        }
        context->hardware.currentImage = gcvNULL;
    }
...
void _VGImageDtor(gcoOS os, _VGImage *image)
{
    ...
    _VGImageDestroy(context, os, image);
...
VGImage  vgCreateImage(VGImageFormat format, VGint width, VGint height, VGbitfield allowedQuality)
{
    ...
    OVG_IF_ERROR(width > context->image_max_width ||
                 height > context->image_max_height ||
                 width*height > context->image_max_pixels ||
                 ((width * ImageFormat2Bpp(format) + 7 ) / 8) * height > context->image_max_bytes, VG_ILLEGAL_ARGUMENT_ERROR, VG_INVALID_HANDLE);
...
Error:
    vgmGetApiEndTime(context);
    gcmFOOTER_ARG("return=%d", (image ? image->object.name : VG_INVALID_HANDLE));
    return (VGImage)(image ? image->object.name : VG_INVALID_HANDLE);
}
...
void  vgDestroyImage(VGImage image)
{
    ...
    if (context->hardware.dstImage == imageObjPtr)
    {
        context->hardware.dstImage = gcvNULL;
    }

    if (context->hardware.srcImage == imageObjPtr)
    {
        context->hardware.srcImage = gcvNULL;
    }

    if (context->hardware.currentImage == imageObjPtr)
    {
        if ((context->hardware.currentImage != gcvNULL) &&
            (context->hardware.currentImage->object.type == VGObject_Image))
        {
...
Error:
    vgmGetApiEndTime(context);
    gcmFOOTER_ARG("return=%d", (imageObjPtr ? imageObjPtr->object.name : VG_INVALID_HANDLE));
    return (VGImage)(imageObjPtr ? imageObjPtr->object.name : VG_INVALID_HANDLE);
}
...
void  vgCopyImage(VGImage dst, VGint dx, VGint dy, VGImage src, VGint sx, VGint sy, VGint width, VGint height, VGboolean dither)
{
    ...
    /* Mark dither or not. */
    imageDst->ditherOut = (gctBOOL)dither;
               ...
               gctBOOL fromDraw)
{
    ...
    gcmVERIFY_OK(gcoSURF_Lock(tmpView.surf, address, memory));
    gcmVERIFY_OK(gcoSURF_GetAlignedSize(tmpView.surf, gcvNULL, gcvNULL, &stride));

    rlvArgs.version = gcvHAL_ARG_VERSION_V2;
    rlvArgs.uArgs.v2.yInverted   = fromDraw;
    rlvArgs.uArgs.v2.srcOrigin.x = sx;
    rlvArgs.uArgs.v2.srcOrigin.y = fromDraw ? (VGint)(sh - height - sy) : sy;
    ...
    gctUINT32       flag)
{
    ...
    /* If the srcImage is marked dither output, we should propagate "dither" to the target images. */
    if (srcImage->ditherOut)
    {
        dstImage->ditherOut = srcImage->ditherOut;

        /* If the dst is for data out and bpp is low, should enable dither immediately. */
        if (dstImage->internalColorDesc.bpp <= 16)
        {
            gco3D_EnableDither(context->engine, gcvTRUE);
        }
    }
...
OnError:
    ...
    /* Restore the dither state. */
    if (srcImage->ditherOut && dstImage->internalColorDesc.bpp <= 16)
    {
        gco3D_EnableDither(context->engine, gcvFALSE);
    }
...
_SetupTexture(_VGContext *context, gctINT32 width, gctINT32 height, gceSURF_FORMAT format, gcoSURF surface)
{
    ...
    do
    {
        ...
        status = gcoTEXTURE_ConstructEx(context->hal, gcvTEXTURE_2D, &texture);
...
vgCreateEGLImageTargetKHR(VGeglImageKHR inImage)
{
    ...
    /*can not use the gcoSURF_Clear here!!, because the surface size may not be aligned, will make the clear funciton slow*/
    {
        ...
        gcoOS_ZeroMemory(memory[0], stride * alignedHeight);
        gcmVERIFY_OK(gcoSURF_Unlock(image->texSurface, memory[0]));
    }
