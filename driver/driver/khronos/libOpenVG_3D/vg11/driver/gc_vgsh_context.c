...
#define _GC_OBJ_ZONE                gcdZONE_VG3D_CONTEXT
...
gctFLOAT    VGCLAMP(gctFLOAT a, gctFLOAT l, gctFLOAT h)
{
    if(IsNaN(a))
    {
        return l;
...
void flush(_VGContext *context)
{
    ...
    {
        gcsSURF_VIEW dView = {context->depth, 0, 1};
...
gctFLOAT inputFloat(VGfloat f)
{
    ...
    r = VGCLAMP(f, -FLOAT_MAX, FLOAT_MAX); /*clamp +-inf to +-gctFLOAT max*/
...
_VGObject *GetVGObject(_VGContext *context, _VGObjectType type, VGHandle handle)
{
    ...
    object = vgshFindObject(context, (VGuint)handle);
...
void _VGContextCtor(gcoOS os, _VGContext *context)
{
    ...
    context->image_max_width = gcoHAL_IsFeatureAvailable(context->hal, gcvFEATURE_TEXTURE_8K) ? 8192 : 2048;
    context->image_max_height = context->image_max_width;
    context->image_max_pixels = context->image_max_width * context->image_max_height;
    context->image_max_bytes = context->image_max_pixels * 4;
...
void _VGContextDtor(gcoOS os, _VGContext *context)
{
    ...
    _VGPaintDestroy(os, &context->defaultPaint);
    _VGImageDestroy(context, os, &context->defaultPaint.colorRamp);
    _VGImageDestroy(context, os, &context->targetImage);
    _VGImageDestroy(context, os, &context->maskImage);
    ...
    case VG_COLOR_TRANSFORM_VALUES:
        {
            ...
            for(i = 0; i < 8; i++)
            {
                ...
                context->colorTransformValues[i] = (i < 4 ? VGCLAMP(context->colorTransformValues[i], -127.0f, 127.0f) :
                                                            VGCLAMP(context->colorTransformValues[i], -1.0f, 1.0f));
            }
        }
        break;
    ...
    case VG_TILE_FILL_COLOR:
        {
            ...
            context->tileFillColor.r      = VGCLAMP(context->inputTileFillColor.r, 0.0f, 1.0f);
            context->tileFillColor.g      = VGCLAMP(context->inputTileFillColor.g, 0.0f, 1.0f);
            context->tileFillColor.b      = VGCLAMP(context->inputTileFillColor.b, 0.0f, 1.0f);
            context->tileFillColor.a      = VGCLAMP(context->inputTileFillColor.a, 0.0f, 1.0f);
        }
        break;
    ...
    case VG_MAX_IMAGE_WIDTH:
        {
            ...
            intToParam(values, floats, count, 0, context->image_max_width);
        }
        break;
    ...
    case VG_MAX_IMAGE_HEIGHT:
        {
            ...
            intToParam(values, floats, count, 0, context->image_max_height);
        }
        break;
    ...
    case VG_MAX_IMAGE_PIXELS:
        {
            ...
            intToParam(values, floats, count, 0, context->image_max_pixels);
        }
        break;
    ...
    case VG_MAX_IMAGE_BYTES:
        {
            ...
            intToParam(values, floats, count, 0, context->image_max_bytes);
        }
        break;
...
static void getPathParameterifv(_VGContext* context, _VGPath* path, VGPathParamType type, VGint count, void* values, gctBOOL floats)
{
    gcmHEADER_ARG("context=0x%x path=0x%x type=0x%x count=%d values=0x%x floats=%s",
                  context, path, type, count, values,
                  floats ? "TRUE" : "FALSE");
    switch(type)
    {
    case VG_PATH_FORMAT:
