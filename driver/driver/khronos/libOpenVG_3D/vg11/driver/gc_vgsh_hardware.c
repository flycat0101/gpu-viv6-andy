...
{
    gceSTATUS status = gcvSTATUS_OK;
    gceSTATUS instanceDraw = gcvSTATUS_TRUE;
    gctSIZE_T vertCount = 0;
    ...
    do
    {
        if (instanceDraw == gcvSTATUS_TRUE)
        {
            switch (core->primitiveType)
            {
            case gcvPRIMITIVE_POINT_LIST:
                vertCount = core->primitiveCount;
                break;

            case gcvPRIMITIVE_LINE_LIST:
                vertCount = core->primitiveCount * 2;
                break;
            ...
            case gcvPRIMITIVE_LINE_STRIP:
                vertCount = core->primitiveCount + 1;
                break;

            case gcvPRIMITIVE_TRIANGLE_LIST:
                vertCount = core->primitiveCount * 3;
                break;
...
{
    return TessFillPath(context, path, matrix);
}
    ...
    do
    {
        if (indexBuffer->data.items)
        {
...
#else
            {
                ...
                gcoOS_MemCopy(memory, indexBuffer->data.items, indexBuffer->data.size);
    ...
    do
    {
        if (vertexBuffer->data.items)
        {
...
#else
            {
                ...
                gcoOS_MemCopy(memory, vertexBuffer->data.items, vertexBuffer->data.size);
    ...
    gcoSURF *surface)
{
    ...
    do
    {
        ...
        *texture = gcvNULL;
        ...
        gcmERR_BREAK(gcoTEXTURE_ConstructEx(context->hal, gcvTEXTURE_2D, texture));
...
static gceSTATUS _TextureBind(_vgHARDWARE *hardware, _VGImage* image, gctINT sampler)
{
    gceSTATUS status = gcvSTATUS_OK;
...
gceSTATUS SetUniform_ImageSampler(_vgHARDWARE *hardware, gctINT sampler)
{
    ...
    {
        gcsSURF_VIEW surfView = {hardware->srcImage->surface, 0, 1};
        gcsSURF_VIEW texView  = {hardware->srcImage->texSurface, 0, 1};
    ...
    status = _TextureBind(hardware, hardware->srcImage, sampler);
...
gceSTATUS SetUniform_GradientSampler(_vgHARDWARE *hardware, gctINT sampler)
{
    ...
    {
        gcsSURF_VIEW surfView = {hardware->paint->colorRamp.surface, 0, 1};
        gcsSURF_VIEW texView  = {hardware->paint->colorRamp.texSurface, 0, 1};
    ...
    return _TextureBind(hardware, &hardware->paint->colorRamp, sampler);
}
...
gceSTATUS SetUniform_PatternSampler(_vgHARDWARE *hardware, gctINT sampler)
{
    ...
    {
        gcsSURF_VIEW surfView = {hardware->paint->pattern->surface, 0, 1};
        gcsSURF_VIEW texView  = {hardware->paint->pattern->texSurface, 0, 1};
    ...
    return _TextureBind(hardware, hardware->paint->pattern,  sampler);
}

gceSTATUS SetUniform_LutSampler(_vgHARDWARE *hardware, gctINT sampler)

{
    return _TextureBind(hardware, hardware->lutImage,  sampler);
}
...
gceSTATUS SetUniform_RenderTargetSampler(_vgHARDWARE *hardware, gctINT sampler)
{
    ...
    do
    {
        ...
        {
            gcsSURF_VIEW surfView = {hardware->context->targetImage.surface, 0, 1};
            gcsSURF_VIEW texView  = {hardware->context->targetImage.texSurface, 0, 1};
        ...
        /* Bind the texture. */
        gcmERR_BREAK(
            _TextureBind(hardware, &hardware->context->targetImage,
                         sampler));
...
gceSTATUS SetUniform_MaskSampler(_vgHARDWARE *hardware, gctINT sampler)
{
    ...
    {
        gcsSURF_VIEW surfView = {hardware->context->maskImage.surface, 0, 1};
        gcsSURF_VIEW texView  = {hardware->context->maskImage.texSurface, 0, 1};
    ...
    return _TextureBind(hardware, &hardware->context->maskImage,  sampler);
...
gceSTATUS SetUniform_SourceMaskSampler(_vgHARDWARE *hardware, gctINT sampler)
{
    ...
    return _TextureBind(hardware, hardware->srcImage, sampler);
...
gctUINT16 _AllocateTemp(_VGShader *Shader)
{
    ...
    result = ++Shader->rLastAllocated;
    Shader->binary->_tempRegCount = Shader->rLastAllocated + 1;
    return result;
}
    ...
    vgUNIFORMSET setfunc)
{
    ...
    nameLength = (gctUINT32) gcoOS_StrLen(Name, gcvNULL);
    ...
    vgSAMPLERSET setfunc)
{
    ...
    Shader->samplers[Shader->samplerCount].uniform = uniform;
...
static gceSTATUS _GenFilterFragmentCode(_vgHARDWARE *hardware)
{
    ...
    #define vgmSEPCONVOLVE_COLOR(SwizzleX, SwizzleY) \
        ...
        */
        ...
        if (!gcoHAL_IsFeatureAvailable(hardware->context->hal, gcvFEATURE_HALTI2) &&
            hardware->isConformance && (hardware->context->model >= 0x2000))
        {
            vgmDEFINE_ALLOCATE_TEMP(temp0);
    ...
    do
    {
        ...
        if (hardware->drawPipe == vgvDRAWPIPE_PATH)
        {
            ...
            if (hardware->paintMode == VG_FILL_PATH)
            {
                ...
                if (gcmIS_ERROR(status) || status == gcvSTATUS_SKIP)
                {
                    break;
                }
        ...
        else if (hardware->drawPipe == vgvDRAWPIPE_COLORRAMP)
        {
            ...
            if (paint->dirty & OVGPaintDirty_ColorStop)
            {
                ...
                /* adjust the struct layout for easy upload, and need to do more optimize */
                {
                    ...
                    paint->colorRampStops.size ++;
                    ARRAY_RESIZE(paint->colorStops,  paint->colorRampStops.size * 8 * 2);
                    if (paint->colorStops.items == gcvNULL)
                    {
                        gcmERR_BREAK(gcvSTATUS_OUT_OF_MEMORY);
                    }
                    for(i = 0; i < paint->colorRampStops.size - 1; i++)
                    {

                        paint->colorStops.items[i * 2 * 8]     = paint->colorRampStops.items[i].color.r;
                        paint->colorStops.items[i * 2 * 8 + 1] = paint->colorRampStops.items[i].color.g;
                        paint->colorStops.items[i * 2 * 8 + 2] = paint->colorRampStops.items[i].color.b;
                        paint->colorStops.items[i * 2 * 8 + 3] = paint->colorRampStops.items[i].color.a;

                        paint->colorStops.items[i * 2 * 8 + 4] = (paint->colorRampStops.items[i].offset * 2.0f - 1.0f) * xScale;
                        paint->colorStops.items[i * 2 * 8 + 5] = y;
                        paint->colorStops.items[i * 2 * 8 + 6] = 0.0f;
                        paint->colorStops.items[i * 2 * 8 + 7] = 1.0f;

                        paint->colorStops.items[(i * 2+1) * 8]     = paint->colorRampStops.items[i].color.r;
                        paint->colorStops.items[(i * 2+1) * 8 + 1] = paint->colorRampStops.items[i].color.g;
                        paint->colorStops.items[(i * 2+1) * 8 + 2] = paint->colorRampStops.items[i].color.b;
                        paint->colorStops.items[(i * 2+1) * 8 + 3] = paint->colorRampStops.items[i].color.a;

                        paint->colorStops.items[(i * 2+1) * 8 + 4] = (paint->colorRampStops.items[i].offset * 2.0f - 1.0f) * xScale;
                        paint->colorStops.items[(i * 2+1) * 8 + 5] = -1.0f;
                        paint->colorStops.items[(i * 2+1) * 8 + 6] = 0.0f;
                        paint->colorStops.items[(i * 2+1) * 8 + 7] = 1.0f;

                    }

                    paint->colorStops.items[i * 2 * 8]     = paint->colorRampStops.items[i-1].color.r;
                    paint->colorStops.items[i * 2 * 8 + 1] = paint->colorRampStops.items[i-1].color.g;
                    paint->colorStops.items[i * 2 * 8 + 2] = paint->colorRampStops.items[i-1].color.b;
                    paint->colorStops.items[i * 2 * 8 + 3] = paint->colorRampStops.items[i-1].color.a;

                    paint->colorStops.items[i * 2 * 8 + 4] = 1.5f;
                    paint->colorStops.items[i * 2 * 8 + 5] = y;
                    paint->colorStops.items[i * 2 * 8 + 6] = 0.0f;
                    paint->colorStops.items[i * 2 * 8 + 7] = 1.0f;


                    paint->colorStops.items[(i * 2 + 1)* 8]     = paint->colorRampStops.items[i-1].color.r;
                    paint->colorStops.items[(i * 2 + 1)* 8 + 1] = paint->colorRampStops.items[i-1].color.g;
                    paint->colorStops.items[(i * 2 + 1)* 8 + 2] = paint->colorRampStops.items[i-1].color.b;
                    paint->colorStops.items[(i * 2 + 1)* 8 + 3] = paint->colorRampStops.items[i-1].color.a;

                    paint->colorStops.items[(i * 2 + 1)* 8 + 4] = 1.5f;
                    paint->colorStops.items[(i * 2 + 1)* 8 + 5] = -1.0f;
                    paint->colorStops.items[(i * 2 + 1)* 8 + 6] = 0.0f;
                    paint->colorStops.items[(i * 2 + 1)* 8 + 7] = 1.0f;
            ...
            primitiveCount      = (paint->colorRampStops.size  - 1)*2;
...
gceSTATUS _LoadUniforms(_vgHARDWARE *hardware)
{
    ...
    {
        gcUNIFORM uniform = vertShader->uniforms[i].uniform;
    ...
    {
        gcUNIFORM uniform = fragShader->uniforms[i].uniform;
        if (uniform && !isUniformInactive(uniform))
        {
    ...
    {
        /* If the sampler is not used on this shader, we don't need to bind it.
        ** And the physical address of sampler may change in gcLinkShader, we should use the latest physical address.
        */
        gcUNIFORM uniform = fragShader->samplers[i].uniform;
    ...
    gctBOOL        flush)
{
    ...
    do
    {
        ...
        {
            gcsSURF_VIEW imgView = {gcvNULL, 0, 1};

            gcoOS_ZeroMemory(&clearArgs, sizeof(clearArgs));
            clearArgs.color.r.floatValue = color->r;
            clearArgs.color.g.floatValue = color->g;
            clearArgs.color.b.floatValue = color->b;
            clearArgs.color.a.floatValue = color->a;
            clearArgs.color.valueType = gcvVALUE_FLOAT;
            clearArgs.clearRect = gcvNULL;
            clearArgs.colorMask = 0xF;
            ...
            if (image != context->hardware.currentImage)
            {
                if ((context->hardware.currentImage != gcvNULL)
                    ...
                    && (context->hardware.currentImage->dirtyPtr)
                    && (*context->hardware.currentImage->dirtyPtr))
                {
                    ...
                    *context->hardware.currentImage->dirtyPtr = gcvFALSE;
                }
                ...
                context->hardware.currentImage = image;
                VGObject_AddRef(context->os, (_VGObject*)context->hardware.currentImage);
    ...
    do
    {
        ...
        if (context->maskImage.surface == gcvNULL)
        {
...
#if DISABLE_TILE_STATUS
            COLOR_SET(zeroColor, 1.0, 1.0, 1.0, 1.0, sRGBA);
#endif
            ...
            gcmVERIFY_OK(vgshClear(context,
                &context->maskImage,
                0, 0, context->maskImage.width, context->maskImage.height,
                &zeroColor,
                gcvFALSE,
                gcvFALSE));
#else
            gcoOS_ZeroMemory(&clearArgs, sizeof(clearArgs));
            clearArgs.color.r.floatValue =
            clearArgs.color.g.floatValue =
            clearArgs.color.b.floatValue =
            clearArgs.color.a.floatValue = 1.0f;
            clearArgs.color.valueType = gcvVALUE_FLOAT;
            clearArgs.clearRect = gcvNULL;
            clearArgs.colorMask = 0xF;
            clearArgs.flags = gcvCLEAR_COLOR;
...
#else
        if ((hardware->currentImage != gcvNULL)
            && (*hardware->currentImage->dirtyPtr == gcvTRUE)
            && (hardware->currentImage != hardware->dstImage) )
        {
            ...
            *hardware->currentImage->dirtyPtr = gcvFALSE;
        }
#endif
        /* state setting */
...
#if DISABLE_TILE_STATUS
        if ((hardware->currentImage != gcvNULL) &&
            (hardware->currentImage->object.type == VGObject_Image))
        {
            VGObject_Release(hardware->context->os, (_VGObject*)hardware->currentImage);
        }
        hardware->currentImage = hardware->dstImage;
        VGObject_AddRef(hardware->context->os, (_VGObject*)hardware->currentImage);
#endif
