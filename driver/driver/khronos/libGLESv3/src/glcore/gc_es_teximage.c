...
static GLboolean __glCheckTexCopyImgFmt(__GLcontext *gc, __GLtextureObject * tex, GLint internalFormat, GLboolean compSizeMatch)
{
                                        ...
                                        const GLvoid *buf)
{
    ...
    if (!__glCheckTexImgFmt(gc, tex, tex->faceMipmap[0][lod].requestedFormat, format, type))
    {
        __GL_EXIT();
    }
...
GLvoid GL_APIENTRY __gles_EGLImageTargetTexture2DOES(__GLcontext *gc, GLenum target, GLeglImageOES image)
{
    ...
    case KHR_IMAGE_TEXTURE_CUBE:
        ...
        if (!__glCheckTexImgFmt(gc, texObj, internalFormat, format, type))
        {
            __GL_EXIT();
        }
