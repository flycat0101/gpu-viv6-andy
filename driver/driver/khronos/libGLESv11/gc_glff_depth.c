    ...
    )
{
    ...
    do
    {
        ...
        if (depthMode != Context->depthStates.depthMode)
        {
            ...
            Context->depthStates.depthOnlyDirty = gcvTRUE;
        }
        ...
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_EnableDepthWrite(
            Context->hw,
                isS8Depth ? gcvFALSE:
                Context->depthStates.testEnabled ?
    ...
    )
{
    ...
    if (Context->depthStates.polygonOffsetDirty)
    {
        ...
        GLfloat depthScale = Context->depthStates.depthFactor;

        GLfloat depthBias = Context->depthStates.depthUnits;

        /* Convert depth bias to float range. */
        depthBias = depthBias * glvFLOATONEOVER65535;
            ...
            != gcvSTATUS_TRUE)
        {
            ...
            */
            ...
            Context->hashKey.hashDepthBias = depthBias + (depthScale * glvFLOATONEOVER65535);
    ...
    )
{
    ...
    result = glmTRANSLATEHALSTATUS(gco3D_EnableDepthWrite(
        Context->hw,
        Context->depthStates.testEnabled ? DepthMask : gcvFALSE
        ));
    ...
    )
{
    ...
    gcmHEADER_ARG("Context=0x%x ClearValue=%1.4f", Context, ClearValue);

    /* Set with [0..1] clamping. */
    Context->depthStates.clearValue = glmFLOATCLAMP_0_TO_1(ClearValue);
...
{
    gcmHEADER_ARG("Context=0x%x Factor=%1.4f Units=%1.4f", Context, Factor, Units);

    Context->depthStates.depthFactor = Factor;
    Context->depthStates.depthUnits = Units;
    ...
    )
{
    ...
    gcmHEADER_ARG("Context=0x%x zNear=%1.4f zFar=%1.4f", Context, zNear, zFar);

    /* Set with [0..1] clamping. */
    Context->depthStates.depthRange[0] = glmFLOATCLAMP_0_TO_1(zNear);
    Context->depthStates.depthRange[1] = glmFLOATCLAMP_0_TO_1(zFar);
    ...
    )
{
    ...
    result = (format == gcvSURF_D24S8 || format == gcvSURF_X24S8 || format == gcvSURF_S8)
            ? 8
            : 0;
    ...
    )
{
    ...
    do
    {
        ...
        Context->stencilStates.hal.maskFront =
        Context->stencilStates.hal.maskBack
            = (gctUINT8)(Mask & 0x00FF);
    ...
    )
{
    ...
    Context->stencilStates.hal.writeMaskFront =
    Context->stencilStates.hal.writeMaskBack
        = (gctUINT8)(Mask & 0x00FF);
    Context->stencilStates.dirty         = GL_TRUE;
}
    ...
    )
{
    ...
    do
    {
        /* Set depth only flag dirty. */
        Context->depthStates.depthOnlyDirty = gcvTRUE;

        /* Flush early-depth: always turn on early-depth. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetEarlyDepth(
            Context->hw,
            gcvTRUE
            )));

        /* Flush depth enabling/compare function. */
        glmERR_BREAK(_UpdateDepthFunction(Context));
        ...
        /* Flush the depth clear value. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetClearDepth(
            Context->hw,
            Context->depthStates.clearValue
            )));

        /* Set polygon offset states dirty and will flush later. */
        Context->depthStates.polygonOffsetDirty = gcvTRUE;

        /* Flush depth range. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetDepthRange(
            Context->hw,
            Context->depthStates.depthMode,
        ...
        /* Set stencil states and will flush later. */
        Context->stencilStates.dirty = gcvTRUE;

        /* Flush stencil clear. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetClearStencil(
            Context->hw,
...
**  glfDetectDepthOnly
**
**  Detects if depth only mode is changed and acts accordingly.
**
**  INPUT:
**
**      Context
    ...
    )
{
    ...
    do
    {
        if ((Context->depthStates.depthOnlyDirty) && (Context->frameBuffer == gcvNULL))
        {
            if (Context->depthStates.depthMode == gcvDEPTH_NONE ||
                Context->pointStates.smooth == GL_TRUE)
            {
                gcmERR_BREAK(gco3D_SetDepthOnly(Context->hw, gcvFALSE));
            }
            else
            {
                gcmERR_BREAK(gco3D_SetDepthOnly(Context->hw, Context->depthStates.depthOnly));
            }
            Context->depthStates.depthOnlyDirty = gcvFALSE;
        }
    }
    while (gcvFALSE);
    gcmFOOTER();
    ...
    )
{
    glmENTER2(glmARGFLOAT, zNear, glmARGFLOAT, zFar)
    {
        ...
        glmERROR(_SetDepthRange(context, zNear, zFar));
    }
    glmLEAVE();
}
    ...
    )
{
    glmENTER1(glmARGFIXED, ClearValue)
    {
        ...
        /* convert to float */
        clearValue = glmFIXED2FLOAT(ClearValue);

        glmERROR(_SetClearDepth(context, clearValue));
    }
    glmLEAVE();
}
    ...
    )
{
    glmENTER2(glmARGFIXED, Factor, glmARGFIXED, Units)
    {
        ...
        /* convert to float */
        factor = glmFIXED2FLOAT(Factor);
        units = glmFIXED2FLOAT(Units);
...
{
    gceCOMPARE function;
    gleTESTFUNCTIONS testFunction=0;
