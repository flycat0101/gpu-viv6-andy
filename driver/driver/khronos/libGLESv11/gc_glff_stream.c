    ...
    )
{
    ...
    gcmHEADER_ARG("Context=0x%x Target=0x%04x Value=0x%x",
    ...
    )
{
    ...
    glsNAMEDOBJECT_PTR preObj = gcvNULL;
    ...
    {
        glsBUFFER_PTR object = (glsBUFFER_PTR)Stream->buffer->object;

        preObj = (glsNAMEDOBJECT_PTR)Stream->buffer;

        if(preObj != Buffer)
        {
            preObj->bindCount--;
            if (!preObj->bindCount && (preObj->flag & __GL_OBJECT_IS_DELETED))
            {
                if (gcmIS_ERROR(glfDeleteNamedObject(
                    Context, shared->bufferList, preObj->name
                    )))
                {
                    glmERROR(GL_INVALID_OPERATION);
                    gcmFATAL("glfSetStreamParameters failed to delete a named object.");
                }
            }
        }
    ...
    if (Buffer)
    {
        ...
        wrapper = (glsNAMEDOBJECT_PTR)Stream->buffer;
        if(preObj != wrapper)
        {
            wrapper->bindCount++;
        }
    ...
    gcmUNLOCK_SHARE_OBJ(shared->bufferList);
    ...
    )
{
    ...
    do
    {
        glmERR_BREAK(_SetCurrentNormal(Context, vec0010));
        glmERR_BREAK(_SetCurrentColor(Context, vec1111));
        glmERR_BREAK(_SetCurrentPointSize(Context, vec1000));
...
gceSTATUS glfFlushStreams(
    glsCONTEXT_PTR Context
    )
{
    gcmHEADER_ARG("Context=0x%x", Context);

    Context->aPositionInfo.dirty = gcvTRUE;
    Context->aPositionDrawTexInfo.dirty = gcvTRUE;
    Context->aPositionDrawClearRectInfo.dirty = gcvTRUE;
    Context->aNormalInfo.dirty = gcvTRUE;
    Context->aColorInfo.dirty = gcvTRUE;
    Context->aPointSizeInfo.dirty = gcvTRUE;
    Context->aMatrixIndexInfo.dirty = gcvTRUE;
    Context->aWeightInfo.dirty = gcvTRUE;
    ...
    )
{
    glmENTER3(glmARGFIXED, X, glmARGFIXED, Y, glmARGFIXED, Z)
    {
        ...
        normal[0] = glmFIXED2FLOAT(X);
        normal[1] = glmFIXED2FLOAT(Y);
        normal[2] = glmFIXED2FLOAT(Z);
        normal[3] = glvFLOATONE;

        glmERROR(_SetCurrentNormal(context, normal));
    }
    glmLEAVE();
}
    ...
    )
{
             ...
             glmARGFIXED, Blue, glmARGFIXED, Alpha)
    {
        ...
        color[0] = glmFIXED2FLOAT(Red);
        color[1] = glmFIXED2FLOAT(Green);
        color[2] = glmFIXED2FLOAT(Blue);
        color[3] = glmFIXED2FLOAT(Alpha);

        glmERROR(_SetCurrentColor(context, color));
    }
    glmLEAVE();
}
    ...
    )
{
             ...
             glmARGUINT, Blue, glmARGUINT, Alpha)
    {
        ...
        color[0] = Red   * glvFLOATONEOVER255;
        color[1] = Green * glvFLOATONEOVER255;
        color[2] = Blue  * glvFLOATONEOVER255;
        color[3] = Alpha * glvFLOATONEOVER255;
    ...
    )
{
    glmENTER1(glmARGFLOAT, Size)
    {
        ...
        pointSize[0] = Size;
        pointSize[1] = glvFLOATZERO;
        pointSize[2] = glvFLOATZERO;
        pointSize[3] = glvFLOATZERO;

        glmERROR(_SetCurrentPointSize(context, pointSize));
    }
    glmLEAVE();
}
    ...
    )
{
    glmENTER1(glmARGFIXED, Size)
    {
        ...
        pointSize[0] = glmFIXED2FLOAT(Size);
        pointSize[1] = glvFLOATZERO;
        pointSize[2] = glvFLOATZERO;
        pointSize[3] = glvFLOATZERO;
    ...
    )
{
              ...
              glmARGFIXED, R, glmARGFIXED, Q)
    {
        ...
        texCoord[0] = glmFIXED2FLOAT(S);
        texCoord[1] = glmFIXED2FLOAT(T);
        texCoord[2] = glmFIXED2FLOAT(R);
        texCoord[3] = glmFIXED2FLOAT(Q);

        glmERROR(_SetCurrentTexCoord(
            context, Target, texCoord
            ));
    }
    glmLEAVE();
}
