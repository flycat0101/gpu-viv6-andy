...
typedef enum _gleFS_UNIFORMS
{
    ...
    glmUNIFORM_INDEX(FS, uFlipPointSprite),
...
#define glmOPCODE(Opcode, TempRegister, ComponentEnable, Precision) \
...
#define glmVARYING_INDEXED_PRECISION(Shader, Name, Index, Swizzle, Precision) \
    gcmERR_BREAK(gcSHADER_AddSourceAttributeIndexedFormattedWithPrecision( \
        ShaderControl->i->shader, \
        glmATTRIBUTE_WRAP_INDEXED(Shader, Name, Index)->attribute, \
        gcSL_SWIZZLE_##Swizzle, \
        0, \
...
#define glmTEMP(TempRegister, Swizzle, Precision) \
    gcmASSERT(TempRegister != 0); \
...
#define glmTEMPV(TempRegister, Swizzle, Precision) \
    gcmASSERT(TempRegister != 0); \
    gcmERR_BREAK(gcSHADER_AddSource( \
        ShaderControl->i->shader, \
        gcSL_TEMP, \
...
typedef struct _glsALPHA
{
    ...
    GLfloat                 testReference;

    /* Latest blend func from separate? */
    GLboolean               blendFuncSeparate;
}
glsALPHA;
...
typedef struct _glsFOG
{
    ...
    GLfloat                 linearFactor0;
    GLfloat                 linearFactor1;
...
typedef struct _glsFSUNIFORMDIRTYINFO
{
    ...
    /* PointSprite uniforms */
    gctBOOL                 uPointSpriteCoordDirty;

    gctBOOL                 uAlphaRefDirty;
}
