    ...
    )
{
    ...
    if (!Context->modelViewProjectionMatrix.dirty && !Context->programDirty)
    {
        status = gcvSTATUS_OK;
        gcmFOOTER();
        return status;
...
{
    GLint i, x, y;
    GLfloat valueArray[4 * 4 * glvMAX_TEXTURES];
    ...
    if (!Context->texture.matrixDirty && !Context->programDirty)
    {
        status = gcvSTATUS_OK;
        gcmFOOTER();
        return status;
    ...
    )
{
    ...
    GLint ViewportScaleY  = Context->viewportStates.viewportBox[3] / 2;
    GLint ViewportOriginY = Context->viewportStates.viewportBox[1] + Context->viewportStates.viewportBox[3] / 2;

    gcmHEADER_ARG("Context=0x%x Uniform=0x%x", Context, Uniform);

    if(Context->drawYInverted)
    {
        ViewportScaleY  = -(Context->viewportStates.viewportBox[3])/2;
        ViewportOriginY = (Context->drawHeight - Context->viewportStates.viewportBox[1])
    ...
    status = glfSetUniformFromFractions(
        ...
        /* ViewportScaleY */
        glmINT2FLOAT(ViewportScaleY),

        /* ViewportOriginY */
        glmINT2FLOAT(ViewportOriginY)
       );
    gcmFOOTER();
    return status;
}
...
{
    GLfloat valueArray[glvMAX_LIGHTS];
    GLfloat   mCosCrli[glvMAX_LIGHTS];
    GLint       i;
    gceSTATUS status;
    ...
    for (i = 0; i < glvMAX_LIGHTS; i++)
    {
        glfCos(&Context->lightingStates.Crli[i], &mCosCrli[i]);
    }
    ...
    )
{
    ...
    do
    {
        ...
        if (Context->matrixPaletteEnabled)
        {
            ...
            glmOPCODE(MUL, temp1, XYZW, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aMatrixIndex, XYZW);
                glmCONST(4);
            ...
            for (i = 0; i < count; i++)
            {
                ...
                glmOPCODE(DP4, temp2, X, gcSHADER_PRECISION_HIGH);
                ...
                else
                {
                    ...
                    glmOPCODE(ADD, temp4, XYZW, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp3, XYZW, gcSHADER_PRECISION_HIGH);
        ...
        else
        {
            ...
            if (Context->modelViewMatrix->identity)
            {
                ...
                glmOPCODE(MOV, ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
                    glmATTRIBUTE(VS, aPosition, XYZW);
            }
            else
            {
                ...
                glmOPCODE(DP4, ShaderControl->rVtxInEyeSpace, X, gcSHADER_PRECISION_HIGH);
                    glmATTRIBUTE(VS, aPosition, XYZW);
                    glmUNIFORM_STATIC(VS, uModelView, XYZW, 0);

                /* dp4 rVtxInEyeSpace.y, aPosition, uModelView[1] */
                glmOPCODE(DP4, ShaderControl->rVtxInEyeSpace, Y, gcSHADER_PRECISION_HIGH);
                    glmATTRIBUTE(VS, aPosition, XYZW);
                    glmUNIFORM_STATIC(VS, uModelView, XYZW, 1);

                /* dp4 rVtxInEyeSpace.z, aPosition, uModelView[2] */
                glmOPCODE(DP4, ShaderControl->rVtxInEyeSpace, Z, gcSHADER_PRECISION_HIGH);
                    glmATTRIBUTE(VS, aPosition, XYZW);
                    glmUNIFORM_STATIC(VS, uModelView, XYZW, 2);

                /* dp4 rVtxInEyeSpace.w, aPosition, uModelView[3] */
                glmOPCODE(DP4, ShaderControl->rVtxInEyeSpace, W, gcSHADER_PRECISION_HIGH);
                    glmATTRIBUTE(VS, aPosition, XYZW);
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(DP4, ShaderControl->vPosition, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
            glmUNIFORM_STATIC(VS, uProjection, XYZW, 0);

        /* dp4 vPosition.y, rVtxInEyeSpace, uProjection[1] */
        glmOPCODE(DP4, ShaderControl->vPosition, Y, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
            glmUNIFORM_STATIC(VS, uProjection, XYZW, 1);

        /* dp4 vPosition.z, rVtxInEyeSpace, uProjection[2] */
        glmOPCODE(DP4, ShaderControl->vPosition, Z, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
            glmUNIFORM_STATIC(VS, uProjection, XYZW, 2);

        /* dp4 vPosition.w, rVtxInEyeSpace, uProjection[3] */
        glmOPCODE(DP4, ShaderControl->vPosition, W, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
    ...
    )
{
    ...
    do
    {
        ...
        if (Context->drawTexOESEnabled)
        {
            ...
            /* vPosition = aPosition */
            glmOPCODE(MOV, ShaderControl->vPosition, XYZW, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aPosition, XYZW);
        }
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(MUL, temp1, XYZ, gcSHADER_PRECISION_HIGH);
        ...
        if (Context->aNormalInfo.streamEnabled)
        {
            ...
            /* temp2 = aNormal */
            glmOPCODE(MOV, temp2, XYZ, gcSHADER_PRECISION_HIGH);
        ...
        for (i = 0; i < count; i++)
        {
            ...
            glmOPCODE(DP3, temp3, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);
            ...
            glmOPCODE(DP3, temp3, Y, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);
            ...
            glmOPCODE(DP3, temp3, Z, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);
                glmUNIFORM_DYNAMIC_MATRIX(VS, uMatrixPaletteInverse, XYZZ, temp1, 2, g_ArrayIndex[i]);

            /* temp4 = temp3 * aMatrixWeight[i] */
            glmOPCODE(MUL, temp4, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp3, XYZZ, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTEV(VS, aMatrixWeight, g_ArraySwizzle[i]);
            ...
            else
            {
                ...
                glmOPCODE(ADD, temp5, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->rNrmInEyeSpace[0], XYZZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp4, XYZZ, gcSHADER_PRECISION_HIGH);
    ...
    )
{
    ...
    do
    {
        ...
        if (Context->aNormalInfo.streamEnabled)
        {
            ...
            /* temp = aNormal */
            glmOPCODE(MOV, temp, XYZ, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aNormal, XYZZ);
        }
        ...
        else
        {
            ...
            glmOPCODE(DP3, ShaderControl->rNrmInEyeSpace[0], X, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                glmUNIFORM_STATIC(VS, uModelViewInverse3x3Transposed, XYZZ, 0);

            /* rNrmInEyeSpace[0].y = dot(temp, uModelViewInverse3x3Transposed[1]) */
            glmOPCODE(DP3, ShaderControl->rNrmInEyeSpace[0], Y, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                glmUNIFORM_STATIC(VS, uModelViewInverse3x3Transposed, XYZZ, 1);

            /* rNrmInEyeSpace[0].z = dot(temp, uModelViewInverse3x3Transposed[2]) */
            glmOPCODE(DP3, ShaderControl->rNrmInEyeSpace[0], Z, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
    ...
    )
{
    ...
    do
    {
            ...
            !glfGetModelViewInverse3x3TransposedMatrix(Context)->identity)
        {
            ...
            glmOPCODE(DP3, temp1, X, gcSHADER_PRECISION_HIGH);
                glmUNIFORM_STATIC(VS, uModelViewInverse3x3Transposed, XYZZ, 2);
                glmUNIFORM_STATIC(VS, uModelViewInverse3x3Transposed, XYZZ, 2);

            /* temp2.x = scale factor */
            glmOPCODE(RSQ, temp2, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp1, XXXX, gcSHADER_PRECISION_HIGH);

            /* Rescale the normal. */
            glmOPCODE(MUL, ShaderControl->rNrmInEyeSpace[0], XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(prevNrmInEyeSpace, XYZZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);
        }
        ...
        if (Context->normalizeNormal)
        {
            ...
            glmOPCODE(NORM, ShaderControl->rNrmInEyeSpace[0], XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(prevNrmInEyeSpace, XYZZ, gcSHADER_PRECISION_HIGH);
        }
        ...
        if (ShaderControl->outputCount == 2)
        {
            ...
            glmOPCODE(SUB, ShaderControl->rNrmInEyeSpace[1], XYZ, gcSHADER_PRECISION_HIGH);
                glmCONST(0);
                glmTEMP(ShaderControl->rNrmInEyeSpace[0], XYZZ, gcSHADER_PRECISION_HIGH);
        }
    }
    ...
    )
{
    ...
    do
    {
        ...
        {
            /* temp1.z = d^2 */
            glmOPCODE(DP3, temp1, YZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rVtxInEyeSpace, XYZZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rVtxInEyeSpace, XYZZ, gcSHADER_PRECISION_HIGH);

            /* Make sure Z is not zero, otherwize RSQ will produce an INF. */
            glmOPCODE_BRANCH(JMP, EQUAL, lblZero);
                glmTEMP(temp1, ZZZZ, gcSHADER_PRECISION_HIGH);
                glmCONST(0);

            {
                /* temp1.y = 1 / sqrt(d^2) */
                glmOPCODE(RSQ, temp2, Y, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, ZZZZ, gcSHADER_PRECISION_HIGH);

                /* temp2.z = temp1.z */
                glmOPCODE(MOV, temp2, Z, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, ZZZZ, gcSHADER_PRECISION_HIGH);

                /* temp1.y = d */
                glmOPCODE(MUL, temp1, Y, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp2, YYYY, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp2, ZZZZ, gcSHADER_PRECISION_HIGH);
            }
            ...
            /* temp1.x = 1 */
            glmOPCODE(MOV, temp1, X, gcSHADER_PRECISION_HIGH);
                glmCONST(1);
        }

        /* temp2.x = a + b * d + c * d^2 */
        glmOPCODE(DP3, temp2, X, gcSHADER_PRECISION_HIGH);
            glmUNIFORM(VS, uPointAttenuation, XYZZ);
            glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);

        /* temp3.x = scale_factor */
        glmOPCODE(RSQ, temp3, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);
        ...
        if (Context->aPointSizeInfo.streamEnabled)
        {
            ...
            glmOPCODE(MUL, temp4, X, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aPointSize, XXXX);
                glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
        }
        ...
        glmOPCODE(MAX, temp5, X, gcSHADER_PRECISION_HIGH);
            glmUNIFORM(VS, uPointSize, YYYY);
            glmTEMP(temp4, XXXX, gcSHADER_PRECISION_HIGH);

        /* Clamp by the upper boundary (uPointSize.z) = derived_size. */
        glmOPCODE(MIN, temp6, X, gcSHADER_PRECISION_HIGH);
            glmUNIFORM(VS, uPointSize, ZZZZ);
            glmTEMP(temp5, XXXX, gcSHADER_PRECISION_HIGH);
        ...
        if (Context->multisampleStates.enabled)
        {
            ...
            glmOPCODE(MAX, ShaderControl->vPointSize, X, gcSHADER_PRECISION_HIGH);
                glmUNIFORM(VS, uPointSize, WWWW);
                glmTEMP(temp6, XXXX, gcSHADER_PRECISION_HIGH);

            /* Initialize fade factor to 1. */
            glmOPCODE(MOV, ShaderControl->vPointFade, X, gcSHADER_PRECISION_HIGH);
                glmCONST(1);
...

            {
                ...
                glmOPCODE(RCP, temp7, X, gcSHADER_PRECISION_HIGH);
                    glmUNIFORM(VS, uPointSize, WWWW);

                /* temp8.x = derived_size / fadeThrdshold. */
                glmOPCODE(MUL, temp8, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp6, XXXX, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp7, XXXX, gcSHADER_PRECISION_HIGH);

                /* vPointFade.x = (derived_size / fadeThrdshold) ^ 2. */
                glmOPCODE(MUL, ShaderControl->vPointFade, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp8, XXXX, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp8, XXXX, gcSHADER_PRECISION_HIGH);
            }
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(RCP, temp1, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->vPosition, WWWW, gcSHADER_PRECISION_HIGH);

        glmOPCODE(MUL, temp2, XY, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp1, XXXX, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->vPosition, XYYY, gcSHADER_PRECISION_HIGH);

        /* PointScreenX (vPointSmooth.x)
              = PositionX * ViewportScaleX + ViewportOriginX */
        glmOPCODE(MUL, temp3, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);
            glmUNIFORM(VS, uViewport, XXXX);

        glmOPCODE(ADD, ShaderControl->vPointSmooth, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
        ...
        glmOPCODE(ADD, ShaderControl->vPointSmooth, Y, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp4, XXXX, gcSHADER_PRECISION_HIGH);
            glmUNIFORM(VS, uViewport, WWWW);

        /* temp3.x = the radius of the point. */
        glmOPCODE(MUL, temp5, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->vPointSize, XXXX, gcSHADER_PRECISION_HIGH);
            glmCONST(0.5f);

        /* vPointSmooth.z = radius^2. */
        glmOPCODE(MUL, ShaderControl->vPointSmooth, Z, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp5, XXXX, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp5, XXXX, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
        ...
        */
        if (Context->lightingStates.Acs.zero3)
        {
            ...
            */
            ...
            {
                /* rLighting[0] = 0 */
                glmOPCODE(MOV, ShaderControl->rLighting[0], XYZ, gcSHADER_PRECISION_HIGH);
                    glmCONST(0);
            }
            /*
            ** BLOCK2: Ecm == 0, (Acs == 0)
                ...
                */
                if (Context->lightingStates.materialEnabled)
                {
                    ...
                    if (Context->aColorInfo.streamEnabled)
                    {
                        ...
                        glmOPCODE(MUL, ShaderControl->rLighting[0], XYZ, gcSHADER_PRECISION_HIGH);
                ...
                */
                if (Context->lightingStates.materialEnabled)
                {
                    ...
                    if (Context->aColorInfo.streamEnabled)
                    {
                        ...
                        /* temp = aColor * Acs */
                        glmOPCODE(MUL, temp, XYZ, gcSHADER_PRECISION_HIGH);
                            glmATTRIBUTE(VS, aColor, XYZZ);
                            glmUNIFORM(VS, uAcs, XYZZ);

                        /* rLighting[0] = Ecm + temp */
                        glmOPCODE(ADD, ShaderControl->rLighting[0], XYZ, gcSHADER_PRECISION_HIGH);
                            glmUNIFORM(VS, uEcm, XYZZ);
                            glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                    }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            /* temp1 = uColor */
                            glmOPCODE(MOV, temp1, XYZ, gcSHADER_PRECISION_HIGH);
                                glmUNIFORM(VS, uColor, XYZZ);

                            /* temp2 = temp1 * Acs */
                            glmOPCODE(MUL, temp2, XYZ, gcSHADER_PRECISION_HIGH);
                                glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                                glmUNIFORM(VS, uAcs, XYZZ);
        ...
        ***********************************************************************/
        ...
        if (!Context->lightingStates.materialEnabled)
        {
            ...
            /* Accordingly to OpenGL ES spec, alpha component comes from alpha value
               associated with Dcm. */
            glmOPCODE(MOV, ShaderControl->rLighting[0], W, gcSHADER_PRECISION_HIGH);
        ...
        else
        {
            if (Context->aColorInfo.streamEnabled)
            {
                glmUSING_ATTRIBUTE(aColor);
                glmOPCODE(MOV, ShaderControl->rLighting[0], W, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aColor, WWWW);
            }

            /* Alpha from a constant. */
            else
            {
                glmUSING_UNIFORM(uColor);
                glmOPCODE(MOV, ShaderControl->rLighting[0], W, gcSHADER_PRECISION_HIGH);
                glmUNIFORM(VS, uColor, WWWW);
            }
        }
        ...
        if (ShaderControl->outputCount == 2)
        {
            ...
            /* Copy the value. */
            glmOPCODE(MOV, ShaderControl->rLighting[1], XYZW, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rLighting[0], XYZW, gcSHADER_PRECISION_HIGH);
        }
    }
    while (gcvFALSE);
    ...
    do
    {
        if (ShaderControl->rVPpli == 0)
        {
            ...
            */
            ...
            glmOPCODE(MOV, temp1, XYZW, gcSHADER_PRECISION_HIGH);
            ...
            if (LightIndex < 0)
            {
                ...
                glmOPCODE_BRANCH(JMP, EQUAL, lblDirectional);
                    glmTEMP(temp1, WWWW, gcSHADER_PRECISION_HIGH);
                    glmCONST(0);

                {
                    /* Compute VPpli:
                       temp1 = temp1 - V. */
                    glmOPCODE(MOV, temp2, XYZW, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp1, XYZW, gcSHADER_PRECISION_HIGH);
                    glmOPCODE(SUB, temp1, XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->rVtxInEyeSpace, XYZZ, gcSHADER_PRECISION_HIGH);
                }
            ...
            {
                    glmOPCODE(MOV, ShaderControl->rVPpli, XYZ, gcSHADER_PRECISION_HIGH);
            ...
            else
            {
                ...
                */
                ...
                glmOPCODE(MUL, ShaderControl->rVPpli, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);

                /* Compute the length of the vector. */
                glmOPCODE(MUL, ShaderControl->rVPpliLength, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
            }
        }
    ...
    )
{
    ...
    do
    {
        if (ShaderControl->rNdotVPpli[OutputIndex] == 0)
        {
            ...
            /* temp.x = dot(n, VPpli) */
            glmOPCODE(DP3, temp, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rNrmInEyeSpace[OutputIndex], XYZZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rVPpli, XYZZ, gcSHADER_PRECISION_HIGH);
            ...
            if (Context->normalizeNormal)
            {
                ...
                glmOPCODE(SAT, ShaderControl->rNdotVPpli[OutputIndex], X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp, XXXX, gcSHADER_PRECISION_HIGH);
            }
            else
            {
                /* rNdotVPpli[OutputIndex] = max(dot(n, VPpli), 0). */
                glmOPCODE(MAX, ShaderControl->rNdotVPpli[OutputIndex], X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp, XXXX, gcSHADER_PRECISION_HIGH);
                    glmCONST(0);
            }
        }
    ...
    )
{
    ...
    do
    {
        ...
        if (
                (
                    ...
                    (
                        ...
                        (
                          (glfISONE(Context->lightingStates.K0i[LightIndex]))  &&
                          (glfISZERO(Context->lightingStates.K1i[LightIndex])) &&
                          (glfISZERO(Context->lightingStates.K2i[LightIndex]))
                         )
                    )
                )
        ...
        else
        {
            ...
            /* temp1.y = len(VPpli) */
            glmOPCODE(MOV, temp1, Y, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rVPpliLength, XXXX, gcSHADER_PRECISION_HIGH);

            /* temp1.z = len(VPpli)^2 */
            glmOPCODE(MUL, temp1, Z, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rVPpliLength, XXXX, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rVPpliLength, XXXX, gcSHADER_PRECISION_HIGH);
            ...
            glmOPCODE(RCP, ShaderControl->rAttenuation, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);
        }
    }
    while (gcvFALSE);
        ...
        else
        {
            ...
            */
            ...
            if (LightIndex < 0)
            {
                ...
                /* rSpot = 1 */
                glmOPCODE(MOV, ShaderControl->rSpot, X, gcSHADER_PRECISION_HIGH);
                    glmCONST(1);

                /* temp2 = 1 */
                glmOPCODE(MOV, temp6, X, gcSHADER_PRECISION_HIGH);
                    glmCONST(1);
            ...
            /* Negate VPli. */
            glmOPCODE(SUB, temp2, XYZ, gcSHADER_PRECISION_HIGH);
                glmCONST(0);
                glmTEMP(ShaderControl->rVPpli, XYZZ, gcSHADER_PRECISION_HIGH);
            ...
            glmOPCODE(MAX, temp4, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
                glmCONST(0);

            /* Get cos(Crli). */
            glmOPCODE(MOV, temp5, X, gcSHADER_PRECISION_HIGH);
...

            {
                ...
                */
                glmOPCODE(POW, ShaderControl->rSpot, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp4, XXXX, gcSHADER_PRECISION_HIGH);
            ...
            */
            ...
            if (Context->lightingStates.materialEnabled)
            {
                ...
                if (Context->aColorInfo.streamEnabled)
                {
                    ...
                    glmOPCODE(MUL, ShaderControl->rAmbient, XYZ, gcSHADER_PRECISION_HIGH);
                        glmATTRIBUTE(VS, aColor, XYZZ);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        glmOPCODE(MOV, ShaderControl->rAmbient, XYZ, gcSHADER_PRECISION_HIGH);
            ...
            */
            ...
            if (Context->lightingStates.materialEnabled)
            {
                ...
                if (Context->aColorInfo.streamEnabled)
                {
                    ...
                    glmOPCODE(MUL, ShaderControl->rDiffuse[OutputIndex], XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->rNdotVPpli[OutputIndex], XXXX, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                }
    ...
    )
{
    ...
    do
    {
        ...
        else
        {
            ...
            glmOPCODE(MOV, ShaderControl->rSpecular[OutputIndex], XYZ, gcSHADER_PRECISION_HIGH);
                glmCONST(0);
...

            {
                ...
                */
                ...
                if (glfISZERO(Context->lightingStates.Srm))
                {
                    glmALLOCATE_LOCAL_TEMP(temp);
                    ...
                    glmOPCODE(MUL, ShaderControl->rSpecular[OutputIndex], XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                ...
                else
                {
                    ...
                    */
                    ...
                    glmOPCODE(ADD, temp1, Z, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->rVPpli, ZZZZ, gcSHADER_PRECISION_HIGH);
                        glmCONST(1);

                    /* Normalize halfway vector. */
                    glmOPCODE(NORM, temp2, XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                    ...
                    glmOPCODE(POW, temp5, X, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp4, XXXX, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM(VS, uSrm, XXXX);

                    /* Multuply by Scm. */
                    glmOPCODE(MUL, temp6, XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp5, XXXX, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM(VS, uScm, XYZZ);

                    /* Multuply by Scli. */
                    glmOPCODE(MUL, ShaderControl->rSpecular[OutputIndex], XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp6, XYZZ, gcSHADER_PRECISION_HIGH);
                        glmLIGHTING_UNIFORM(VS, uScli, XYZZ, LightIndex);
                }
            }
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(MOV, tempOut, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rLighting[OutputIndex], XYZZ, gcSHADER_PRECISION_HIGH);
        /* Initialize with ambient component. */
        if (ShaderControl->rAmbient)
        {
            glmALLOCATE_TEMP(accumulator);
            glmOPCODE(MOV, accumulator, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rAmbient, XYZZ, gcSHADER_PRECISION_HIGH);
        }
        ...
        if (ShaderControl->rDiffuse[OutputIndex])
        {
            ...
            {
                glmOPCODE(MOV, temp, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(accumulator, XYZZ, gcSHADER_PRECISION_HIGH);
                glmOPCODE(ADD, accumulator, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->rDiffuse[OutputIndex], XYZZ, gcSHADER_PRECISION_HIGH);
            }
            else
            {
                glmALLOCATE_TEMP(accumulator);
        ...
        if (ShaderControl->rSpecular[OutputIndex])
        {
            if (accumulator)
            {
                ...
                glmOPCODE(ADD, accumulator, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->rSpecular[OutputIndex], XYZZ, gcSHADER_PRECISION_HIGH);
            }
            else
            {
                glmALLOCATE_TEMP(accumulator);
        ...
        if (ShaderControl->rSpot)
        {
            glmOPCODE(MOV, temp, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(accumulator, XYZZ, gcSHADER_PRECISION_HIGH);
            glmOPCODE(MUL, accumulator, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rSpot, XXXX, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
        }
        ...
        if (ShaderControl->rAttenuation)
        {
            glmOPCODE(MOV, temp, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(accumulator, XYZZ, gcSHADER_PRECISION_HIGH);
            glmOPCODE(MUL, accumulator, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(ShaderControl->rAttenuation, XXXX, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp, XYZZ, gcSHADER_PRECISION_HIGH);
        }

        /* Add to the final result. */
        glmOPCODE(ADD, ShaderControl->rLighting[OutputIndex], XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(tempOut, XYZZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(accumulator, XYZZ, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(MOV, ShaderControl->rLightIndex, X, gcSHADER_PRECISION_HIGH);
            glmCONST(LightIndex);
    ...
    do
    {
        ...
        if (gcmIS_SUCCESS(status))
        {
            for (i = 0; i < ShaderControl->outputCount; i++)
            {
                ...
                /* Clamp. */
                glmOPCODE(SAT, ShaderControl->vColor[i], XYZW, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->rLighting[i], XYZW, gcSHADER_PRECISION_HIGH);
            }
        }
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
            ...
            )
        {
            ...
            /* Move to the output. */
            glmOPCODE(MOV, temp, XYZW, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aColor, XYZW);

            /* Clamp. */
            glmOPCODE(SAT, ShaderControl->vColor[0], XYZW, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp, XYZW, gcSHADER_PRECISION_HIGH);
        }
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(MOV, ShaderControl->vEyePosition, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rVtxInEyeSpace, ZZZZ, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        */
        ...
        /* find 2 * depth bias */
        glmOPCODE(MUL, temp1, X, gcSHADER_PRECISION_HIGH);
            glmCONST(Context->hashKey.hashDepthBias);
            glmCONST(2.0);

        /* find 2 * depth bias * w */
        glmOPCODE( MUL, temp2, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->vPosition, WWWW, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp1, XXXX, gcSHADER_PRECISION_HIGH);

        /* Add Depth bias to the vPosition.z. */
        glmOPCODE(ADD, temp3, X, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->vPosition, ZZZZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);

        /* mov temp to position. */
        glmOPCODE(MOV, ShaderControl->vPosition, Z, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
    ...
    do
    {
        ...
        if (needReflection)
        {
            ...
            */
            ...
            else
            {
                ...
                glmOPCODE(NORM, unitNormal, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->rNrmInEyeSpace[0], XYZZ, gcSHADER_PRECISION_HIGH);
            }
            ...
            */
            ...
            /* Allocate temp to get u. */
            glmALLOCATE_TEMP(vPositionInEye);

            /* Allocate resources. */
            glmUSING_UNIFORM(uModelView);

            /* dp4 vPositionInEye.x, aPosition, uModelView[0] */
            glmOPCODE(DP4, vPositionInEye, X, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aPosition, XYZW);
                glmUNIFORM_STATIC(VS, uModelView, XYZW, 0);

            /* dp4 vPositionInEye.y, aPosition, uModelView[1] */
            glmOPCODE(DP4, vPositionInEye, Y, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aPosition, XYZW);
                glmUNIFORM_STATIC(VS, uModelView, XYZW, 1);

            /* dp4 vPositionInEye.z, aPosition, uModelView[2] */
            glmOPCODE(DP4, vPositionInEye, Z, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aPosition, XYZW);
                glmUNIFORM_STATIC(VS, uModelView, XYZW, 2);

            /* dp4 vPositionInEye.w, aPosition, uModelView[3] */
            glmOPCODE(DP4, vPositionInEye, W, gcSHADER_PRECISION_HIGH);
                glmATTRIBUTE(VS, aPosition, XYZW);
                glmUNIFORM_STATIC(VS, uModelView, XYZW, 3);

            /* Normalize the position vector. */
            glmOPCODE(NORM, unitPosition, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(vPositionInEye, XYZZ, gcSHADER_PRECISION_HIGH);
            ...
            */
            ...
            glmOPCODE(DP3, temp1, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(unitNormal, XYZZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(unitPosition, XYZZ, gcSHADER_PRECISION_HIGH);
            ...
            */
            ...
            glmOPCODE(MUL, temp3, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);
                glmCONST(2);
            ...
            */
            ...
            /* Compute reflection. */
            glmOPCODE(SUB, reflection, XYZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(unitPosition, XYZZ, gcSHADER_PRECISION_HIGH);
                glmTEMP(temp3, XYZZ, gcSHADER_PRECISION_HIGH);
        }
        ...
        for (i = 0; i < Context->texture.pixelSamplers; i++)
        {
            ...
            if (sampler->genEnable)
            {
                ...
                {
                    glmOPCODE(MOV, ShaderControl->vTexCoord[i], XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->rNrmInEyeSpace[0], XYZZ, gcSHADER_PRECISION_HIGH);
                }
                ...
                {
                    glmOPCODE(MOV, ShaderControl->vTexCoord[i], XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(reflection, XYZZ, gcSHADER_PRECISION_HIGH);
                }

                /* Set Q to 1.0. */
                glmOPCODE(MOV, ShaderControl->vTexCoord[i], W, gcSHADER_PRECISION_HIGH);
                    glmCONST(1);
                ...
                if (!stack->topMatrix->identity)
                {
                    ...
                    glmDEFINE_TEMP(temp9);
                    glmDEFINE_TEMP(temp10);
                    ...
                    glmALLOCATE_TEMP(ShaderControl->vTexCoord[i]);
                    glmALLOCATE_TEMP(temp9);
                    glmALLOCATE_TEMP(temp10);

                    /* vTexCoord[i].x = dp4(preXformTexCoord, uTexMatrix[i * 4 + 0]) */
                    glmOPCODE(DP4, ShaderControl->vTexCoord[i], X, gcSHADER_PRECISION_HIGH);
                        glmTEMP(preXformTexCoord, XYZW, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM_STATIC(VS, uTexMatrix, XYZW, i * 4 + 0);

                    /* vTexCoord[i].y = dp4(preXformTexCoord, uTexMatrix[i * 4 + 1]) */
                    glmOPCODE(DP4, ShaderControl->vTexCoord[i], Y, gcSHADER_PRECISION_HIGH);
                        glmTEMP(preXformTexCoord, XYZW, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM_STATIC(VS, uTexMatrix, XYZW, i * 4 + 1);

                    /* vTexCoord[i].z = dp4(preXformTexCoord, uTexMatrix[i * 4 + 2]) */
                    glmOPCODE(DP4, ShaderControl->vTexCoord[i], Z, gcSHADER_PRECISION_HIGH);
                        glmTEMP(preXformTexCoord, XYZW, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM_STATIC(VS, uTexMatrix, XYZW, i * 4 + 2);

                    /* vTexCoord[i].w = dp4(preXformTexCoord, uTexMatrix[i * 4 + 3]) */
                    glmOPCODE(DP4, ShaderControl->vTexCoord[i], W, gcSHADER_PRECISION_HIGH);
                        glmTEMP(preXformTexCoord, XYZW, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM_STATIC(VS, uTexMatrix, XYZW, i * 4 + 3);

                    /* temp9.x = 1 / vTexCoord[i].w */
                    glmOPCODE(RCP, temp9, X, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->vTexCoord[i], WWWW, gcSHADER_PRECISION_HIGH);

                    /* temp10.xyz = ShaderControl->vTexCoord[i].xyz */
                    glmOPCODE(MOV, temp10, XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->vTexCoord[i], XYZZ, gcSHADER_PRECISION_HIGH);

                    /* vTexCoord[i].xyz = vTexCoord[i].xyz / vTexCoord[i].www */
                    glmOPCODE(MUL, ShaderControl->vTexCoord[i], XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp10, XYZZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(temp9, XXXX, gcSHADER_PRECISION_HIGH);
                }
            }
            ...
            else if (Context->drawTexOESEnabled)
            {
                ...
                glmOPCODE(MOV, ShaderControl->vTexCoord[i], XY, gcSHADER_PRECISION_HIGH);
            ...
            else if (sampler->aTexCoordInfo.streamEnabled)
            {
                ...
                if (stack->topMatrix->identity)
                {
                    ...
                    else
                    {
                        ...
                        glmOPCODE(MOV, ShaderControl->vTexCoord[i], XYZW, gcSHADER_PRECISION_HIGH);
                ...
                else
                {
                    ...
                    if (sampler->coordType == gcSHADER_FLOAT_X2)
                    {
                        glmDEFINE_TEMP(temp9);
                        glmDEFINE_TEMP(temp10);
                        glmDEFINE_TEMP(temp11);

                        glmALLOCATE_TEMP(temp9);
                        glmALLOCATE_TEMP(temp10);
                        glmALLOCATE_TEMP(temp11);
                        ...
                        /* temp9.x = dp4(temp1, uTexMatrix[i * 4 + 3]) */
                        glmOPCODE(DP4, temp9, X, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp1, XYZW, gcSHADER_PRECISION_HIGH);
                            glmUNIFORM_STATIC(VS, uTexMatrix, XYZW, i * 4 + 3);

                        /* temp10.x = 1 / temp9.x */
                        glmOPCODE(RCP, temp10, X, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp9, X, gcSHADER_PRECISION_HIGH);

                        /* temp11.xy = ShaderControl->vTexCoord[i].xy */
                        glmOPCODE(MOV, temp11, XY, gcSHADER_PRECISION_HIGH);
                            glmTEMP(ShaderControl->vTexCoord[i], XYYY, gcSHADER_PRECISION_HIGH);

                        /* vTexCoord[i].xy = vTexCoord[i].xy / temp10.xx */
                        glmOPCODE(MUL, ShaderControl->vTexCoord[i], XY, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp11, XYYY, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp10, XXXX, gcSHADER_PRECISION_HIGH);
                    ...
                    else if (sampler->coordType == gcSHADER_FLOAT_X3)
                    {
                        ...
                        /* temp11.xyz = ShaderControl->vTexCoord[i].xyz */
                        glmOPCODE(MOV, temp11, XYZ, gcSHADER_PRECISION_HIGH);
                            glmTEMP(ShaderControl->vTexCoord[i], XYZZ, gcSHADER_PRECISION_HIGH);

                        /* vTexCoord[i].xyz = vTexCoord[i].xyz / temp10.xxx */
                        glmOPCODE(MUL, ShaderControl->vTexCoord[i], XYZ, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp11, XYZZ, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp10, XXXX, gcSHADER_PRECISION_HIGH);
                    }
    ...
    do
    {
        ...
        for (i = 0; i < glvMAX_CLIP_PLANES; i++)
        {
            if (Context->clipPlaneEnabled[i])
            {
                ...
                glmOPCODE(DP4, ShaderControl->vClipPlane[i], X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->rVtxInEyeSpace, XYZW, gcSHADER_PRECISION_HIGH);
