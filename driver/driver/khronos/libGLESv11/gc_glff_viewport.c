...
#define _GC_OBJ_ZONE gcdZONE_ES11_VIEWPORT
    ...
    )
{
    ...
    {
        top    = Context->drawHeight - top;
        bottom = Context->drawHeight - bottom;
    }
    ...
    )
{
        ...
        !Context->viewportStates.clippedOut)
    {
        ...
        if (Context->drawYInverted)
        {
            GLint temp;
            temp     = top;
            top    = Context->drawHeight - bottom;
            bottom = Context->drawHeight - temp;
        }
    ...
    case GL_MAX_VIEWPORT_DIMS:
        {
            ...
            dimensions[0] = (GLint)Context->maxWidth;
            dimensions[1] = (GLint)Context->maxHeight;
    ...
    )
{
    ...
    if(Context->recomputeViewport)
    {
        status = _ProgramViewport(Context);
        Context->recomputeViewport = gcvFALSE;
    }
    ...
    )
{
    glmENTER4(glmARGINT, X, glmARGINT, Y, glmARGINT, Width, glmARGINT, Height)
    {
        ...
        glfSetViewport(context, X, Y, Width, Height);
    ...
    )
{
    glmENTER4(glmARGINT, X, glmARGINT, Y, glmARGINT, Width, glmARGINT, Height)
    {
        ...
        glfSetScissor(context , X, Y, Width, Height);
    }
    glmLEAVE();
}
