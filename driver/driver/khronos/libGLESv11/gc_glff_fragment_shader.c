    ...
    )
{
    ...
    {
        if (!Context->aColorInfo.dirty && !Context->forceFlushPSUniforms && !Context->programDirty)
        {
            gcmFOOTER_NO();
            return gcvSTATUS_OK;
        }
    ...
    )
{
    ...
    for (i = 0; i < Context->texture.pixelSamplers; i++)
    {
        vector[0] =
        vector[1] =
        vector[2] = Context->texture.sampler[i].combColor.scale;
        vector[3] = Context->texture.sampler[i].combAlpha.scale;
        vector += 4;
    }
...
static gceSTATUS _Using_uFlipPointSprite(
    glsCONTEXT_PTR Context,
    glsFSCONTROL_PTR ShaderControl
    )
{
    ...
    status = glfUsingUniform(
        ...
        gcSHADER_FLOAT_X4,
        1,
        _Set_uFlipPointSprite,
        &Context->fsUniformDirty.uPointSpriteCoordDirty,
        &glmUNIFORM_WRAP(FS, uFlipPointSprite)
        );
    ...
    )
{
    ...
    gceSTATUS status;
    glsCONTEXT * context = GetCurrentContext();
    ...
    if (context == gcvNULL)
    {
        status = gcvSTATUS_CONTEXT_LOSSED;
        gcmFOOTER();
        return status;
    }
    ...
    )
{
    ...
    status = glfUsingVarying(
        ...
        1,
        Context->pointStates.spriteActive ? (POINTSPRITE_TEX_ATTRIBUTE) : gcvTRUE,
    ...
    )
{
    ...
    /* Set UCP varying as HighP to keep precision under Dual16 */
    status = glfUsingVarying(
        ShaderControl->i,
        vName[ClipPlane],
        gcSHADER_FLOAT_X1,
    ...
    )
{
    ...
    do
    {
        ...
        /* Clamp. */
        glmOPCODE(SAT, ShaderControl->oColor, XYZW, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->oPrevColor, XYZW, gcSHADER_PRECISION_HIGH);
        ...
        for (i = 0; i < glvMAX_CLIP_PLANES; i++)
        {
            if (Context->clipPlaneEnabled[i])
            {
                ...
                glmOPCODE_BRANCH(KILL, LESS, 0);
                    glmVARYING_INDEXED_PRECISION(FS, vClipPlane0, i, XXXX, gcSHADER_PRECISION_HIGH);
                    glmCONST(0);
            }
        }
    }
    ...
    )
{
    ...
    do
    {
            ...
            (!Context->drawClearRectEnabled))
        {
            ...
            /* Assume front face. */
            glmOPCODE(MOV, ShaderControl->rColor, XYZW, gcSHADER_PRECISION_HIGH);
        ...
        else if (((Context->lightingStates.lightingEnabled
        ...
        /* Copy the value. */
        glmOPCODE(MOV, ShaderControl->oColor, XYZW, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->rColor, XYZW, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    do
    {
        ...
        if (Source == glvTEXTURE)
        {
                ...
                Context->pointStates.spriteActive)
            {
                ...
                if (Context->pointStates.spriteActive)
                {
                    ...
                    glmALLOCATE_LOCAL_LABEL(lblFlip);
                    glmUSING_UNIFORM(uFlipPointSprite);

                    glmOPCODE(MOV, coord, XYZW, gcSHADER_PRECISION_HIGH);
                        glmVARYINGV_INDEXED(FS, vTexCoord0, SamplerNumber, sampler->coordSwizzle);

                    glmOPCODE_BRANCH(JMP, EQUAL, lblFlip);
                        glmUNIFORM(FS, uFlipPointSprite, XXXX);
                        glmCONST(0);

                    /* Flip Y coordinate. */
                    glmOPCODE(SUB, coord, Y, gcSHADER_PRECISION_HIGH);
                        glmCONST(1.0f);
                        glmVARYINGV_INDEXED(FS, vTexCoord0, SamplerNumber, sampler->coordSwizzle);
                    ...
                    /* Load texture. */
                    glmOPCODE(TEXLD, temp, XYZW, gcSHADER_PRECISION_HIGH);
                        glmUNIFORM_INDEXED(FS, uTexSampler0, SamplerNumber, XYZW);
                        glmTEMP(coord, XYYY, gcSHADER_PRECISION_HIGH);
                }
                else
                {
                    /* Allocate a temporary for the argument. */
                    ...
                    if((sampler->coordType == gcSHADER_FLOAT_X4) &&
                        (sampler->aTexCoordInfo.streamEnabled))
                    {
                        glmOPCODE(MOV, temp3, XYZW, gcSHADER_PRECISION_HIGH);
                            glmVARYINGV_INDEXED(FS, vTexCoord0, SamplerNumber, sampler->coordSwizzle);
                        glmOPCODE(RCP, temp1, X, gcSHADER_PRECISION_HIGH);
                            glmTEMP(temp3, WWWW, gcSHADER_PRECISION_HIGH);
        ...
        else if (Source == glvPREVIOUS)
        {
            ...
            if (ShaderControl->clampColor)
            {
                ...
                glmOPCODE(SAT, temp, XYZW, gcSHADER_PRECISION_HIGH);
                    glmTEMP(ShaderControl->oColor, XYZW, gcSHADER_PRECISION_HIGH);
            }
            else
            {
                /* Move to the argument register. */
    ...
    )
{
    ...
    do
    {
        ...
        else if (Operand == glvSRCCOLORINV)
        {
            ...
            /* temp = 1 - source. */
            glmOPCODE(SUB, temp, XYZW, gcSHADER_PRECISION_HIGH);
                glmCONST(1);
                glmTEMP(SourceRegister, XYZW, gcSHADER_PRECISION_HIGH);
        }
        ...
        else if (Operand == glvSRCALPHA)
        {
            ...
            /* temp2 = temp1.w */
            glmOPCODE(MOV, temp, XYZW, gcSHADER_PRECISION_HIGH);
                glmTEMP(SourceRegister, WWWW, gcSHADER_PRECISION_HIGH);
        }
    ...
    )
{
    ...
    do
    {
        glmOPCODEV(MOV, ShaderControl->oColor, CombineFlow->targetEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[0], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        glmOPCODEV(MUL, ShaderControl->oColor, CombineFlow->targetEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[0], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[1], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        glmOPCODEV(ADD, ShaderControl->oColor, CombineFlow->targetEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[0], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[1], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODEV(ADD, temp, CombineFlow->tempEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[0], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[1], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);

        glmOPCODEV(SUB, ShaderControl->oColor, CombineFlow->targetEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(temp, CombineFlow->tempSwizzle, gcSHADER_PRECISION_HIGH);
            glmCONST(0.5f);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        /* temp1 = Arg0 * Arg2. */
        glmOPCODEV(MUL, temp1, CombineFlow->tempEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[0], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[2], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);

        /* temp2 = Arg0 * Arg2 + Arg1. */
        glmOPCODEV(ADD, temp2, CombineFlow->tempEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(temp1, CombineFlow->tempSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[1], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);

        /* temp3 = Arg1 * Arg2 */
        glmOPCODEV(MUL, temp3, CombineFlow->tempEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[1], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(Arguments[2], CombineFlow->argSwizzle, gcSHADER_PRECISION_HIGH);

        /* Finally compute the result. */
        glmOPCODEV(SUB, ShaderControl->oColor, CombineFlow->targetEnable, gcSHADER_PRECISION_HIGH);
            glmTEMPV(temp2, CombineFlow->tempSwizzle, gcSHADER_PRECISION_HIGH);
            glmTEMPV(temp3, CombineFlow->tempSwizzle, gcSHADER_PRECISION_HIGH);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(SUB, temp1, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(Arguments[0], XYZZ, gcSHADER_PRECISION_HIGH);
            glmCONST(0.5f);

        /* temp2 = Arg1 - 0.5 */
        glmOPCODE(SUB, temp2, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(Arguments[1], XYZZ, gcSHADER_PRECISION_HIGH);
            glmCONST(0.5f);
    ...
    )
{
    ...
    do
    {
        ...
        /* oColor = dp3 * 4 */
        glmOPCODE(MUL, ShaderControl->oColor, XYZW, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
            glmCONST(4);
    }
    while (gcvFALSE);
    ...
    )
{
    ...
    do
    {
        ...
        gctBOOL doScale = (CombineCount == 1)
            ? ! glfISONE(Combine[0].scale)
            : (!glfISONE(Combine[0].scale) || !glfISONE(Combine[1].scale));
        ...
        for (i = 0; i < CombineCount; i++)
        {
            ...
            if (combine->combineFlow->targetEnable & TargetEnable)
            {
                ...
                {
                    glmOPCODE(MOV, ShaderControl->oColor, W, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->oPrevColor, WWWW, gcSHADER_PRECISION_HIGH);
                }
                else if (combine->combineFlow->targetEnable == gcSL_ENABLE_W)
                {
                    glmOPCODE(MOV, ShaderControl->oColor, XYZ, gcSHADER_PRECISION_HIGH);
                        glmTEMP(ShaderControl->oPrevColor, XYZZ, gcSHADER_PRECISION_HIGH);
                }
            }
#if gcmIS_DEBUG(gcdDEBUG_TRACE)
            else
    ...
    )
{
    ...
    do
    {
        ...
        */
        ...
        glmOPCODE(ABS, eyeDistance, X, gcSHADER_PRECISION_HIGH);
        ...
        */
        ...
        if (Context->fogStates.mode == glvLINEARFOG)
        {
            ...
            */
            ...
            /* _temp1.x = c * [(1 / (s - e))] */
            glmOPCODE(MUL, _temp1, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(eyeDistance, XXXX, gcSHADER_PRECISION_HIGH);
                glmUNIFORM(FS, uFogFactors, XXXX);

            /* _temp2.x = c * [(1 / (s - e))] + [e / (e - s) */
            glmOPCODE(ADD, _temp2, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(_temp1, XXXX, gcSHADER_PRECISION_HIGH);
                glmUNIFORM(FS, uFogFactors, YYYY);

            /* fogFactor = sat(_temp3.x) */
            glmOPCODE(SAT, fogFactor, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(_temp2, XXXX, gcSHADER_PRECISION_HIGH);
        }
        else
        {
            /*******************************************************************
            ...
            */
            ...
            else
            {
                ...
                glmOPCODE(MUL, _temp2, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(_temp1, XXXX, gcSHADER_PRECISION_HIGH);
                    glmTEMP(_temp1, XXXX, gcSHADER_PRECISION_HIGH);
            }

            /* _temp3.x = -_temp2.x */
            glmOPCODE(SUB, _temp3, X, gcSHADER_PRECISION_HIGH);
                glmCONST(0);
                glmTEMP(_temp2, XXXX, gcSHADER_PRECISION_HIGH);

            /* _temp4.x = exp(_temp3.x) */
            glmOPCODE(EXP, _temp4, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(_temp3, XXXX, gcSHADER_PRECISION_HIGH);

            /* fogFactor = sat(_temp4.x) */
            glmOPCODE(SAT, fogFactor, X, gcSHADER_PRECISION_HIGH);
                glmTEMP(_temp4, XXXX, gcSHADER_PRECISION_HIGH);
        }
        ...
        */
        ...
        glmOPCODE(MUL, temp1, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(fogFactor, XXXX, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->oPrevColor, XYZZ, gcSHADER_PRECISION_HIGH);

        /* temp2 = f * Cr + Cf. */
        glmOPCODE(ADD, temp2, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
        ...
        /* Finally compute the result. */
        glmOPCODE(SUB, ShaderControl->oColor, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(SUB, temp1, XY, gcSHADER_PRECISION_HIGH);
            glmVARYING(FS, vPosition, XYYY);
            glmVARYING(FS, vPointSmooth, XYYY);

        /* Zero out Z so that we can use DP3 on X and Y. */
        glmOPCODE(MOV, temp1, Z, gcSHADER_PRECISION_HIGH);
            glmCONST(0);

        /* Reset the resulting alpha. */
        glmOPCODE(MOV, temp2, X, gcSHADER_PRECISION_HIGH);
            glmCONST(0);
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (i == 0)
            {
                glmOPCODE(MOV, temp3, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                glmOPCODE(SUB, temp1, XY, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, XYYY, gcSHADER_PRECISION_HIGH);
                    glmCONST(0.5f);
            }
            ...
            else if (i == 1)
            {
                glmOPCODE(MOV, temp3, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                glmOPCODE(ADD, temp1, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
                    glmCONST(1);
            }
            ...
            else if (i == 2)
            {
                glmOPCODE(MOV, temp3, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                glmOPCODE(ADD, temp1, Y, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, YYYY, gcSHADER_PRECISION_HIGH);
                    glmCONST(1);
            }
            ...
            else
            {
                glmOPCODE(MOV, temp3, XYZ, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp1, XYZZ, gcSHADER_PRECISION_HIGH);
                glmOPCODE(SUB, temp1, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
                    glmCONST(1);
            }
...

            {
                ...
                glmOPCODE(MOV, temp3, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp2, XXXX, gcSHADER_PRECISION_HIGH);
                glmOPCODE(ADD, temp2, X, gcSHADER_PRECISION_HIGH);
                    glmTEMP(temp3, XXXX, gcSHADER_PRECISION_HIGH);
                    glmCONST(0.25f);
            }
    ...
    )
{
    ...
    do
    {
        ...
        glmOPCODE(MUL, ShaderControl->oColor, W, gcSHADER_PRECISION_HIGH);
            glmTEMP(ShaderControl->oPrevColor, WWWW, gcSHADER_PRECISION_HIGH);
    ...
    )
{
    ...
    do
    {
        ...
        /* Remove the fraction. */
        glmOPCODE(FLOOR, temp3, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp2, XYZZ, gcSHADER_PRECISION_HIGH);

        /* Convert back to 0..1 range. */
        glmOPCODE(MUL, ShaderControl->oColor, XYZ, gcSHADER_PRECISION_HIGH);
            glmTEMP(temp3, XYZZ, gcSHADER_PRECISION_HIGH);
