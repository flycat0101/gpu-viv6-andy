...
{
    GLenum result;
    gcmHEADER_ARG("Context=0x%x Function=0x%04x Value=%1.4f",
    ...
    do
    {
        ...
        do
        {
            ...
            GLfixed referencex = glmFLOAT2FIXED(Context->alphaStates.testReference);
    ...
    )
{
    ...
    do
    {
        ...
        /* Blend function from combine group. */
        Context->alphaStates.blendFuncSeparate = GL_FALSE;
    ...
    )
{
    ...
    do
    {
        ...
        /* Blend function from separate group. */
        Context->alphaStates.blendFuncSeparate = GL_TRUE;
    ...
    )
{
    ...
    do
    {
        ...
        gceBLEND_FUNCTION srcFunctionAlpha;
        gceBLEND_FUNCTION dstFunctionAlpha;
        gceBLEND_MODE blendModeRGB;
        gceBLEND_MODE blendModeAlpha;

        /* Flush alpha blending enabling. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_EnableBlending(
            Context->hw,
            Context->alphaStates.blendEnabled
            )));

        /* Get current blend function. */
        if (Context->alphaStates.blendFuncSeparate)
        {
            /* From separate group. */
            srcFunctionRGB =
                _BlendFunctionValues[Context->alphaStates.blendSrcFunctionRGB];

            srcFunctionAlpha =
                _BlendFunctionValues[Context->alphaStates.blendSrcFunctionAlpha];

            dstFunctionAlpha =
                _BlendFunctionValues[Context->alphaStates.blendDstFunctionAlpha];

            dstFunctionRGB =
                _BlendFunctionValues[Context->alphaStates.blendDstFunctionRGB];
        }
        else
        {
            /* From combined group. */
            srcFunctionRGB =
                _BlendFunctionValues[Context->alphaStates.blendSrcFunction];

            srcFunctionAlpha =
                (srcFunctionRGB == gcvBLEND_SOURCE_ALPHA_SATURATE)
                ? gcvBLEND_ONE
                : srcFunctionRGB;

            dstFunctionRGB = dstFunctionAlpha =
                _BlendFunctionValues[Context->alphaStates.blendDestFunction];
        }

        /* Flush blend function separate. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetBlendFunction(
            Context->hw,
            gcvBLEND_SOURCE,
            srcFunctionRGB,
            srcFunctionAlpha
        ...
        /* Get current blend equation. */
        blendModeRGB = _BlendModeValues[Context->alphaStates.blendModeRGB];
        blendModeAlpha = _BlendModeValues[Context->alphaStates.blendModeAlpha];

        /* Flush blend equation separate. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetBlendMode(
            Context->hw,
            blendModeRGB,
            blendModeAlpha
        ...
        /* Flush alpha test enabling. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetAlphaTest(
            Context->hw,
        ...
        do
        {
            ...
            /* Flush alpha test function and reference. */
            gcmERR_BREAK(gco3D_SetAlphaCompare(
                Context->hw,
                _AlphaTestValues[Context->alphaStates.testFunction]
                ));
        ...
        glmERR_BREAK(glmTRANSLATEHALSTATUS(status));
    }
    while (gcvFALSE);
    ...
    )
{
    glmENTER2(glmARGENUM, Function, glmARGFIXED, Reference)
    {
        ...
        reference = glmFIXED2FLOAT(Reference);
