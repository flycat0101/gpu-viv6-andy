    ...
    )
{
    ...
    gcmHEADER_ARG("Context=0x%x Plane=%d Equation=0x%x", Context, Plane, Equation);

    matrix = glfGetModelViewInverse4x4TransposedMatrix(Context);

    /* Set clip plane vector. */
    glfSetVector4(&Context->clipPlane[Plane], Equation);
...
/*******************************************************************************
**
**  glfFlushClipPlaneStates
**
**  Flush clip plane states.
**
**  INPUT:
**
**      Context
...
gceSTATUS glfFlushClipPlaneStates(
    glsCONTEXT_PTR Context
    )
{
    gcmHEADER_ARG("Context=0x%x", Context);
    ...
    )
{
    glmENTER2(glmARGENUM, Plane, glmARGPTR, Equation)
    {
        ...
        _SetClipPlane(context, plane, Equation);
    }
    glmLEAVE();
}
...
{
    glmENTER2(glmARGENUM, p, glmARGPTR, Equation)
    {
        GLint plane = p - GL_CLIP_PLANE0;

        gcmDUMP_API("${ES11 glClipPlanefIMG 0x%08X (0x%08x)", p, Equation);
        gcmDUMP_API_ARRAY(Equation, 4);
        gcmDUMP_API("$}");
    ...
    )
{
    glmENTER2(glmARGENUM, Plane, glmARGPTR, Equation)
    {
        ...
        /* Convert to float */
        equation[0] = glmFIXED2FLOAT(Equation[0]);
        equation[1] = glmFIXED2FLOAT(Equation[1]);
        equation[2] = glmFIXED2FLOAT(Equation[2]);
        equation[3] = glmFIXED2FLOAT(Equation[3]);
    ...
    )
{
    glmENTER2(glmARGENUM, p, glmARGPTR, Equation)
    {
        ...
        gcmDUMP_API("${ES11 glClipPlanexIMG 0x%08X (0x%08x)", p, Equation);
        gcmDUMP_API_ARRAY(Equation, 4);
        gcmDUMP_API("$}");
