...
#undef _GC_OBJ_ZONE
#endif
#define _GC_OBJ_ZONE    gcdZONE_ES11_PIXEL
    ...
    )
{
              ...
              glmARGENUM, Format, glmARGENUM, Type, glmARGPTR, Pixels)
    {
        ...
        {
            srcView.surf = glfGetFramebufferSurface(&context->frameBuffer->color);
        }
        else
        {
            srcView.surf = context->read;
        }
        ...
        rlvArgs.uArgs.v2.srcOrigin.y = context->drawYInverted
                                     ? (GLint)(context->drawHeight - Y - Height)
                                     : Y;
    ...
    if (dstView.surf != gcvNULL)
    {
        gcmVERIFY_OK(gcoSURF_Destroy(dstView.surf));
    }
}
