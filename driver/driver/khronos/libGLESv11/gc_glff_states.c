    ...
    )
{
    ...
    /* Set depth only when old value is different than new value */
    Context->depthStates.depthOnly = (enable == 0);
    ...
    )
{
    ...
    gcmHEADER_ARG("Context=0x%x ClearColor=0x%x", Context, ClearColor);

    /* Set with [0..1] clamping. */
    glfSetClampedVector4(&Context->clearColor, ClearColor);
...
**  glfFlushMiscStates
**
**  Flush current miscellaneous states.
**
**  INPUT:
**
**      Context
    ...
    )
{
    ...
    do
    {
        ...
        /* Flush logic operation. */
        glmERR_BREAK(_UpdateLogicOp(
            Context
            ));

        /* Compute color mask. */
        enable
            =  (gctUINT8) Context->colorMask[0]
            | ((gctUINT8) Context->colorMask[1] << 1)
            | ((gctUINT8) Context->colorMask[2] << 2)
            | ((gctUINT8) Context->colorMask[3] << 3);

        /* Flush color mask. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetColorWrite(
            Context->hw,
            enable
            )));

        /* Set dirty flag for depth only. */
        Context->depthStates.depthOnlyDirty = gcvTRUE;
        ...
        /* Flush clear color. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_SetClearColorFrac(
            Context->hw,
            clearColor[0],
            clearColor[1],
            clearColor[2],
        ...
        /* Flush dithering. */
        glmERR_BREAK(glmTRANSLATEHALSTATUS(gco3D_EnableDither(
            Context->hw,
            Context->dither
            )));
    ...
    )
{
              ...
              glmARGFIXED, Blue, glmARGFIXED, Alpha)
    {
        ...
        clearColor[0] = glmFIXED2FLOAT(Red);
        clearColor[1] = glmFIXED2FLOAT(Green);
        clearColor[2] = glmFIXED2FLOAT(Blue);
        clearColor[3] = glmFIXED2FLOAT(Alpha);

        glmERROR(_SetClearColor(context, clearColor));
    }
    glmLEAVE();
}
    ...
    )
{
    ...
    /* cosθ -sinθ    90D  x = -y  180D x = -x 270D x = y
       sinθ cosθ          y = x        y = -y      y = -x */
