    ...
    )
{
    ...
    {
        /* Cast the object. */
        glsRENDER_BUFFER_PTR object = (glsRENDER_BUFFER_PTR)(((glsNAMEDOBJECT_PTR)Object)->object);
    ...
    )
{
    ...
    do
    {
        ...
        case KHR_IMAGE_LINUX_DMA_BUF:
            ...
            {
                /* Get native buffer. */
                Attributes->nativeBuffer = Image->u.ANativeBuffer.nativeBuffer;
            }
        ...
        for (i = 0; i < Count; i++)
        {
            ...
            if (gcvNULL != context->frameBuffer)
            {
                if (context->frameBuffer->color.object == wrapper)
                {
                    ...
                    {
                        gcmVERIFY_OK(gco3D_UnsetTarget(
                            context->hw,
                            0,
                    ...
                    else
                    {
                        if (((glsRENDER_BUFFER_PTR)wrapper->object)->surface)
                        {
                            gcmVERIFY_OK(gco3D_UnsetTarget(
                                context->hw,
                                 0,
                                ((glsRENDER_BUFFER_PTR)wrapper->object)->surface
                                ));
                        }
                    }
