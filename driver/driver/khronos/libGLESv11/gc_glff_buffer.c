    ...
    )
{
    ...
    wrapper = (glsNAMEDOBJECT_PTR)Object;
    /* Cast the object. */
    ...
    )
{
    ...
    do
    {
        ...
        if (object->listIndexEven)
        {
            gcoINDEX_Destroy(object->listIndexEven);
            object->listIndexEven = gcvNULL;
        }

        if (object->listIndexOdd)
        {
            gcoINDEX_Destroy(object->listIndexOdd);
            object->listIndexOdd = gcvNULL;
        }
    ...
    )
{
    ...
    if (ArrayToElement)
    {
        if (Buffer->stream == gcvNULL || Buffer->copyDirty == gcvTRUE)
        {
            return status;
        }
        ...
        Buffer->copyDirty = gcvTRUE;
    }
    else
    {
        if (Buffer->index == gcvNULL || Buffer->copyDirty == gcvTRUE)
        {
            return status;
        }
    ...
    )
{
    ...
    do
    {
        ...
        object->listIndexEven = gcvNULL;
        object->listIndexOdd  = gcvNULL;
        object->patchDirty = gcvTRUE;
        object->copyDirty = gcvFALSE;
    }
    while (gcvFALSE);
    ...
    )
{
    glmENTER2(glmARGINT, Count, glmARGPTR, Buffers)
    {
        ...
        for (i = 0; i < Count; i++)
        {
            ...
            if (wrapper->bindCount != 0)
            {
                /* Set the flag to indicate the object is marked for delete */
                wrapper->flag |= __GL_OBJECT_IS_DELETED;
                break;
            }
    ...
    )
{
    glmENTER2(glmARGENUM, Target, glmARGUINT, Buffer)
    {
        ...
        if (preObj && (preObj != wrapper))
        {
            /* Cast the object. */
            glsBUFFER_PTR object = (glsBUFFER_PTR)preObj->object;
            ...
            if (GL_ARRAY_BUFFER == Target || object->stream)
            {
                ...
                if (Data != gcvNULL)
                {
                    ...
                    /*Patch for game */
                    {
                        ...
                        if(patchId == gcvPATCH_WATER2_CHUKONG)
                        {
                            gctUINT32 *dataPtr;
                            gctINT i  = 0;
                            dataPtr = (gctUINT32 *) Data;
                            if(Size == 0x13880)
                            {
                                for( i = 0; i < Size / 4; i++)
                                {
                                    if(dataPtr[i] == 0xFFAA8C00)
                                    {
                                        object->dataInvalid = gcvTRUE;
                                        break;
                                    }
                                }
                            }
        ...
        if (object->stream)
        {
            ...
            /*Patch for game*/
            {
                        ...
                        if(patchId == gcvPATCH_WATER2_CHUKONG)
                        {
                            ...
                            gctINT i  = 0;
                            dataPtr = (gctUINT32 *) Data;
                            if(object->size == 0x13880)
                            {
                                for( i = 0; i < Size / 4; i++)
                                {
                                    if(dataPtr[i] == 0xFFAA8C00) /*Nan float */
                                    {
                                        object->dataInvalid = gcvTRUE;
                                        break;
                                    }
    ...
    )
{
    ...
    glmENTER2(glmARGENUM, Target, glmARGENUM, Access)
    {
        ...
        if(Target == GL_ARRAY_BUFFER)
        {
            gcmERR_BREAK(gcoSTREAM_WaitFence(object->stream));
