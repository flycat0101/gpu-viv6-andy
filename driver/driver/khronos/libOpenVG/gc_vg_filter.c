...
#endif
{
    ...
    vgmENTERSUBAPI(_GetTileMode);
    {
        ...
        {
            result = _tilingMode[index];
        }
        else
        {
            result = (gceTILE_MODE) -1;
        }
    }
    vgmLEAVESUBAPI(_GetTileMode);
    return result;
}
    ...
    )
{
    ...
    vgmENTERSUBAPI(_ComponentToIndex);
    /* Convert to 0..255 range and round. */
    roundedValue = Value * 255.0f + 0.5f;
    /* Clamp the value. */
    clampedValue = (gctUINT8) gcmMIN(gcmMAX((gctINT) floor(roundedValue), 0), 255);

    vgmLEAVESUBAPI(_ComponentToIndex);
    /* Return result. */
    return clampedValue;
}
    ...
    )
{
    ...
    gctFLOAT resultValue;
    vgmENTERSUBAPI(_IndexToComponent);

    /* Convert back to float. */
    resultValue = ((gctFLOAT) Index) / 255.0f;

    vgmLEAVESUBAPI(_IndexToComponent);
    /* Return result. */
    return resultValue;
}
    ...
    )
{
    ...
    gctUINT8 index;
    gctUINT8 lookupValue;
    gctFLOAT resultValue;
    vgmENTERSUBAPI(_LookupComponent);

    /* Get the index value. */
    index = _ComponentToIndex(Context, Value);
    /* Lookup the 8-bit value. */
    lookupValue = Table[index];
    /* Convert back to float. */
    resultValue = _IndexToComponent(Context, lookupValue);

    vgmLEAVESUBAPI(_LookupComponent);
    /* Return result. */
    return resultValue;
}
...
{
    vgmENTERSUBAPI(_ConvertColor32);
    {
        /* Extract components. */
        ...
        Result[0] = _IndexToComponent(Context, red);
        Result[1] = _IndexToComponent(Context, green);
        Result[2] = _IndexToComponent(Context, blue);
        Result[3] = _IndexToComponent(Context, alpha);
    }
    vgmLEAVESUBAPI(_ConvertColor32);
}
    ...
    )
{
    ...
    vgmENTERSUBAPI(_NormalizeSource);
    ...
    do
    {
        ...
        vgsPIXELWALKER_Initialize(Context, &srcPixel, Source, 0, 0);
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_NormalizeSource);
    /* Return status. */
    return status;
}
    ...
    )
{
        ...
        (Y < 0) || (Y >= Height))
    {
        ...
        case gcvTILE_REPEAT:
            X = _Mod(Context, X, Width);
            Y = _Mod(Context, Y, Height);
        ...
        case gcvTILE_REFLECT:
            X = _Mod(Context, X, Width  * 2);
            Y = _Mod(Context, Y, Height * 2);
    ...
    )
{
    vgmENTERAPI(vgColorMatrix)
    {
        ...
        /* Initialize walkers. */
        vgsPIXELWALKER_Initialize(Context, &srcPixel, source, 0, 0);
        vgsPIXELWALKER_Initialize(Context, &trgPixel, target, 0, 0);
    ...
    )
{
    vgmENTERAPI(vgConvolve)
    {
        ...
        tilingMode = _GetTileMode(Context, TilingMode);
        if (tilingMode == (gceTILE_MODE) -1)
        {
            vgmERROR(VG_ILLEGAL_ARGUMENT_ERROR);
            break;
        }
    ...
    )
{
    vgmENTERAPI(vgGaussianBlur)
    {
        ...
        vgtFLOATVECTOR4 trgValue;
        vgtFLOATVECTOR4 fillColor;
        vgtFLOATVECTOR4 srcPixel = {0, 0, 0, 0};
        vgtFLOATVECTOR4 * tempBuffer;
        gctINT tmpWidth, tmpHeight;
    ...
    vgmENTERAPI(vgLookup)
    {
        ...
        for (y = 0; y < height; y++)
        {
            for (x = 0; x < width; x++)
            {
                ...
                /* Lookup channels. */
                trgValue[0] = _LookupComponent(Context, RedLUT,   srcValue[0]);
                trgValue[1] = _LookupComponent(Context, GreenLUT, srcValue[1]);
                trgValue[2] = _LookupComponent(Context, BlueLUT,  srcValue[2]);
                trgValue[3] = _LookupComponent(Context, AlphaLUT, srcValue[3]);
    ...
    vgmENTERAPI(vgLookupSingle)
    {
        ...
        for (y = 0; y < height; y++)
        {
            for (x = 0; x < width; x++)
            {
                ...
                {
                case VG_RED:
                    index = _ComponentToIndex(Context, srcValue[0]);
                    break;

                case VG_GREEN:
                    index = _ComponentToIndex(Context, srcValue[1]);
                    break;

                case VG_BLUE:
                    index = _ComponentToIndex(Context, srcValue[2]);
                    break;

                case VG_ALPHA:
                    index = _ComponentToIndex(Context, srcValue[3]);
                    break;
                ...
                _ConvertColor32(Context, LookupTable[index], trgValue);
