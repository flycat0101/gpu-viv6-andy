...
#define _GC_OBJ_ZONE                gcdZONE_VG_VGU
    ...
    )
{
    ...
    do
    {
        ...
        Destination->set(Context, Destination, X);
        Destination->set(Context, Destination, Y);
    ...
    )
{
    ...
    gctINT32 bottom;
    gctBOOL result;
    vgmENTERSUBAPI(vgfNormalizeCoordinates);
    ...
    vgmLEAVESUBAPI(vgfNormalizeCoordinates);
    ...
    )
{
    ...
    vgmENTERSUBAPI(vgfNormalizeCoordinatePairs);
    do
    {
    ...
    while(gcvFALSE);
    vgmLEAVESUBAPI( vgfNormalizeCoordinatePairs);
    /* Success. */
    return result;
}
    ...
    )
{
    ...
    vgmENTERAPI(vguLine)
    {
        ...
        /* Add path commands. */
        gcmERR_GOTO(_MOVE_TO_ABS(Context, &destination, X0, Y0));
        gcmERR_GOTO(_LINE_TO_ABS(Context, &destination, X1, Y1));
    ...
    )
{
    ...
    vgmENTERAPI(vguPolygon)
    {
        ...
        gcmERR_GOTO(_MOVE_TO_ABS(Context, &destination, points[0], points[1]));
        points += 2;

        /* Add polygon sides. */
        for (i = 1; i < Count; i++)
        {
            gcmERR_GOTO(_LINE_TO_ABS(Context, &destination, points[0], points[1]));
            points += 2;
        }
        ...
        if (Closed)
        {
            gcmERR_GOTO(_CLOSE_PATH(Context, &destination));
        }
    ...
    )
{
    ...
    vgmENTERAPI(vguRect)
    {
        ...
        /* Add the initial move. */
        gcmERR_GOTO(_MOVE_TO_ABS(Context, &destination, X, Y));

        /* Add rectangle sides. */
        gcmERR_GOTO(_HLINE_TO_REL(Context, &destination,  Width));
        gcmERR_GOTO(_VLINE_TO_REL(Context, &destination,  Height));
        gcmERR_GOTO(_HLINE_TO_REL(Context, &destination, -Width));
    ...
    )
{
    ...
    vgmENTERAPI(vguRoundRect)
    {
        ...
        /* Add the initial move. */
        gcmERR_GOTO(_MOVE_TO_ABS(Context, &destination, X + hRadius, Y));

        /* Add rectangle sides and corners. */
        gcmERR_GOTO(_HLINE_TO_REL(Context, &destination, width));
        gcmERR_GOTO(_SMALL_ARC(
            Context, &destination, gcvTRUE, gcvTRUE,
    ...
    )
{
    ...
    vgmENTERAPI(vguEllipse)
    {
        ...
        /* Add the initial move. */
        gcmERR_GOTO(_MOVE_TO_ABS(Context, &destination, CenterX + hRadius, CenterY));
    ...
    )
{
    ...
    vgmENTERAPI(vguArc)
    {
        ...
        if (angleExtent > 0)
        {
            ...
            {
                gcmERR_GOTO(_SMALL_ARC(
                    Context, &destination, gcvTRUE, gcvFALSE,
                    hRadius, vRadius, 0,
        ...
        else
        {
            ...
            {
                gcmERR_GOTO(_SMALL_ARC(
                    Context, &destination, gcvFALSE, gcvFALSE,
                    hRadius, vRadius, 0,
                    CenterX + (gctFLOAT) cos(angle) * hRadius,
                    CenterY + (gctFLOAT) sin(angle) * vRadius
                    ));
        ...
        {
            /* Draw 'pie'. */
            gcmERR_GOTO(_LINE_TO_ABS(Context, &destination, CenterX, CenterY));
    ...
    )
{
    ...
    vgmENTERAPI(vguComputeWarpQuadToSquare)
    {
        ...
        /* Invert the matrix. */
        if (!vgfInvertMatrix(Context, &projective, &warped))
        {
    ...
    )
{
    ...
    vgmENTERAPI(vguComputeWarpQuadToQuad)
    {
        ...
        /* Find the product. */
        vgfMultiplyMatrix3x3(Context, &s2q, &q2s, &product);
