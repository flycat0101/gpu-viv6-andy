...
{
    vgmENTERSUBAPI(_UpdateInternalColorRamp);
    /* Color ramp dirty? */
    if (Paint->intColorRampRecompute)
    {
        ...
        for (
            ...
            (srcRamp < srcRampLast) && (trgCount < vgvMAX_COLOR_RAMP_STOPS + 2);
            srcRamp += 1
            )
        {
    ...
    vgmLEAVESUBAPI(_UpdateInternalColorRamp);
}
...
{
    gceSTATUS status;
    vgmENTERSUBAPI(_UpdateColorRampSurface);
    do
    {
        ...
        if (Paint->colorRampSurfaceRecompute)
        {
            ...
            {
                gctFLOAT x0 = Paint->linearGradient[0];
                gctFLOAT y0 = Paint->linearGradient[1];
                gctFLOAT x1 = Paint->linearGradient[2];
                gctFLOAT y1 = Paint->linearGradient[3];
                r = (gctFLOAT)sqrt((x1-x0)*(x1-x0) + (y1-y0)*(y1-y0));
            ...
            for (i = 0; i < width; ++i)
            {
                ...
                *bits++ = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, color[3]);
                *bits++ = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, color[2]);
                *bits++ = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, color[1]);
                *bits++ = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, color[0]);
#else
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_UpdateColorRampSurface);
    /* Return status. */
    return status;
}
...
{
    gceSTATUS status;
    vgmENTERSUBAPI(_SetDefaultGradientColor);
    {
    gctUINT8 r, g, b, a;
    ...
    /* Convert the color. */
#if gcdGC355_PROFILER
    r =  gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth,  rampEntry->red);
    g =  gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth,  rampEntry->green);
    b =  gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth,  rampEntry->blue);
    a =  gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth,  rampEntry->alpha);
#else
    ...
    vgmLEAVESUBAPI(_SetDefaultGradientColor);
    /* Return status. */
    return status;
}
    ...
    )
{
    /* Assume success. */
    gceSTATUS status;
    vgmENTERSUBAPI(_UpdateColorPaint);
...
#endif
    vgmLEAVESUBAPI(_UpdateColorPaint);
    /* Return status. */
    return status;
}
    ...
    )
{
    /* Assume success. */
    gceSTATUS status = gcvSTATUS_OK;
    vgmENTERSUBAPI(_UpdateLinearGradientPaint);
    do
    {
        gctFLOAT zeroX, oneX;
        gctFLOAT zeroY, oneY;
        ...
        _UpdateInternalColorRamp(Context, Paint);
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_UpdateLinearGradientPaint);
    /* Return status. */
    return status;
}
    ...
    )
{
    /* Assume success. */
    gceSTATUS status = gcvSTATUS_OK;
    vgmENTERSUBAPI(_UpdateRadialGradientPaint);
    do
    {
        gctFLOAT radius;
        ...
        if (radius > 0)
        {
            ...
            if (Paint->rgRecompute || vgvFORCE_PAINT_RECOMPUTE)
            {
                ...
                if (fx*fx + fy*fy > radius2)
                {
                    ...
                    */
                    gctFLOAT fr = (gctFLOAT)sqrt(fx*fx + fy*fy);
                    fx = radius * fx / fr * 0.9f;
                    fy = radius * fy / fr * 0.9f;
                    focalX = Paint->radialGradient[0] + fx;
                    focalY = Paint->radialGradient[1] + fy;
                }
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_UpdateRadialGradientPaint);
    /* Return status. */
    return status;
}
    ...
    )
{
    /* Assume success. */
    gceSTATUS status = gcvSTATUS_OK;
    vgmENTERSUBAPI(_UpdatePatternPaint);
    do
    {
        ...
        if (Paint->pattern != gcvNULL)
        {
            ...
            gcmERR_BREAK(gcoVG_SetPattern(
                ...
                Context->saveLayerTreeDepth,
                Context->varTreeDepth,
                Paint->pattern->size.width,
                Paint->pattern->size.height,
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_UpdatePatternPaint);
    /* Return status. */
    return status;
}
...
{
    gceSTATUS status = gcvSTATUS_OK;
    vgmENTERSUBAPI(_PaintDestructor);
    do
    {
        /* Cast the object. */
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_PaintDestructor);
    /* Return status. */
    return status;
}
...
#if gcdGC355_PROFILER
    byteColor[0] = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, floarColor[0]);
    byteColor[1] = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, floarColor[1]);
    byteColor[2] = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, floarColor[2]);
    byteColor[3] = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, floarColor[3]);
#else
    ...
    )
{
    ...
    vgmENTERSUBAPI(vgfReferencePaint);
    do
    {
...
ErrorHandler:
    ...
    vgmLEAVESUBAPI(vgfReferencePaint);
    /* Return status. */
    return status;
}
...
{
    gceSTATUS status;
    vgmENTERSUBAPI(vgfUpdatePaint);
    do
    {
        /* Update the matrix. */
        if (!isRenderToMask)
        {
            gcmERR_BREAK(Matrix->update(Context));
        }
        else
        {
    ...
    )
{
    ...
    {
    vgmENTERSUBAPI(vgfPaintMatrixChanged);
    /* Invalidate paint. */
    Paint->lgRecompute  = gcvTRUE;
    Paint->rgRecompute  = gcvTRUE;
    Paint->patRecompute = gcvTRUE;
    ...
    vgmLEAVESUBAPI(vgfPaintMatrixChanged);
    }
}
    ...
    )
{
    ...
    gcmASSERT(SurfaceToPaint != gcvNULL);
    {
    vgmENTERSUBAPI(vgfPaintMatrixVerifyCurrent);
    /* Not the same? */
    ...
    vgmLEAVESUBAPI(vgfPaintMatrixVerifyCurrent);
    }
}
    ...
    )
{
    ...
    vgmENTERAPI(vgPaintPattern)
    {
        ...
        else
        {
            ...
            /* Set the size. */
            paint->patternWidth  = (gctFLOAT) pattern->size.width;
            paint->patternHeight = (gctFLOAT) pattern->size.height;
