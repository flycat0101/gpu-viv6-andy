    ...
    )
{
    ...
    vgmENTERSUBAPI(_FreePathResources);
    do
    {
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_FreePathResources);
    /* Return status. */
    return status;
}
...
{
    gceSTATUS status = gcvSTATUS_OK;
    vgmENTERSUBAPI(_PathDestructor);
    do
    {
        /* Cast the object. */
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_PathDestructor);
    return status;
}
...
{
    gceSTATUS status;
    vgmENTERSUBAPI(_WaitForPathIdle);
    do
    {
        ...
        gcmERR_BREAK(gcoHAL_WaitCompletion(Context->hal,
                Context->vg, Context->TreeDepth,
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_WaitForPathIdle);
    /* Return status. */
    return status;
}
    ...
    )
{
    ...
    vgmENTERSUBAPI(vgfReferencePath);
    ...
    do
    {
        ...
        if (*Path == gcvNULL)
        {
            ...
            /* Reset the object. */
            gcoOS_ZeroMemory(path, sizeof(vgsPATH));
...
ErrorHandler:
    ...
    vgmLEAVESUBAPI(vgfReferencePath);
    /* Return status. */
    return status;
}
    ...
    )
{
    ...
    vgmENTERSUBAPI(vgfFreePathDataCallback);
    ...
    Path->stroke      = gcvNULL;
    vgmLEAVESUBAPI(vgfFreePathDataCallback);
    ...
    )
{
    ...
    do
    {
         ...
         * TODO: Should be optimized by cache the result.
        */
...
{
    gctUINT result;
    vgmENTERSUBAPI(vgfGetPathDataSize);
    {
        /* Path data sizes. */
        ...
        gcmASSERT((Type >= 0) && ((gctSIZE_T) Type < gcmCOUNTOF(_datatypeSize)));
        result = _datatypeSize[Type];
    }
    vgmLEAVESUBAPI(vgfGetPathDataSize);
    /* Return the size. */
    return result;
}
...
{
    gctUINT result;
    vgmENTERSUBAPI(vgfGetSegmentDataCount);
    {
    ...
    /* Make sure we are within range. */
    gcmASSERT(Command < gcmCOUNTOF(_dataCount));
    result = _dataCount[Command];
    }
    vgmLEAVESUBAPI(vgfGetSegmentDataCount);
    /* Return the size. */
    return result;
}
...
#if gcdMOVG
        path->boundsDirty = gcvTRUE;
        path->UpdateBoundsDirty = gcvTRUE;
#endif
    ...
    )
{
    vgmENTERAPI(vgClearPath)
    {
        ...
        path->numSegments = 0;
        path->numCoords   = 0;
        gcoOS_MemFill(&path->coords, 0, sizeof(path->coords));
    ...
    )
{
...
#endif
    {
        ...
        */
        ...
        vgfGetAppendArray(Context, &appendSegment, &appendCount);
        ...
        */
        ...
        while (gcvTRUE)
        {
            ...
            status = appendSegment[command] (Context, &destination, &source);
            ...
            status = vgsPATHWALKER_NextSegment(Context, &source);

            /* Finished? */
            if (status == gcvSTATUS_NO_MORE_DATA)
            {
                /* Close the buffer. */
                status = vgsPATHWALKER_DoneWriting(Context, &destination);
        ...
        {
            /* Free path data. */
            vgsPATHWALKER_Rollback(Context, &destination);
        }

#if gcdMOVG
        dstPath->boundsDirty = gcvTRUE;
        dstPath->UpdateBoundsDirty = gcvTRUE;
#endif
    ...
    )
{
...
#endif
    {
        ...
        */
        ...
        datatypeSize = vgfGetPathDataSize(Context, dstPath->halDataType);
        ...
        */
        ...
        vgfGetImportArray(Context, &importSegment, &importCount);
        ...
        */
        ...
        while (gcvTRUE)
        {
            ...
            status = importSegment[command] (Context, &destination, &source);
            ...
            /* Finished? */
            if (currentSegment == (gctUINT) NumSegments)
            {
                /* Close the buffer. */
                status = vgsPATHWALKER_DoneWriting(Context, &destination);
...
**  vgPathNativeVIV
**
**  This function provide an entry to directly access to the data memory of a path.
**  User can directly write path data (segments & coordinates) into this memory
**  to optimize performance and memory usage.
**  Usage: App first call this function with proper arguments, then it can get the
**         access to the memory. Then app writes its data to this memory with given
**         rules.
**  Note: It has strictly limitations to access this memory since it's the internal
**        representation of a path. Improper data written to the memory may lead to
**        unexpected HW behavior.
*/
VG_API_CALL void* VG_API_ENTRY vgPathNativeVIV(
    VGPath Path,
    VGint  Size,
    VGint  NumSegments,
    VGint  NumCoords
    )
{
    ...
    vgmENTERAPI(vgPathNativeVIV)
    {
        do
        {
            path = (vgsPATH_PTR)Path;

            /* Open access to the storage. */
            ...
            /* Set the buffer type. */
            pathData->data.dataType = path->halDataType;

            /* Set the owner. */
            pathData->path = path;
            pathData->numSegments = NumSegments;
            pathData->numCoords   = NumCoords;

            /* Determine the beginning of the command buffer. */
            path->head = pathData;
            result = (void*)( (gctUINT8_PTR) buffer + buffer->bufferOffset + path->pathInfo.reservedForHead);

            /* buffer ready. Finialize. */
            buffer->offset += Size;
            vgsPATHSTORAGE_Close(Context, Context->pathStorage, pathData);
            gcoVG_FinalizePath(Context->vg, &pathData->data);
        }
        while (gcvFALSE);
    }
    vgmLEAVEAPI(vgPathNativeVIV);
    ...
    )
{
...
#endif
    {
        ...
        */
        ...
        if (StartSegment == 0)
        {
            vgfGetModifyArray(Context, &handler, &entryCount);
        }
        else
        {
            vgfGetSkipArray(Context, &handler, &entryCount);
        }
        ...
        */
        ...
        while (gcvTRUE)
        {
            ...
            status = handler[command] (Context, &destination, &source);
            ...
            /* Skipping finished? */
            if (currentSegment == (gctUINT) StartSegment)
            {
            ...
            {
                /* Advance to the next segment. */
                gcmVERIFY_OK(vgsPATHWALKER_NextSegment(Context, &destination));
            ...
            {
                /* Get the modification function array. */
                vgfGetUpdateArray(Context, &handler, &entryCount);
            }

            /* Advance to the next segment. */
            status = vgsPATHWALKER_NextSegment(Context, &destination);
    ...
    )
{
...
#endif
    {
        ...
        */
        ...
        vgfGetTransformArray(Context, &transformSegment, &transformCount);
        ...
        */
        ...
        while (gcvTRUE)
        {
            ...
            status = transformSegment[command] (
                Context, &destination, &source, matrix
                );
    ...
    )
{
...
#endif
    {
        ...
        */
        ...
        vgfGetNormalizationArray(Context, &normalizeSegment, &normalizeCount);
        vgfGetInterpolationArray(Context, &interpolateSegment, &interpolateCount);
        ...
        */
        ...
        while (gcvTRUE)
        {
            ...
            normalizeSegment[start.command] (
                Context, &start, startCoordinates, &startGroup, &startCommand
                );

            /* Normalize end path segment for iterpolation. */
            normalizeSegment[end.command] (
                Context, &end, endCoordinates, &endGroup, &endCommand
                );
            ...
            gcmVERIFY_OK(vgsPATHWALKER_NextSegment(Context, &start));
            gcmVERIFY_OK(vgsPATHWALKER_NextSegment(Context, &end));
        }
...
#endif
    {
        ...
        if (success)
        {
            ...
            vgfMultiplyVector3ByMatrix3x3(
                Context, lt, vgmGETMATRIX(&Context->pathUserToSurface), lt
                );

            vgfMultiplyVector3ByMatrix3x3(
                Context, rt, vgmGETMATRIX(&Context->pathUserToSurface), rt
                );

            vgfMultiplyVector3ByMatrix3x3(
                Context, lb, vgmGETMATRIX(&Context->pathUserToSurface), lb
                );

            vgfMultiplyVector3ByMatrix3x3(
                Context, rb, vgmGETMATRIX(&Context->pathUserToSurface), rb
                );
