...
#define vgvENABLE_FAST_FILTER           0
...
#define vgmENTERAPI(FunctionName) \
    ...
    { \
        ...
        Context->saveLayerTreeDepth = Context->TreeDepth; \
        if(Context->enableGetAPITimes) \
        { \
            Context->varTreeDepth = Context->saveLayerTreeDepth; \
            if ((Context->TreeDepth > 0) && (Context->varTreeDepth > 0)) \
            { \
                Context->varTreeDepth = Context->saveLayerTreeDepth - 1; \
            } \
...
#define vgmLEAVEAPI(FunctionName) \
    ...
    while (gcvFALSE); \
    \
    if((Context != gcvNULL) && (Context->enableGetAPITimes)) \
    { \
        gcoOS_GetTime(&endTimeusec); \
        deltaValue = endTimeusec - startTimeusec; \
        if ((Context->TreeDepth > 0) && (Context->saveLayerTreeDepth > 0)) \
        { \
            Context->varTreeDepth = Context->saveLayerTreeDepth; \
        \
        } \
        _WriteAPITimeInfo(Context, #FunctionName, deltaValue); \
    } \
...
#define vgmENTERSUBAPI(FunctionName) \
    ...
    { \
        vgmDEBUGENTRY(Context, #FunctionName); \
        \
        if(Context->enableGetAPITimes && (Context->TreeDepth > 0)) \
        { \
            Context->saveLayerTreeDepth = Context->varTreeDepth; \
            if ((Context->varTreeDepth > 0) && !(Context->saveLayerTreeDepth & 0x80000000)) \
            { \
                gcoOS_GetTime(&startTimeusec); \
            } \
            Context->varTreeDepth = Context->saveLayerTreeDepth - 1; \
...
#define vgmLEAVESUBAPI(FunctionName) \
     } \
    while (gcvFALSE); \
    if((Context != gcvNULL) && (Context->enableGetAPITimes) && (Context->TreeDepth > 0)) \
    { \
        gcoOS_GetTime(&endTimeusec); \
        deltaValue = endTimeusec - startTimeusec; \
        if ((Context->saveLayerTreeDepth > 0) && !(Context->saveLayerTreeDepth & 0x80000000)) \
        { \
            Context->varTreeDepth = Context->saveLayerTreeDepth; \
            _WriteAPITimeInfo(Context, #FunctionName, deltaValue); \
        \
        } \
        if (Context->TreeDepth != Context->saveLayerTreeDepth) \
        { \
...
#define vgvMAX_SCISSOR_RECTS            256

/* VG_MAX_DASH_COUNT */
#define vgvMAX_DASH_COUNT               256
...
#define vgvMAX_COLOR_RAMP_STOPS         256
