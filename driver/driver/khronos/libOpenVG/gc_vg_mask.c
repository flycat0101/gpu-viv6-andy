    ...
    )
{
    ...
    vgmENTERSUBAPI(_MaskDestructor);
    do
    {
        /* Cast the object. */
    ...
    while (gcvFALSE);
    vgmLEAVESUBAPI(_MaskDestructor);
    ...
    )
{
    ...
    do
    {
        ...
        width  = Context->maskImage->size.width;
        height = Context->maskImage->size.height;
    ...
    )
{
    ...
    vgmENTERSUBAPI(vgfReferenceMask);
    do
    {
...
ErrorHandler:
    ...
    vgmLEAVESUBAPI(vgfReferenceMask);
    /* Return status. */
    return status;
}
    ...
    )
{
    vgmENTERAPI(vgFillMaskLayer)
    {
        ...
        byteColor[3] = gcoVG_PackColorComponent(Context->vg, Context->TreeDepth, Context->saveLayerTreeDepth, Context->varTreeDepth, Value);
#else
