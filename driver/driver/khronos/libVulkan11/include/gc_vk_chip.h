...
typedef struct __vkChipFuncTableRec
{
    ...
    VkResult (*DrawDirect)(VkCommandBuffer cmdBuf, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance);
    VkResult (*DrawIndexedDirect)(VkCommandBuffer commandBuffer, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex, int32_t vertexOffset, uint32_t firstInstance);
    ...
    VkResult (*Dispatch)(VkCommandBuffer commandBuffer, uint32_t baseX, uint32_t baseY, uint32_t baseZ, uint32_t x, uint32_t y, uint32_t z);
    ...
    VkBool32 (*tweakCopy)(VkCommandBuffer cmdBuf,VkImage srcImage, VkBuffer destBuffer);
    VkResult (*getQueryResult)(VkDevice device, VkQueryPool queryPool, uint32_t queryIndex, uint64_t *retValue);
    VkResult (*copyQueryResult)(VkCommandBuffer commandBuffer, VkQueryPool queryPool, VkBuffer dstBuffer, uint32_t queryIndex, VkDeviceSize dstOffset, VkDeviceSize dstStride, VkQueryResultFlags flags);
} __vkChipFuncTable;
