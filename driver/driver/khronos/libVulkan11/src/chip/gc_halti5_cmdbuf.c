    ...
    )
{
    ...
    /* Check if the group count of any dimension is zero, if so, just skip this draw. */
    if (x == 0 || y == 0 || z == 0)
    {
        __VK_DEBUG_PRINT(__VK_DBG_LEVEL_WARNING, "The group count is(%d, %d, %d), just skip it.\n", x, y, z);
        return result;
    }
