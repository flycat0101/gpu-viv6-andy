    ...
    )
{
    ...
    if(Program->patchIndex)
    {
        if(cl_patch_array[Program->patchIndex].handleKernel && cl_patch_array[Program->patchIndex].handleKernel(kernel, kernel->name))
        {
            vscCompileParams.cfg.ctx.appNameId = cl_patch_array[Program->patchIndex].patchID;
        }
    }
    else
    {
