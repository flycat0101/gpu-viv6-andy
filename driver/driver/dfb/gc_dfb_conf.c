...
#define GDCF_FILLTRAPEZOID (VIVANTE_GAL_DRAWING_FLAGS)
...
#define FILLTRAPEZOID_NAME     "filltrapezoid"
...
#define BLIT_SOURCE2_NAME      "source2"
                 ...
                 GALConfig *config )
{
...
#if (DIRECTFB_MAJOR_VERSION >= 1) && (DIRECTFB_MINOR_VERSION >= 6) && (DIRECTFB_MINOR_VERSION < 7)
    strcpy( config->blit_flags[7].name, BLIT_SOURCE2_NAME );
    config->blit_flags[7].value = DSBLIT_SOURCE2;
#endif
    ...
    strcpy( config->primitive_settings[GAL_CONF_FILLTRAPEZOID].name, FILLTRAPEZOID_NAME );
    config->primitive_settings[GAL_CONF_FILLTRAPEZOID].flag_num      = config->draw_flag_num;
    config->primitive_settings[GAL_CONF_FILLTRAPEZOID].render_flags  = config->draw_flags;
    ...
    if (!vdev->hw_2d_pe20) {
        config->primitive_settings[GAL_CONF_FILLTRAPEZOID].none   = false;
    }
    else {
        config->primitive_settings[GAL_CONF_FILLTRAPEZOID].none   = true;
    }
    config->primitive_settings[GAL_CONF_FILLTRAPEZOID].flags      = GDCF_FILLTRAPEZOID;
