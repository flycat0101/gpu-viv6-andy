    ...
    )
{
    ...
    gcmONERROR(gco2D_SetClipping(Context->engine, &framebuffer->res));

    if (Context->dumpCompose)
    {
        LOGD("COMPOSE: target=0x%08x", target->physical);
    }
    ...
    */
    ...
    if (Context->swapArea != NULL)
    {
        ...
        gcmONERROR(_CopySwapRect(Context));
    }
...
#   endif
    ...
    */
    ...
    gcmONERROR(_ClearWormHole(Context));
    ...
    */
    ...
    if (Context->multiSourceBlt == gcvFALSE)
    {
        ...
        if (Context->hasDim)
        {
            ...
            for (gctUINT i = 0; i < Context->layerCount; i++)
            {
                ...
                &&  (Context->layers[i].yuv || Context->layers[i].stretch)
                )
                {
            ...
            while (area != NULL)
            {
                ...
                for (gctUINT32 i = 0; i < Context->layerCount; i++)
                {
                    ...
                    for (gctUINT32 j = (i + 1); j < Context->layerCount; j++)
                    {
                        ...
                        {
                            gctUINT32 d = 0xFF - (Context->layers[j].color32 >> 24);
        ...
        for (gctUINT32 i = 0; i < Context->layerCount; i++)
        {
            ...
            for (gctUINT32 j = 0; j < layer->clipCount; j++)
            {
                ...
                clipRect.left   = gcmMAX(framebuffer->res.left,   rect->left);
                clipRect.top    = gcmMAX(framebuffer->res.top,    rect->top);
                clipRect.right  = gcmMIN(framebuffer->res.right,  rect->right);
                clipRect.bottom = gcmMIN(framebuffer->res.bottom, rect->bottom);
#endif
    ...
    for (gctUINT start = 0; start < Context->layerCount; start++)
    {
        ...
        for (barrier = start; barrier < Context->layerCount; barrier++)
        {
            ...
            {
                if (layer->stretch || layer->yuv)
                {
                    /* Do not support stretch or yuv layer. */
                    break;
                }
            ...
            if (layer->compositionType != HWC_DIM)
            {
                gctINT res   = framebuffer->res.right * framebuffer->res.bottom;
            ...
            if ((barrier == Context->layerCount - 1)
            &&  layer->opaque
            )
            {
        ...
        while (area != NULL)
        {
            ...
            for (gctUINT32 i = start; i < barrier; i++)
            {
                ...
                else
                {
                    ...
                    gcmONERROR(_Blit(Context, i, dim, &clipRect));
                }
            }
            ...
            else if (sourceCount == 1)
            {
                /* Use single-source blit instead. */
                gcmONERROR(
                    _Blit(Context, sourceIndices[0], sourceDims[0], &clipRect));
            }
    ...
    )
{
    ...
    while (area != NULL)
    {
        ...
        if ((rectCount == 16) || ((area == NULL) && (rectCount > 0)))
        {
            if (Context->dumpCompose)
            {
                LOGD("  WORMHOLE: (%08x)", framebuffer->target->physical);
    ...
    if (layer->source != gcvNULL)
    {
        if (layer->filter)
        {
            ...
            case gcvSURF_90_DEGREE:
                ...
                dstRect.left   = framebuffer->res.bottom - layer->dstRect.bottom;
                dstRect.top    = layer->dstRect.left;
                dstRect.right  = framebuffer->res.bottom - layer->dstRect.top;
                dstRect.bottom = layer->dstRect.right;

                subRect.left   = framebuffer->res.bottom - ClipRect->bottom;
                subRect.top    = ClipRect->left;
                subRect.right  = framebuffer->res.bottom - ClipRect->top;
            ...
            case gcvSURF_180_DEGREE:
                ...
                dstRect.left   = framebuffer->res.right  - layer->dstRect.right;
                dstRect.top    = framebuffer->res.bottom - layer->dstRect.bottom;
                dstRect.right  = framebuffer->res.right  - layer->dstRect.left;
                dstRect.bottom = framebuffer->res.bottom - layer->dstRect.top;

                subRect.left   = framebuffer->res.right  - ClipRect->right;
                subRect.top    = framebuffer->res.bottom - ClipRect->bottom;
                subRect.right  = framebuffer->res.right  - ClipRect->left;
                subRect.bottom = framebuffer->res.bottom - ClipRect->top;
                break;
            ...
            case gcvSURF_270_DEGREE:
                ...
                dstRect.top    = framebuffer->res.right - layer->dstRect.right;
                dstRect.right  = layer->dstRect.bottom;
                dstRect.bottom = framebuffer->res.right - layer->dstRect.left;

                subRect.left   = ClipRect->top;
                subRect.top    = framebuffer->res.right - ClipRect->right;
                subRect.right  = ClipRect->bottom;
                subRect.bottom = framebuffer->res.right - ClipRect->left;
    ...
    )
{
    ...
    if (!Context->msSourceMirror)
    {
        hwcLayer * layer = &Context->layers[Indices[0]];
    ...
    {
        gceSURF_ROTATION rot0 = Context->layers[Indices[0]].rotation;
    ...
    */
    ...
    for (gctUINT32 j = 0; j < Count; j++)
    {
        ...
        else
        {
            ...
            gcmONERROR(gco2D_SetROP(Context->engine, 0xF0, 0xF0));
