    ...
    IN struct copybit_region_t const * Clip)
{
    ...
    gc_native_handle_t * dsthnd = gc_native_handle_get(Dest->handle);
    gc_native_handle_t * srchnd = gc_native_handle_get(Source->handle);

    LOGV("Blit from Source hnd=%p, to Dest hnd=%p", srchnd, dsthnd);

    if (private_handle_t::validate(Dest->handle) < 0)
    {
    ...
    do
    {
        ...
        perpixelAlpha = _HasAlpha(siFormat) &&
                 (dsth->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) &&
                 !(srch->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER);
