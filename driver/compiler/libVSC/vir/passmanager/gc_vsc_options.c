    ...
    )
{
    ...
    VSC_OPTN_DUAL16Options_SetKernelName(options, gcvNULL);
}
    ...
    )
{
    while (str[0] == ':')
    {
        ...
        else if (gcvSTATUS_OK == gcoOS_StrNCmp(str,"kernel:", sizeof("kernel:")-1))
        {
            gctSTRING kernelName = gcvNULL;
            gctPOINTER  pointer = gcvNULL;
            gctUINT32 len;
            str += sizeof("kernel:") -1;
            len = _VSC_OPTN_GetSubOptionLength(str);
            gcoOS_Allocate(gcvNULL, (len+1), &pointer);
            kernelName = pointer;
            gcoOS_StrCopySafe(kernelName, (len+1), str);
            VSC_OPTN_DUAL16Options_SetKernelName(options, kernelName);
            str += len;
        }
        else if (gcvSTATUS_OK == gcoOS_StrNCmp(str, "trace:", sizeof("trace:")-1))
        {
