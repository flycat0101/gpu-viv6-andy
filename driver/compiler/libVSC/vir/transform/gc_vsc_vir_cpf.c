                    ...
                    BB_GET_END_INST(psuccBasicBlk) != gcvNULL)
                {
                       ...
                       )
                    {
                        ...
                        if (!_VSC_CPF_InWorkList(pCPF, psuccBasicBlk) &&
                            /* if loopinfo is created, add SuccBB only if bb is in a loop or the predBB are all visited */
                            (pCurrentLoopInfo != NULL || (pLoopOpts && (VIR_LoopOpts_GetLoopInfoMgr(pLoopOpts) == gcvNULL)) ||
                             _VSC_CPF_CheckPredsAreVisited(pCPF, psuccBasicBlk)))
                        {
