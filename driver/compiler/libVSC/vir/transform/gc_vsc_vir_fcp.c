                            ...
                            (VIR_GetTypeFlag(ty1) & VIR_TYFLAG_ISINTEGER))
                        {
                            ...
                            else
                            {
                                ...
                                */
                                ...
                                errCode = vscVIR_AddNewDef(pDuInfo, pNewInsertedInst, newDstRegNo, 1,
                                                           VIR_ENABLE_XYZW,
                                                           VIR_HALF_CHANNEL_MASK_FULL,
                                                           gcvNULL, gcvNULL);
    ...
    )
{
        ...
        VIR_OPCODE_isVX(VIR_Inst_GetOpcode(usage_inst_of_icast)))
    {
        ...
        /* replace def_vx_inst's dest and update usage */
        {
            ...
            /* add the use of vx_inst_src0 to the def of icast_src0 */
            {
                    ...
                    IS_VALID_DIRECT_HNODE_PAIR(&def_inst_set_pair); def_inst_set_pair = vscHTBLIterator_DirectNext(&def_inst_set_iter))
                {
                    ...
                    {
                        errCode = vscVIR_AddNewUsageToDef(du_info, def_inst, vx_inst, vx_inst_src0, gcvFALSE,
        ...
        {
            errCode = vscVIR_DeleteDef(du_info, icast, icast_dest_info.u1.virRegInfo.virReg,
    ...
    )
{
        ...
        VIR_OPCODE_isVX(VIR_Inst_GetOpcode(def_vx_inst)))
    {
         ...
         */
        {
            ...
            {
                errCode = vscVIR_DeleteDef(du_info, def_vx_inst, vx_inst_dest_info.u1.virRegInfo.virReg,
            ...
            errCode = vscVIR_AddNewDef(du_info, vx_inst, icast_dest_info.u1.virRegInfo.virReg, 1,
                                       icast_enable,
                                       VIR_HALF_CHANNEL_MASK_FULL,
                                       gcvNULL, gcvNULL);
            ON_ERROR(errCode, "Failed to add new def.");
            ...
            for (channel = VIR_CHANNEL_X; channel < VIR_CHANNEL_NUM; channel ++)
            {
                ...
                {
                    errCode = vscVIR_AddNewUsageToDef(du_info,
                                                      vx_inst,
                    ...
                    src2Swizzle != VIR_SWIZZLE_XYZW)
                {
                        ...
                        VIR_Symbol_GetUniformKind(src2Sym) == VIR_UNIFORM_CONST_BORDER_VALUE)
                    {
                        ...
                        if (pMovConstBorderInst != gcvNULL)
                        {
                            ...
                            errCode = vscVIR_AddNewUsageToDef(pDuInfo, pMovConstBorderInst, pInst,
                                                              pSrc2Opnd,
                    ...
                    /* update the usage of new temp */
                    errCode = vscVIR_AddNewUsageToDef(pDuInfo, pNewInst, pInst,
                                                      pSrc2Opnd,
