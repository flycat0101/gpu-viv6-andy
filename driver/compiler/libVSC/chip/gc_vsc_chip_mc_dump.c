...
#define OPERAND_SEC_COLUMN_SIZE  12
                          ...
                          VSC_DUMPER* pDumper)
{
    ...
    if (pInstCtrl->bPacked)
    {
        vscDumper_PrintStrSafe(pDumper, ".pack");
    }
    ...
    {
        if (pInstCtrl->u.bNeedRestartPrim)
        {
                         ...
                         VSC_DUMPER* pDumper)
{
    ...
    vscDumper_PrintStrSafe(pDumper, "%c%u", bAddrReg ? 'a' : 'r', pDst->regNo);
