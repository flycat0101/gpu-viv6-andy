              ...
              ppoTOKEN    Token)
{
    ...
    cloCOMPILER_Dump(PP->compiler, clvDUMP_PREPROCESSOR, "<Token ");

    if(local->poolString == PP->keyword->newline)
    {
        cloCOMPILER_Dump(PP->compiler, clvDUMP_PREPROCESSOR, " poolString=\"NewLine\">");
    }
    else
    {
        cloCOMPILER_Dump(PP->compiler, clvDUMP_PREPROCESSOR, " poolString=\"%s\">",
            local->poolString);
    }
