...
#else
    lResult = RegOpenKeyEx(hRootKey, (LPCWSTR) partialKey, 0, KEY_READ, &hTopKey);
#endif
...
#else
      for (index = 0; RegEnumKeyEx(hTopKey, index, (LPWSTR) keyName, &size, NULL,
          NULL, NULL, NULL) == ERROR_SUCCESS; index++)
#endif
      {
...
#else
        lResult = RegOpenKeyEx(hTopKey, (LPCWSTR) bestName, 0, KEY_READ, &hKey);
#endif
        if (lResult == ERROR_SUCCESS) {
#if !defined(UNDER_CE)
...
#else
          lResult = RegQueryValueEx(hKey, (LPCWSTR) valueName, NULL, &valueType,
            (LPBYTE)value, &valueSize);
#endif
          if (lResult == ERROR_SUCCESS)
            returnValue = true;
...
#else
    lResult = RegOpenKeyEx(hRootKey, (LPCWSTR) subKey, 0, KEY_READ, &hKey);
#endif
    if (lResult == ERROR_SUCCESS) {
#if !defined(UNDER_CE)
      lResult = RegQueryValueEx(hKey, valueName, NULL, &valueType,
...
static bool getVisualStudioDir(std::string &path) {
  ...
  char* vcinstalldir = gcvNULL;
  gcoOS_GetEnv(gcvNULL, "VCINSTALLDIR", &vcinstalldir);
  if(vcinstalldir) {
  ...
  if (hasVCDir && vsIDEInstallDir[0]) {
    ...
    // Try the environment.
    char* vs100comntools = gcvNULL;
    char* vs90comntools = gcvNULL;
    char* vs80comntools = gcvNULL;
    char* vscomntools = NULL;
    gcoOS_GetEnv(gcvNULL, "VS100COMNTOOLS", &vs100comntools);
    gcoOS_GetEnv(gcvNULL, "VS90COMNTOOLS", &vs90comntools);
    gcoOS_GetEnv(gcvNULL, "VS80COMNTOOLS", &vs80comntools);
