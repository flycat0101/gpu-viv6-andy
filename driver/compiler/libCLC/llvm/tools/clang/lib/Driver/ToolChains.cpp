...
Tool &Darwin::SelectTool(const Compilation &C, const JobAction &JA) const {
  ...
  if (!T) {
    ...
    case Action::BindArchClass:
      assert(0 && "Invalid tool kind.");break;
...
void DarwinClang::AddLinkSearchPathArgs(const ArgList &Args,
  ...
  switch (getTriple().getArch()) {
  default:
    assert(0 && "Invalid Darwin arch!");break;
...
void Darwin::AddDeploymentTarget(DerivedArgList &Args) const {
  ...
  if (OSXVersion && iPhoneVersion) {
    ...
    // environment defines.
    char *OSXTarget = gcvNULL;
    char *iPhoneOSTarget = gcvNULL;
    gcoOS_GetEnv(gcvNULL, "MACOSX_DEPLOYMENT_TARGET", &OSXTarget);
    gcoOS_GetEnv(gcvNULL, "IPHONEOS_DEPLOYMENT_TARGET", &iPhoneOSTarget);
  ...
  setTarget(iPhoneVersion ? true : false, Major, Minor, Micro);
}
...
bool Darwin::UseDwarfDebugFlags() const {
  char *S = gcvNULL;
  gcoOS_GetEnv(gcvNULL, "RC_DEBUG_OPTIONS", &S);
  if (S)
    return S[0] != '\0';
  return false;
}
