...
raw_fd_ostream::raw_fd_ostream(const char *Filename, std::string &ErrorInfo,
  ...
  ErrorInfo.clear();
#else
  ErrorInfo.erase(ErrorInfo.begin(), ErrorInfo.end());
#endif
...
#else
  gceSTATUS status;
  gceFILE_MODE mode = gcvFILE_CREATE;
  if (Flags & F_Append)
      mode = gcvFILE_APPEND;
...
#else
    if (ShouldClose)
      if (gcmIS_ERROR(gcoOS_Close(gcvNULL, (gctFILE)FD))) {
          error_detected();
        }
#endif
  }
...
#else
  if (gcmIS_ERROR(gcoOS_Write(gcvNULL, (gctFILE)FD, Size, Ptr))) {
          error_detected();
        }
#endif
}
...
uint64_t raw_fd_ostream::seek(uint64_t off) {
  ...
  if (pos != off)
    error_detected();
#else
  if (gcmIS_ERROR(gcoOS_Seek(gcvNULL, (gctFILE)FD, off, gcvFILE_SEEK_SET))) {
      error_detected();
  }
  pos = off;
#endif
