...
%token<token>		T_ATTRIBUTE T_CONST T_BOOL T_FLOAT T_INT
					...
					T_ISAMPLER2D T_ISAMPLERCUBE T_ISAMPLERCUBEARRAY T_ISAMPLER3D T_ISAMPLER2DARRAY
					T_USAMPLER2D T_USAMPLERCUBE T_USAMPLERCUBEARRAY T_USAMPLER3D T_USAMPLER2DARRAY
					...
					T_IMAGECUBE T_IIMAGECUBE T_UIMAGECUBE T_IMAGECUBEARRAY T_IIMAGECUBEARRAY T_UIMAGECUBEARRAY
                    T_IMAGEBUFFER T_IIMAGEBUFFER T_UIMAGEBUFFER
					T_GEN_SAMPLER  T_GEN_ISAMPLER T_GEN_USAMPLER
					T_ATOMIC_UINT
					T_BUFFER T_SHARED
...
%token<token>		T_IDENTIFIER T_TYPE_NAME T_FLOATCONSTANT T_INTCONSTANT T_BOOLCONSTANT T_UINTCONSTANT
					T_FIELD_SELECTION
					...
					T_SUB_ASSIGN T_LENGTH_METHOD

%token<token>		T_INVARIANT T_HIGH_PRECISION T_MEDIUM_PRECISION T_LOW_PRECISION T_PRECISION T_PRECISE

%token<token>		T_COHERENT T_VOLATILE T_RESTRICT T_READONLY T_WRITEONLY

/* types */
%type<token>		'(' ')' '[' ']' '{' '}' '.' ',' ':' '=' ';' '!'
					'-' '~' '+' '*' '/' '%' '<' '>' '|' '^' '&' '?'

%type<token>		function_identifier unary_operator assignment_operator
					precision_qualifier type_qualifier parameter_storage_qualifier
					single_parameter_qualifier parameter_qualifiers
					parameter_type_qualifier type_qualifiers
					layout_qualifier layout_qualifier_id layout_qualifier_id_list
					auxiliary_qualifier interpolation_qualifier storage_qualifier
					memory_access_qualifier

%token<token>		T_VIV_ASM T_ASM_OPND_BRACKET
...
%type<expr>			variable_identifier primary_expression postfix_expression integer_expression
					...
					condition conditionopt viv_asm_opnd

%type<asmOpcode>	viv_asm_opcode

%type<declOrDeclList>	init_declarator_list single_declaration

%type<dataType>		fully_specified_type type_specifier struct_specifier type_specifier_nonarray
			opaque_type_specifier default_precision_type_specifier
			interface_block_member_declarator_list

%type<fieldDeclList>	struct_declarator_list array_length_list

%type<fieldDecl>	struct_declarator

%type<fieldDecl>	interface_block_member_declarator

%type<blockName>	interface_block_decl
...
%type<statement>	declaration statement statement_no_new_scope simple_statement
					...
					interface_block viv_asm_statement
...
primary_expression :
	...
	| T_UINTCONSTANT
		{ $$ = slParseUintConstant(Compiler, &$1); }
	| T_FLOATCONSTANT
...
postfix_expression :
	...
	| postfix_expression '.' T_LENGTH_METHOD
		{ $$ = slParseLengthMethodExpr(Compiler, $1); }
...
viv_asm_statement :
	viv_asm_opnd_list ')' ';'
		{ $$ = slParseAsmAsStatement(Compiler, $1); }
	;

viv_asm_opnd_list :
	viv_asm_opcode
		{ $$ = slParseAsmOpcode(Compiler, &$1); }
	| viv_asm_opnd_list ',' viv_asm_opnd
		{ $$ = slParseAsmOperand(Compiler, $1, $3); }
	;
...
viv_asm_opnd :
	...
	| assignment_expression T_ASM_OPND_BRACKET viv_asm_modifier_list '>'
		{ $$ = slParseAsmAppendOperandModifiers(Compiler, $1, &$3); }
	;

viv_asm_modifier_list :
	viv_asm_modifier
		{ $$ = slParseAsmAppendModifier(Compiler, gcvNULL, &$1); }
	| viv_asm_modifier_list ',' viv_asm_modifier
		{ $$ = slParseAsmAppendModifier(Compiler, &$1, &$3); }
	;

viv_asm_modifier :
	T_IDENTIFIER ':' T_IDENTIFIER
		{ $$ = slParseAsmModifier(Compiler, &$1, &$3); }
	;

viv_asm_opcode :
	T_VIV_ASM '(' T_IDENTIFIER
		{ $$ = slParseAsmCreateOpcode(Compiler, &$3, gcvNULL); }
	| T_VIV_ASM '(' T_IDENTIFIER T_ASM_OPND_BRACKET viv_asm_modifier_list '>'
		{ $$ = slParseAsmCreateOpcode(Compiler, &$3, &$5); }
	;
...
array_length_list :
    '[' ']' '[' ']'
        { $$ = slParseArrayLengthList(Compiler, gcvNULL, gcvNULL, gcvNULL); }
    | '[' ']' '[' constant_expression ']'
        { $$ = slParseArrayLengthList(Compiler, gcvNULL, gcvNULL, $4); }
	| '[' constant_expression ']' '[' ']'
        { $$ = slParseArrayLengthList(Compiler, gcvNULL, $2, gcvNULL); }
	| '[' constant_expression ']' '[' constant_expression ']'
        { $$ = slParseArrayLengthList(Compiler, gcvNULL, $2, $5); }
    | array_length_list '[' ']'
        { $$ = slParseArrayLengthList2(Compiler, $1, gcvNULL, gcvNULL); }
    | array_length_list '[' constant_expression ']'
        { $$ = slParseArrayLengthList2(Compiler, $1, $3, gcvNULL); }
    ;
...
declaration :
	...
	| T_PRECISION precision_qualifier default_precision_type_specifier ';'
		{ $$ = slParseDefaultPrecisionQualifier(Compiler, &$1, &$2, $3); }
	| interface_block
        { $$ = $1; }
	| type_qualifiers ';'
		{ $$ = slParseQualifierAsStatement(Compiler, &$1); }
	;
...
parameter_declarator :
	...
	| type_specifier T_IDENTIFIER array_length_list
		{ $$ = slParseArrayListParameterDecl(Compiler, $1, &$2, $3); }
	;

parameter_declaration :
	parameter_declarator
		{ $$ = slParseQualifiedParameterDecl(Compiler, gcvNULL, $1); }
	| parameter_type_specifier
		{ $$ = slParseQualifiedParameterDecl(Compiler, gcvNULL, $1); }
	| parameter_qualifiers parameter_declarator
		{ $$ = slParseQualifiedParameterDecl(Compiler, &$1, $2); }
	| parameter_qualifiers parameter_type_specifier
		{ $$ = slParseQualifiedParameterDecl(Compiler, &$1, $2); }
	;

parameter_storage_qualifier :
	T_IN
		{ $$ = $1; }
	| T_OUT
		{ $$ = $1; }
...
parameter_qualifiers:
	single_parameter_qualifier
		{ $$ = $1; }
	| parameter_qualifiers single_parameter_qualifier
		{ $$ = slMergeParameterQualifiers(Compiler, &$1, &$2); }
	;
...
init_declarator_list :
	...
	| init_declarator_list ',' T_IDENTIFIER '[' ']'
		{ $$ = slParseArrayVariableDecl2(Compiler, $1, &$3, gcvNULL); }
	| init_declarator_list ',' T_IDENTIFIER array_length_list
		{ $$ = slParseArrayListVariableDecl2(Compiler, $1, &$3, $4); }
	| init_declarator_list ',' T_IDENTIFIER '=' initializer
		{ $$ = slParseVariableDeclWithInitializer2(Compiler, $1, &$3, $5); }
	| init_declarator_list ',' T_IDENTIFIER '[' ']' '=' initializer
		{ $$ = slParseArrayVariableDeclWithInitializer2(Compiler, $1, &$3, gcvNULL, $7); }
	| init_declarator_list ',' T_IDENTIFIER '[' constant_expression ']' '=' initializer
		{ $$ = slParseArrayVariableDeclWithInitializer2(Compiler, $1, &$3, $5, $8); }
	| init_declarator_list ',' T_IDENTIFIER array_length_list '=' initializer
		{ $$ = slParseArrayListVariableDeclWithInitializer2(Compiler, $1, &$3, $4, $6); }
	;
...
single_declaration :
	...
	| fully_specified_type T_IDENTIFIER '[' ']'
		{ $$ = slParseArrayVariableDecl(Compiler, $1, &$2, gcvNULL); }
	| fully_specified_type T_IDENTIFIER array_length_list
		{ $$ = slParseArrayListVariableDecl(Compiler, $1, &$2, $3); }
	| fully_specified_type T_IDENTIFIER '[' ']' '=' initializer
		{ $$ = slParseArrayVariableDeclWithInitializer(Compiler, $1, &$2, gcvNULL, $6); }
	| fully_specified_type T_IDENTIFIER '[' constant_expression ']' '=' initializer
		{ $$ = slParseArrayVariableDeclWithInitializer(Compiler, $1, &$2, $4, $7); }
	| fully_specified_type T_IDENTIFIER '=' initializer
		{ $$ = slParseVariableDeclWithInitializer(Compiler, $1, &$2, $4); }
	| type_qualifiers T_IDENTIFIER		/* Vertex only. */
		{ $$ = slParseInvariantOrPreciseDecl(Compiler, &$1, &$2); }
	| fully_specified_type T_IDENTIFIER array_length_list '=' initializer
		{ $$ = slParseArrayListVariableDeclWithInitializer(Compiler, $1, &$2, $3, $5); }
	;
...
fully_specified_type :
	type_specifier
		{ $$ = slParseFullySpecifiedType(Compiler, gcvNULL, $1); }
...
type_qualifiers :
	type_qualifier
		{ $$ = slMergeTypeQualifiers(Compiler, gcvNULL, &$1); }
	| type_qualifiers type_qualifier
		{ $$ = slMergeTypeQualifiers(Compiler, &$1, &$2); }
	;
...
layout_qualifier :
	T_LAYOUT '(' layout_qualifier_id_list
		{
		  sloCOMPILER_SetScannerState(Compiler, slvSCANNER_NORMAL);
		}
	')'
		{
		   $$ = slParseLayoutQualifier(Compiler, &$3);
		}
	| T_LAYOUT '(' error ')'
		{
		  sloCOMPILER_SetScannerState(Compiler, slvSCANNER_NORMAL);
...
layout_qualifier_id_list :
	...
	| layout_qualifier_id_list ',' layout_qualifier_id
		{ $$ = slParseAddLayoutId(Compiler, &$1, &$3); }
	;

layout_qualifier_id :
	T_IDENTIFIER
		{ $$ = slParseLayoutId(Compiler, &$1, gcvNULL); }
	| T_IDENTIFIER '=' T_INTCONSTANT
		{ $$ = slParseLayoutId(Compiler, &$1, &$3) ; }
	| T_IDENTIFIER '=' T_UINTCONSTANT
		{ $$ = slParseLayoutId(Compiler, &$1, &$3) ; }
	;
...
storage_qualifier :
	...
	| T_IN
		{ $$ = $1; }
	| T_PATCH T_IN
		{ $$ = slParseCheckStorage(Compiler, $1, $2); }
	| T_OUT
		{ $$ = $1; }
	| T_PATCH T_OUT
		{ $$ = slParseCheckStorage(Compiler, $1, $2); }
...
type_specifier :
	...
	| type_specifier_nonarray '[' ']'
	  { $$ = slParseArrayDataType(Compiler, $1, gcvNULL); }
	| type_specifier_nonarray '[' constant_expression ']'
	  { $$ = slParseArrayDataType(Compiler, $1, $3); }
	| type_specifier_nonarray array_length_list
	  { $$ = slParseArrayListDataType(Compiler, $1, $2); }
	;
...
opaque_type_specifier :
	...
	| T_SAMPLER2DSHADOW
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLER2DSHADOW); }
	| T_SAMPLER2DARRAYSHADOW
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLER2DARRAYSHADOW); }
	| T_SAMPLERCUBESHADOW
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLERCUBESHADOW); }
	| T_SAMPLERCUBEARRAYSHADOW
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLERCUBEARRAYSHADOW); }
	| T_ISAMPLER2D
	...
	| T_SAMPLER2DMS
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLER2DMS); }
	| T_ISAMPLER2DMS
		{ $$ = slParseNonStructType(Compiler, &$1, T_ISAMPLER2DMS); }
	| T_USAMPLER2DMS
		{ $$ = slParseNonStructType(Compiler, &$1, T_USAMPLER2DMS); }
	| T_SAMPLER2DMSARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLER2DMSARRAY); }
	| T_ISAMPLER2DMSARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_ISAMPLER2DMSARRAY); }
	| T_USAMPLER2DMSARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_USAMPLER2DMSARRAY); }
	| T_SAMPLERCUBEARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLERCUBEARRAY); }
	| T_ISAMPLERCUBEARRAY
	...
	| T_IIMAGE2D
		{ $$ = slParseNonStructType(Compiler, &$1, T_IIMAGE2D); }
	| T_UIMAGE2D
		{ $$ = slParseNonStructType(Compiler, &$1, T_UIMAGE2D); }
	| T_IMAGE2DARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_IMAGE2DARRAY); }
	| T_IIMAGE2DARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_IIMAGE2DARRAY); }
	| T_UIMAGE2DARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_UIMAGE2DARRAY); }
	| T_IMAGE3D
		{ $$ = slParseNonStructType(Compiler, &$1, T_IMAGE3D); }
	| T_IIMAGE3D
		{ $$ = slParseNonStructType(Compiler, &$1, T_IIMAGE3D); }
	| T_UIMAGE3D
		{ $$ = slParseNonStructType(Compiler, &$1, T_UIMAGE3D); }
	| T_IMAGECUBE
		{ $$ = slParseNonStructType(Compiler, &$1, T_IMAGECUBE); }
	| T_IMAGECUBEARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_IMAGECUBEARRAY); }
	| T_IIMAGECUBE
		{ $$ = slParseNonStructType(Compiler, &$1, T_IIMAGECUBE); }
	| T_IIMAGECUBEARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_IIMAGECUBEARRAY); }
	| T_UIMAGECUBE
		{ $$ = slParseNonStructType(Compiler, &$1, T_UIMAGECUBE); }
	| T_UIMAGECUBEARRAY
		{ $$ = slParseNonStructType(Compiler, &$1, T_UIMAGECUBEARRAY); }
	| T_ATOMIC_UINT
		{ $$ = slParseNonStructType(Compiler, &$1, T_ATOMIC_UINT); }
    | T_SAMPLERBUFFER
		{ $$ = slParseNonStructType(Compiler, &$1, T_SAMPLERBUFFER); }
    | T_ISAMPLERBUFFER
		{ $$ = slParseNonStructType(Compiler, &$1, T_ISAMPLERBUFFER); }
    | T_USAMPLERBUFFER
		{ $$ = slParseNonStructType(Compiler, &$1, T_USAMPLERBUFFER); }
    | T_IMAGEBUFFER
		{ $$ = slParseNonStructType(Compiler, &$1, T_IMAGEBUFFER); }
    | T_IIMAGEBUFFER
		{ $$ = slParseNonStructType(Compiler, &$1, T_IIMAGEBUFFER); }
    | T_UIMAGEBUFFER
		{ $$ = slParseNonStructType(Compiler, &$1, T_UIMAGEBUFFER); }
	;
...
default_precision_type_specifier :
	T_FLOAT
		{ $$ = slParseNonStructType(Compiler, &$1, T_FLOAT); }
	| T_INT
		{ $$ = slParseNonStructType(Compiler, &$1, T_INT); }
...
struct_declaration :
	...
	| precision_qualifier type_specifier struct_declarator_list ';'
		{
		   $2->qualifiers.precision = $1.u.qualifiers.precision;
		   slsQUALIFIERS_SET_FLAG(&($2->qualifiers), slvQUALIFIERS_FLAG_PRECISION);
		   slParseTypeSpecifiedFieldDeclList(Compiler, $2, $3);
		}
	;
...
struct_declarator :
	...
	| T_IDENTIFIER array_length_list
		{ $$ = slParseFieldListDecl(Compiler, &$1, $2, gcvFALSE); }
	;

interface_block :
	interface_block_decl ';'
		{ $$ = slParseInterfaceBlock(Compiler, $1, gcvNULL, gcvNULL, gcvTRUE); }
	| interface_block_decl T_IDENTIFIER ';'
		{ $$ = slParseInterfaceBlock(Compiler, $1, &$2, gcvNULL, gcvTRUE); }
	| interface_block_decl T_IDENTIFIER '[' constant_expression ']' ';'
		{ $$ = slParseInterfaceBlock(Compiler, $1, &$2, $4, gcvTRUE); }
	| interface_block_decl T_IDENTIFIER '[' ']' ';'
		{ $$ = slParseInterfaceBlockImplicitArrayLength(Compiler, $1, &$2); }
	;
...
interface_block_decl :
	...
	| type_qualifiers T_IDENTIFIER '{'
        ...
        error '}'
        {
            yyclearin;
            yyerrok;
            $$ = slParseInterfaceBlockDeclEnd(Compiler, gcvNULL, gcvNULL);
		}
        ;

interface_block_member_list :
	interface_block_member
	| interface_block_member_list interface_block_member
	;

interface_block_member_declarator_list :
	interface_block_member_declarator
		{ $$ = slParseInterfaceBlockMember(Compiler, $<dataType>0, $1); }
	| interface_block_member_declarator_list ',' interface_block_member_declarator
		{ $$ = slParseInterfaceBlockMember(Compiler, $1, $3); }
	;

interface_block_member :
	fully_specified_type interface_block_member_declarator_list ';'
	;
...
interface_block_member_declarator :
	...
	| T_IDENTIFIER '[' constant_expression ']'
		{ $$ = slParseFieldDecl(Compiler, &$1, $3); }
	| T_IDENTIFIER '[' ']'
		{ $$ = slParseImplicitArraySizeFieldDecl(Compiler, &$1); }
	| T_IDENTIFIER array_length_list
		{ $$ = slParseFieldListDecl(Compiler, &$1, $2, gcvTRUE); }
	;
...
selection_statement :
	...
	| T_SWITCH '(' integer_expression ')' switch_body
		{ $$ = slParseSwitchStatement(Compiler, &$1, $3, $5); }
	;
...
switch_body_statement:
	...
	| T_CASE constant_expression  ':'
		{ $$ = slParseCaseStatement(Compiler, &$1, $2); }
	| T_DEFAULT ':'
		{ $$ = slParseDefaultStatement(Compiler, &$1); }
	;
...
switch_body :
	...
	| '{'
		...
		switch_body_statement_list '}'
		{ $$ = slParseSwitchBodyEnd(Compiler, &$1, $3); }
	;
...
function_definition :
	function_prototype
        { slParseFuncDefinitionBegin(Compiler, $1); }
    compound_statement_no_new_scope
        { slParseFuncDefinitionEnd(Compiler, $1); }
		{ slParseFuncDef(Compiler, $1, $3); }
	;
