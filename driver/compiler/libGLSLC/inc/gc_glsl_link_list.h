...
#define slsDLINK_LIST_Replace(src, tgt) \
    { \
        ((slsDLINK_NODE *)(src))->prev = ((slsDLINK_NODE *)(tgt))->prev; \
        ((slsDLINK_NODE *)(src))->next = ((slsDLINK_NODE *)(tgt))->next; \
        if(((slsDLINK_NODE *)(tgt))->prev) \
        { \
            ((slsDLINK_NODE *)(tgt))->prev->next = (slsDLINK_NODE *)src; \
        } \
        if(((slsDLINK_NODE *)(tgt))->next) \
        { \
            ((slsDLINK_NODE *)(tgt))->next->prev = (slsDLINK_NODE *)src; \
        } \
    }
...
#define slsSLINK_NODE_Invalid(node) \
    do \
    { \
        (node)->next    = (slsSLINK_NODE *)INVALID_ADDRESS; \
    } \
    while (gcvFALSE)
#else
#define slsSLINK_NODE_Invalid(node) \
...
#define slsSLINK_LIST_DetachFirst(list, nodeType, firstNode) \
    ...
    { \
        *(firstNode) = slsSLINK_LIST_First((list), nodeType); \
        slsSLINK_NODE_Detach((slsSLINK_NODE *)*(firstNode), (list)); \
    } \
    while (gcvFALSE)
