...
{
    if(Bis != gcvNULL &&
      (Bis->curpos > 0 && Bis->curpos <= Bis->count)) {

        return Bis->src[Bis->curpos - 1];
    }
    else return ppvCHAR_EOF;
}
                              ...
                              gctBOOL               WhiteSpace)
{
    ...
    if(ntoken->type == ppvTokenType_ERROR)
    {
        ppoPREPROCESSOR_Report(
            PP,
            PP->doWeInValidArea ? slvREPORT_INTERNAL_ERROR : slvREPORT_WARN,
    ...
    *Token = ntoken;
    ...
    (*Token)->srcFileLine = PP->currentSourceFileLineNumber;
    ...
    )
{
    ...
    char c1,c2;
    gctUINT32 version = sloCOMPILER_GetLanguageVersion(PP->compiler);
