...
1. Files layout of android-addon
================================
<AQROOT>/
    |
    ...
    +---compiler
    |   |
    |   |---libVSC                   : Make file for compiler backend
    |   |
    |   \---libGLSLC                 : Make file for compiler frontend
    |
    \---driver
        |
        +---khronos
        ...
        |   |
        |   \---libOpenVG_3D         : Make file for 3D OVG driver
        |
        \---android
            |
...
====================
  ...
  7) Check out build result.
        ...
        $OUT/system/lib/libVSC.so
        $OUT/system/lib/hw/gralloc.<property>.so
        $OUT/system/lib/hw/copybit.<property>.so    # for froyo and gingerbread;
        $OUT/system/lib/hw/hwcomposer.<property>.so # for honeycomb and later
...
================
   ...
   To enable/disable an option, set <OPTION>=<value> in the command line.
    ...
    gcdFPGA_BUILD                1     To fix a FPGA special issue;
