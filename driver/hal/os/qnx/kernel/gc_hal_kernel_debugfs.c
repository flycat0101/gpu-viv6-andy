    ...
    )
{
    ...
    _PrintCounter(&database->contiguous, "contiguous");
    _PrintCounter(&database->mapUserMemory, "mapUserMemory");
...
static int debugfs_read_meminfo()
{
    ...
    _CounterPrint(&contiguousCounter, "gcvPOOL_CONTIGUOUS");
...
static int debugfs_read_idle()
{
    ...
    /* Idle time since last call */
    debugfs_print("Start:   %llu ns\n", start);
    debugfs_print("End:     %llu ns\n", end);
