    ...
    )
{
    ...
    if (currentType == gcvHARDWARE_VG)
    {
        status = gcoVGHARDWARE_GetProductName(gcvNULL, ProductName);
    }
    else
#endif
    {
...
#if gcdENABLE_2D
typedef struct _gcsUSER_MEMORY_INFO
{
    gctUINT32 node;
}
gcsUSER_MEMORY_INFO;
...
{
    gceSTATUS status;
    gcsUSER_MEMORY_INFO *info;
    gcsUSER_MEMORY_DESC desc;

    gcmHEADER();

    /* It is for 2D only. */
    gcmVERIFY_ARGUMENT(gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_PIPE_2D));

    gcmONERROR(gcoOS_Allocate(gcvNULL, gcmSIZEOF(gcsUSER_MEMORY_INFO), (gctPOINTER *)&info));

    gcoOS_ZeroMemory(info, gcmSIZEOF(gcsUSER_MEMORY_INFO));

    desc.flag = gcvALLOC_FLAG_EXTERNAL_MEMORY;
...
{
    gcsUSER_MEMORY_INFO * info = (gcsUSER_MEMORY_INFO *)Handle;
...
{
    gcoHARDWARE_AlignToTileCompatible(gcvNULL, type & 0xFF,
                 0, format, width, height, 1,
