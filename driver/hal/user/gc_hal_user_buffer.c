    ...
    )
{
    ...
    gcmASSERT(Buffer->hal != gcvNULL);

    if (Buffer->hal->commitMutex != gcvNULL)
    {
        gcmONERROR(gcoOS_AcquireMutex(gcvNULL, Buffer->hal->commitMutex, gcvINFINITE));
    }
    gcmFOOTER_NO();
    return gcvSTATUS_OK;
    ...
    )
{
    ...
    if (Buffer->hal->commitMutex != gcvNULL)
    {
        gcmONERROR(gcoOS_ReleaseMutex(gcvNULL, Buffer->hal->commitMutex));
    }
    gcmFOOTER_NO();
    return gcvSTATUS_OK;
    ...
    )
{
    ...
    gcmONERROR(gcoOS_CreateMutex(gcvNULL, &worker->mutex));
...
OnError:
    ...
    if (worker)
    {
        ...
        if (worker->mutex)
        {
            gcoOS_DeleteMutex(gcvNULL, worker->mutex);
        }
    ...
    )
{
    ...
    if (Worker->mutex)
    {
        gcmVERIFY_OK(gcoOS_DeleteMutex(gcvNULL, Worker->mutex));
    }
    ...
    )
{
    ...
    gctBOOL commit = gcvFALSE;
    gcoWorkerInfo *currWorker = gcvNULL;
    gcoWorkerInfo *deltaWorker = gcvNULL;
    gcoCommitWorker *commitWorker = gcvNULL;

    gcmHEADER_ARG("ThreadData=0x%x", ThreadData);

    commitWorker = (gcoCommitWorker *)ThreadData;

    while (gcvTRUE)
    {
        /* Wait for the start signal. */
        if gcmIS_ERROR(gcoOS_WaitSignal(gcvNULL, commitWorker->startSignal, gcvINFINITE))
        {
            break;
        }

        /* Check the thread's stop signal. */
        if (gcmIS_SUCCESS(gcoOS_WaitSignal(gcvNULL, commitWorker->stopSignal, 0)))
        {
        ...
        for (;;)
        {
            currWorker = commitWorker->workerFifo[commitWorker->workerHead];
            ...
            gcmVERIFY_OK(gcoOS_AcquireMutex(gcvNULL, currWorker->mutex, gcvINFINITE));
            commit = currWorker->commit;
            gcmVERIFY_OK(gcoOS_ReleaseMutex(gcvNULL, currWorker->mutex));
            ...
            if (currWorker->queue->head)
            {
                ...
                while (nextWorker != currWorker)
                {
                    gcmVERIFY_OK(gcoOS_AcquireMutex(gcvNULL, nextWorker->mutex, gcvINFINITE));
                    commit = nextWorker->commit;
                    gcmVERIFY_OK(gcoOS_ReleaseMutex(gcvNULL, nextWorker->mutex));

                    if (commit != gcvTRUE || nextWorker->emptyBuffer != gcvTRUE
                    ...
                    nextWorker = commitWorker->workerFifo[(commitWorker->workerHead + 1) % gcmCOUNTOF(commitWorker->workerFifo)];
                }
            }

            if ((currWorker->hardwareType != gcvHARDWARE_2D) && !currWorker->emptyBuffer)
            {
                deltaWorker = currWorker->buffer->deltaWorker;
            }
            else
            {
                deltaWorker = gcvNULL;
            ...
            gcoBUFFER_Commit_Worker(currWorker, deltaWorker ? deltaWorker->stateDelta : gcvNULL);

            if (deltaWorker && currWorker->stateDelta)
            {
    ...
    for (i = 0; i < gcmCOUNTOF(commitWorker->workerFifo); i++)
    {
        if(commitWorker->workerFifo[i])
        {
            gcoFreeWorker(gcvNULL, commitWorker->workerFifo[i]);
            commitWorker->workerFifo[i] = gcvNULL;
        }
    }
    ...
    )
{
    ...
    */
    ...
    gcoOS_ZeroMemory(buffer->deltaWorker, gcmSIZEOF(gcoWorkerInfo));

    gcoSuspendWorker(buffer);

    commitWorker = Hal->commitWorker;

    if (!commitWorker)
    {
        gcmONERROR(gcoOS_Allocate(gcvNULL, gcmSIZEOF(gcoCommitWorker), (gctPOINTER *)&commitWorker));
        gcoOS_ZeroMemory(commitWorker, gcmSIZEOF(gcoCommitWorker));

        gcmONERROR(gcoOS_CreateSignal(gcvNULL, gcvFALSE, &commitWorker->startSignal));
    ...
    if (commitWorker)
    {
        ...
        if (!commitWorker->workerRef)
        {
            ...
            gcmVERIFY_OK(gcoOS_Signal(
                gcvNULL,
                commitWorker->stopSignal,
                gcvTRUE
                ));

            /* Set thread's start signal. */
            gcmASSERT(commitWorker->startSignal != gcvNULL);
            gcmVERIFY_OK(gcoOS_Signal(
                gcvNULL,
                commitWorker->startSignal,
                gcvTRUE
                ));
    ...
    )
{
        ...
        if (tailCommandBuffer->using3D)
        {
                ...
                Buffer->inRerserved == gcvFALSE)
            {
                ...
                if (signal)
                {
                    ...
                    iface.u.Signal.auxSignal = 0;
                    iface.u.Signal.process   = gcmPTR_TO_UINT64(Buffer->commitProcess);
    ...
    worker = gcoGetWorker(Queue, Buffer, emptyBuffer);
    ...
    )
{
    gceSTATUS status;
    gcsHAL_INTERFACE iface;
    gcoQUEUE Queue = Worker->queue;

    gcmHEADER_ARG("Worker=0x%x StateDelta=0x%x", Worker, StateDelta);
    ...
    else
    {
        ...
        gcoHARDWARE Hardware = Worker->buffer->hardware;
        gcoCMDBUF CommandBuffer = Worker->commandBuffer;
        ...
        {
            gctUINT32 i;
            gctUINT32_PTR Contexts =  Worker->contexts;
