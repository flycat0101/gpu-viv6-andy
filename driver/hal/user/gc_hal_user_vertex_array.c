    ...
    )
{
    ...
    gcmHEADER_ARG("Vertex=0x%x EnableBits=0x%x VertexArray=0x%x First=%u "
                  ...
                  Vertex, EnableBits, VertexArray, First, gcmOPT_VALUE(Count), InstanceCount,
    ...
    ***** Phase 2: Index Management *******************************************/
    ...
    else
    {
        ...
        if (IndexObject != gcvNULL)
        {
            ...
            {
                if (InstanceCount > 1 || *Count <= *SpilitCount)
                {
                    bytes = 0;
                    ...
                    if (patchId == gcvPATCH_GTFES30)
                    {
                        gcoINDEX_WaitFence(IndexObject, gcvFENCE_TYPE_WRITE);
                    }
#endif
    ...
    )
{
    ...
    ***** Phase 2: Index Management *******************************************/
        ...
        if (IndexObject != gcvNULL)
    {
        /* fixed gc5000 hang issue , if the last index address locates in 0-47 every 64 bytes,it will cause hang */
        if ((gcoHAL_IsFeatureAvailable(gcvNULL,gcvFEATURE_INDEX_FETCH_FIX) != gcvSTATUS_TRUE)
            && ( PrimitiveRestart || gcvSTATUS_TRUE == _SpilitIndex(IndexObject, gcvNULL, IndexMemory, IndexType, PrimitiveType, *Count, SpilitCount))
            && (InstanceCount == 1)
           )
        {
            gcoHARDWARE_GetPatchID(gcvNULL, &patchId);
            if (PrimitiveRestart || InstanceCount > 1 || *Count <= *SpilitCount)
            {
                bytes = 0;
                gcmCOMPUTE_INDEX_BYTES((gctUINT)*Count);
#if gcdSYNC
                if (patchId == gcvPATCH_GTFES30)
                {
                    gcoBUFOBJ_WaitFence(IndexObject, gcvFENCE_TYPE_WRITE);
                }
#endif
