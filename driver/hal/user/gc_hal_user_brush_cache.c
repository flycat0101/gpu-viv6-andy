    ...
    do
    {
        ...
        if (BrushCache->curAllocated < BrushCache->maxCached)
        {
...

            {
                ...
                gcmGETHARDWAREADDRESS(node->patternNode, address);
    ...
    )
{
    ...
    while (BrushCache->curAllocated > BrushCache->maxCached)
    {
        ...
        gcmONERROR(gcoHARDWARE_Unlock(&node->patternNode, gcvSURF_BITMAP));

        gcmONERROR(gcsSURF_NODE_Destroy(&node->patternNode));
